...
function withPreviewQuery(path: string, currentQuery: Record<string, unknown>) {
  if (import.meta.env.DEV && currentQuery.wui_preview === '1') {
    return { path, query: { ...currentQuery, wui_preview: '1' } }
  }
  return path
}
...
const routes: RouteRecordRaw[] = [
  ...
  { path: '/home-v2', name: 'HomeV2', component: () => import('@/views/HomeV2.vue'), meta: { requiresAuth: false } },
  { path: '/home-v3', name: 'HomeV3', component: () => import('@/views/HomeV3.vue'), meta: { requiresAuth: false } },
  { path: '/home-v4', name: 'HomeV4', component: () => import('@/views/HomeV4.vue'), meta: { requiresAuth: false } },
  { path: '/home-v5', name: 'HomeV5', component: () => import('@/views/HomeV5.vue'), meta: { requiresAuth: false } },
  { path: '/home-v6', name: 'HomeV6', component: () => import('@/views/HomeV6.vue'), meta: { requiresAuth: false } },
  { path: '/home-v7', name: 'HomeV7', component: () => import('@/views/HomeV7.vue'), meta: { requiresAuth: false } },
  {
    path: '/login',
    name: 'Login',
...
router.beforeEach(async (to, _from, next) => {
  ...
  if (to.name === 'CustomPage') {
    ...
    if (menuItem?.label) {
      const siteName = appStore.siteName || 'CCF'
  ...
  if (!requiresAuth) {
    ...
    if (authStore.isAuthenticated && (to.path === '/login' || to.path === '/register')) {
      ...
      next(withPreviewQuery(authStore.isAdmin ? '/admin/dashboard' : '/dashboard', to.query))
      return
    }
    ...
    if (appStore.backendModeEnabled && !authStore.isAuthenticated) {
      ...
      if (!isAllowed) {
        next(withPreviewQuery('/login', to.query))
        return
      }
    }
    next()
  ...
  if (!authStore.isAuthenticated) {
    ...
    next({
      path: '/login',
      query: { ...to.query, redirect: to.fullPath } // Save intended destination
    })
    return
  }
  ...
  if (requiresAdmin && !authStore.isAdmin) {
    ...
    next(withPreviewQuery('/dashboard', to.query))
    return
  }
  ...
  if (to.meta.requiresRiskControl) {
    ...
    if (!riskControlEnabled) {
      next(withPreviewQuery(authStore.isAdmin ? '/admin/settings' : '/dashboard', to.query))
      return
    }
  }
