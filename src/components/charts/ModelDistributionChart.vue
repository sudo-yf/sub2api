  ...
  <div class="card p-4">
    <div class="mb-4 flex items-center justify-between gap-3">
      ...
      <div class="flex flex-wrap items-center justify-end gap-2">
        ...
        <div v-if="enableRankingView" class="inline-flex rounded-lg bg-gray-100 p-1 dark:bg-neutral-800">
          <button
            type="button"
            class="rounded-md px-2.5 py-1 text-xs font-medium transition-colors"
            :class="
        ...
        <table class="w-full text-xs">
          ...
          <tbody>
            <template v-for="model in displayModelStats" :key="model.model">
              <tr
                class="border-t border-gray-100 cursor-pointer transition-colors hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-neutral-900/40"
                @click="toggleBreakdown('model', model.model)"
              >
                <td
                  class="max-w-[100px] truncate py-1.5 font-medium text-[#81a1c1]/80 hover:text-blue-400/80 dark:text-[#81a1c1] dark:hover:text-blue-400/70"
                  :title="model.model"
                >
        ...
        <table class="w-full text-xs">
          ...
          <tbody>
            <tr
              ...
              :class="item.isOther
                ? 'bg-gray-50/70 dark:bg-neutral-900/20'
                : 'cursor-pointer hover:bg-[#161616]/40'"
              @click="item.isOther ? undefined : emit('ranking-click', item)"
            >
              <td class="py-1.5">
                <div class="flex min-w-0 items-center gap-2">
                  <span class="shrink-0 text-[11px] font-semibold text-neutral-400">
                    {{ item.isOther ? 'Σ' : `#${index + 1}` }}
                  </span>
                  <span
                    class="block max-w-[140px] truncate font-medium text-neutral-100"
