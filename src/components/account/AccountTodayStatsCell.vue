<template>
  <div>
    ...
    <div v-if="props.loading && !props.stats" class="space-y-0.5">
      ...
      <div class="h-3 w-16 animate-pulse rounded bg-gray-200 dark:bg-[#252525]"></div>
      <div class="h-3 w-10 animate-pulse rounded bg-gray-200 dark:bg-[#252525]"></div>
    </div>

    <!-- Error state -->
    <div v-else-if="props.error && !props.stats" class="text-xs text-[#bf616a]/80">
      {{ props.error }}
    </div>
    ...
    <div v-else-if="props.stats" class="space-y-0.5 text-xs">
      ...
      <!-- Cost (Account) -->
      <div class="flex items-center gap-1">
        <span class="text-neutral-500 dark:text-neutral-400">{{ t('usage.accountBilled') }}:</span>
        <span class="font-medium text-[#88c0d0]/80 dark:text-[#88c0d0]/80">{{
          formatCurrency(props.stats.cost)
        }}</span>
      </div>
      <!-- Cost (User/API Key) -->
      <div v-if="props.stats.user_cost != null" class="flex items-center gap-1">
        <span class="text-neutral-500 dark:text-neutral-400">{{ t('usage.userBilled') }}:</span>
        <span class="font-medium text-neutral-600 dark:text-neutral-300">{{
