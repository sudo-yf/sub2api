<template>
  <div ref="rootRef" v-if="showUsageWindows">
    ...
    >
      ...
      <div v-if="loading" class="space-y-1.5">
        ...
        <div class="flex items-center gap-1">
          <div class="h-3 w-[32px] animate-pulse rounded bg-gray-200 dark:bg-[#252525]"></div>
          <div class="h-1.5 w-8 animate-pulse rounded-full bg-gray-200 dark:bg-[#252525]"></div>
          <div class="h-3 w-[32px] animate-pulse rounded bg-gray-200 dark:bg-[#252525]"></div>
        </div>
      ...
      <!-- Error state -->
      <div v-else-if="error" class="text-xs text-[#bf616a]/80">
        {{ error }}
      </div>
      ...
      <div v-else-if="usageInfo" class="space-y-1">
        ...
        <div v-if="usageInfo.error" class="text-xs text-[#d08770]/80 dark:text-[#d08770] truncate max-w-[200px]" :title="usageInfo.error">
          {{ usageInfo.error }}
        </div>
        <!-- 5h Window -->
        <UsageProgressBar
        ...
        <div class="flex items-center gap-1.5 mt-0.5">
          <span
            ...
            class="text-[9px] text-neutral-400 dark:text-neutral-500 italic"
          >
            {{ t('admin.accounts.usageWindow.passiveSampled') }}
          </span>
          <button
            type="button"
            class="inline-flex items-center gap-0.5 rounded px-1.5 py-0.5 text-[9px] font-medium text-[#81a1c1]/80 hover:bg-[#81a1c1] dark:text-[#81a1c1] dark:hover:bg-[#81a1c1]/15 transition-colors"
    ...
    <template v-else-if="account.platform === 'antigravity' && account.type === 'oauth'">
      ...
      <div v-if="antigravityTierLabel" class="mb-1 flex items-center gap-1">
        ...
        >
          <svg
            class="h-3.5 w-3.5 text-[#bf616a]/80"
            fill="currentColor"
            viewBox="0 0 20 20"
          >
            <path
          ...
          <span
            class="pointer-events-none absolute left-0 top-full z-50 mt-1 w-80 whitespace-normal break-words rounded bg-neutral-900 px-3 py-2 text-xs leading-relaxed text-white opacity-0 shadow-lg transition-opacity group-hover:opacity-100 dark:bg-[#252525]"
          >
      ...
      <div v-if="isForbidden" class="space-y-1">
        ...
        <div v-if="validationURL" class="flex items-center gap-1">
          <a
            ...
            class="text-[10px] text-[#81a1c1]/80 hover:text-blue-400/80 hover:underline dark:text-[#81a1c1] dark:hover:text-blue-400/70"
          ...
          <button
            type="button"
            class="text-[10px] text-neutral-500 hover:text-neutral-400 dark:hover:text-neutral-500"
      ...
      <div v-else-if="needsReauth" class="space-y-1">
        <span class="inline-block rounded px-1.5 py-0.5 text-[10px] font-medium bg-[#ebcb8b] text-neutral-800 dark:bg-[#ebcb8b]/15 dark:text-[#ebcb8b]">
      ...
      <div v-else-if="hasAntigravityQuotaFromAPI" class="space-y-1">
        ...
        <div v-if="aiCreditsDisplay" class="mt-1 flex items-center gap-1 text-[10px] text-neutral-400">
          <Icon name="creditCard" size="xs" class="shrink-0" />{{ t('admin.accounts.aiCreditsBalance') }}: {{ aiCreditsDisplay }}
        </div>
      </div>
      <div v-else-if="aiCreditsDisplay" class="flex items-center gap-1 text-[10px] text-neutral-400">
        <Icon name="creditCard" size="xs" class="shrink-0" />{{ t('admin.accounts.aiCreditsBalance') }}: {{ aiCreditsDisplay }}
      </div>
      <div v-else class="text-xs text-neutral-400">-</div>
    </template>
    ...
    <template v-else-if="account.platform === 'gemini'">
      ...
      <div v-if="geminiAuthTypeLabel" class="mb-1 flex items-center gap-1">
        ...
        >
          <svg
            class="h-3.5 w-3.5 text-neutral-400 hover:text-gray-600 dark:text-neutral-500 dark:hover:text-neutral-400"
            fill="currentColor"
            viewBox="0 0 20 20"
          >
            <path
          ...
          >
            ...
            <div class="mb-2 text-neutral-400">{{ t('admin.accounts.gemini.quotaPolicy.note') }}</div>
            <div class="space-y-1">
              <div><strong>{{ geminiQuotaPolicyChannel }}:</strong></div>
              <div class="pl-2">• {{ geminiQuotaPolicyLimits }}</div>
              <div class="mt-2">
                <a :href="geminiQuotaPolicyDocsUrl" target="_blank" rel="noopener noreferrer" class="text-[#81a1c1]/80 hover:text-blue-400/70 underline">
      ...
      <div class="space-y-1">
        ...
        >
          <div class="flex items-center gap-1.5 text-[9px] text-neutral-400">
            <span class="rounded bg-gray-100 px-1.5 py-0.5 dark:bg-[#1e1e1e]">
              {{ formatKeyRequests }} req
            </span>
            <span class="rounded bg-gray-100 px-1.5 py-0.5 dark:bg-[#1e1e1e]">
              {{ formatKeyTokens }}
            </span>
            <span class="rounded bg-gray-100 px-1.5 py-0.5 dark:bg-[#1e1e1e]" :title="t('usage.accountBilled')">
              A ${{ formatKeyCost }}
            </span>
            <span
              v-if="todayStats.user_cost != null"
              class="rounded bg-gray-100 px-1.5 py-0.5 dark:bg-[#1e1e1e]"
        ...
        >
          <div class="h-3 w-10 animate-pulse rounded bg-gray-200 dark:bg-[#252525]"></div>
          <div class="h-3 w-8 animate-pulse rounded bg-gray-200 dark:bg-[#252525]"></div>
          <div class="h-3 w-12 animate-pulse rounded bg-gray-200 dark:bg-[#252525]"></div>
        </div>
        ...
        <div v-else-if="geminiUsageAvailable" class="space-y-1">
          <UsageProgressBar
            ...
            :color="bar.color"
          />
          <p class="mt-1 text-[9px] leading-tight text-neutral-400 dark:text-neutral-500 italic">
  ...
  <div ref="rootRef" v-else>
    ...
    <div v-else class="space-y-1">
      ...
      <!-- No data at all -->
      <div v-if="!todayStats && !todayStatsLoading && !hasApiKeyQuota" class="text-xs text-neutral-400">-</div>
    </div>
  </div>
</template>
...
const geminiTierClass = computed(() => {
  ...
  if (channel === 'google one') {
    if (level === 'ultra') return 'bg-[#b48ead] text-white dark:bg-[#b48ead]/12 dark:text-[#b48ead]'
    if (level === 'pro') return 'bg-[#81a1c1] text-white dark:bg-[#81a1c1]/12 dark:text-[#81a1c1]'
    return 'bg-gray-100 text-gray-600 dark:bg-[#252525] dark:text-neutral-300'
  }

  if (channel === 'gcp') {
    if (level === 'enterprise') return 'bg-[#b48ead] text-white dark:bg-[#b48ead]/12 dark:text-[#b48ead]'
    return 'bg-[#81a1c1] text-white dark:bg-[#81a1c1]/12 dark:text-[#81a1c1]'
  }
...
const antigravityTierClass = computed(() => {
  switch (antigravityTier.value) {
    ...
    case 'g1-pro-tier':
      return 'bg-[#81a1c1] text-white dark:bg-[#81a1c1]/12 dark:text-[#81a1c1]'
    case 'g1-ultra-tier':
      return 'bg-[#b48ead] text-white dark:bg-[#b48ead]/12 dark:text-[#b48ead]'
    default:
      return ''
  }
})
...
const forbiddenBadgeClass = computed(() => {
  if (forbiddenType.value === 'validation') {
    return 'bg-[#ebcb8b] text-neutral-900 dark:bg-[#ebcb8b]/12 dark:text-[#ebcb8b]'
  }
