    ...
    <div class="text-xs text-neutral-400 dark:text-neutral-500">
      ...
      <span
        ...
        :class="[
          'font-medium',
          isUrgent
            ? 'text-[#bf616a]/80 dark:text-[#bf616a] animate-pulse'
...
const tierBadgeClass = computed(() => {
  ...
  if (isCodeAssist.value) {
    const tier = (creds?.tier_id || '').toString().trim().toLowerCase()
    if (tier === 'gcp_enterprise') return 'bg-[#b48ead] text-white dark:bg-[#b48ead]/12 dark:text-[#b48ead]'
    if (tier === 'gcp_standard') return 'bg-[#81a1c1] text-white dark:bg-[#81a1c1]/12 dark:text-[#81a1c1]'
    // Backward compatibility
    const upper = (creds?.tier_id || '').toString().trim().toUpperCase()
    if (upper.includes('ULTRA') || upper.includes('ENTERPRISE')) return 'bg-[#b48ead] text-white dark:bg-[#b48ead]/12 dark:text-[#b48ead]'
    return 'bg-[#81a1c1] text-white dark:bg-[#81a1c1]/12 dark:text-[#81a1c1]'
  }

  if (isGoogleOne.value) {
    const tier = (creds?.tier_id || '').toString().trim().toLowerCase()
    if (tier === 'google_ai_ultra') return 'bg-[#b48ead] text-white dark:bg-[#b48ead]/12 dark:text-[#b48ead]'
    if (tier === 'google_ai_pro') return 'bg-[#81a1c1] text-white dark:bg-[#81a1c1]/12 dark:text-[#81a1c1]'
    if (tier === 'google_one_free') return 'bg-gray-100 text-gray-600 dark:bg-[#252525] dark:text-neutral-300'
    // Backward compatibility
    const upper = (creds?.tier_id || '').toString().trim().toUpperCase()
    if (upper === 'GOOGLE_ONE_UNLIMITED') return 'bg-[#b48ead] text-white dark:bg-[#b48ead]/12 dark:text-[#b48ead]'
    if (upper === 'AI_PREMIUM') return 'bg-[#81a1c1] text-white dark:bg-[#81a1c1]/12 dark:text-[#81a1c1]'
    return 'bg-gray-100 text-gray-600 dark:bg-[#252525] dark:text-neutral-300'
  }

  // AI Studio 默认样式：蓝色
  const tier = (creds?.tier_id || '').toString().trim().toLowerCase()
  if (tier === 'aistudio_paid') return 'bg-[#81a1c1] text-white dark:bg-[#81a1c1]/12 dark:text-[#81a1c1]'
  if (tier === 'aistudio_free') return 'bg-gray-100 text-gray-600 dark:bg-[#252525] dark:text-neutral-300'
  return 'bg-[#81a1c1] text-white dark:bg-[#81a1c1]/12 dark:text-[#81a1c1]'
})
