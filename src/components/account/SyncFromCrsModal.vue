<template>
  ...
  >
    ...
    >
      ...
      <div
        class="rounded-lg bg-gray-50 p-3 text-xs text-neutral-500 dark:bg-neutral-900/60 dark:text-neutral-400"
      >
      ...
      <div class="grid grid-cols-1 gap-4">
        ...
        <label class="flex items-center gap-2 text-sm text-neutral-300">
          <input
            ...
            type="checkbox"
            class="rounded border-gray-300 dark:border-neutral-800/60"
          />
    ...
    <div v-else-if="currentStep === 'preview' && previewResult" class="space-y-4">
      ...
      <div
        ...
        class="rounded-lg bg-gray-50 p-3 dark:bg-neutral-900/60"
      >
        <div class="mb-2 text-sm font-medium text-neutral-300">
          {{ t('admin.accounts.crsExistingAccounts') }}
          <span class="ml-1 text-xs text-neutral-400">({{ previewResult.existing_accounts.length }})</span>
        </div>
        <div class="max-h-32 overflow-auto text-xs text-neutral-400">
          <div
          ...
          >
            <span
              class="inline-block rounded bg-[#81a1c1]/12 px-1.5 py-0.5 text-[10px] font-medium text-white dark:bg-[#81a1c1]/12 dark:text-[#81a1c1]"
      ...
      <div v-if="previewResult.new_accounts.length">
        <div class="mb-2 flex items-center justify-between">
          <div class="text-sm font-medium text-neutral-100">
            ...
            <span class="ml-1 text-xs text-neutral-400">({{ previewResult.new_accounts.length }})</span>
          </div>
          <div class="flex gap-2">
            ...
            <button
              type="button"
              class="text-xs text-neutral-500 hover:text-neutral-400"
              @click="selectNone"
            >{{ t('admin.accounts.crsSelectNone') }}</button>
          </div>
        </div>
        <div
          class="max-h-48 overflow-auto rounded-lg border border-gray-200 p-2 dark:border-neutral-800/60"
        >
          <label
            v-for="acc in previewResult.new_accounts"
            :key="acc.crs_account_id"
            class="flex cursor-pointer items-center gap-2 rounded px-2 py-1.5 hover:bg-[#161616]/40"
          >
            <input
              type="checkbox"
            ...
            />
            <span
              class="inline-block rounded bg-[#a3be8c]/12 px-1.5 py-0.5 text-[10px] font-medium text-white dark:bg-[#a3be8c]/12 dark:text-[#a3be8c]"
            >{{ acc.platform }} / {{ acc.type }}</span>
            <span class="truncate text-sm text-neutral-300">{{ acc.name }}</span>
          </label>
        </div>
      ...
      <div class="flex items-center gap-2 text-xs text-neutral-400">
        ...
        <span :class="form.sync_proxies ? 'text-[#a3be8c]/80 dark:text-[#a3be8c]' : 'text-neutral-500'">
      ...
      <div
        ...
        class="rounded-lg bg-gray-50 p-4 text-center text-sm text-neutral-500 dark:bg-neutral-900/60 dark:text-neutral-400"
      >
    ...
    <div v-else-if="currentStep === 'result' && result" class="space-y-4">
      <div
        class="space-y-2 rounded-xl border border-gray-200 p-4 dark:border-neutral-800/60"
      >
        <div class="text-sm font-medium text-neutral-100">
        ...
        <div v-if="errorItems.length" class="mt-2">
          <div class="text-sm font-medium text-[#bf616a]/80 dark:text-[#bf616a]">
            {{ t('admin.accounts.syncErrors') }}
          </div>
          <div
            class="mt-2 max-h-48 overflow-auto rounded-lg bg-gray-50 p-3 font-mono text-xs dark:bg-neutral-800"
          >
