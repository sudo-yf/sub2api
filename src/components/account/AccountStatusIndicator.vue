<template>
  <div class="flex items-center gap-2">
    ...
    <div v-if="isRateLimited" class="flex flex-col items-center gap-1">
      ...
      <span class="text-[11px] text-neutral-400 dark:text-neutral-500">{{ rateLimitResumeText }}</span>
    </div>
    ...
    <div v-else-if="isOverloaded" class="flex flex-col items-center gap-1">
      ...
      <span class="text-[11px] text-neutral-400 dark:text-neutral-500">{{ overloadCountdown }}</span>
    </div>
    ...
    <div v-if="hasError && account.error_message" class="group/error relative">
      <svg
        class="h-4 w-4 cursor-help text-[#bf616a]/80 transition-colors hover:text-red-400/70 dark:text-[#bf616a] dark:hover:text-red-300"
        fill="none"
        viewBox="0 0 24 24"
        stroke="currentColor"
        stroke-width="2"
      ...
      <!-- Tooltip - 向下显示 -->
      <div
        class="invisible absolute left-0 top-full z-[100] mt-1.5 min-w-[200px] max-w-[300px] rounded-lg bg-[#1e1e1e] px-3 py-2 text-xs text-white opacity-0 shadow-xl transition-all duration-200 group-hover/error:visible group-hover/error:opacity-100 dark:bg-neutral-900"
      >
        <div class="whitespace-pre-wrap break-words leading-relaxed text-neutral-400">
    ...
    <div v-if="isRateLimited" class="group relative">
      <span
        class="inline-flex items-center gap-1 rounded bg-[#ebcb8b] px-1.5 py-0.5 text-xs font-medium text-[#d08770] dark:bg-[#ebcb8b]/15 dark:text-[#ebcb8b]"
      >
      ...
      <!-- Tooltip -->
      <div
        class="pointer-events-none absolute bottom-full left-1/2 z-50 mb-2 w-56 -translate-x-1/2 whitespace-normal rounded bg-neutral-900 px-3 py-2 text-center text-xs leading-relaxed text-white opacity-0 transition-opacity group-hover:opacity-100 dark:bg-[#252525]"
      >
    ...
    >
      <div v-for="item in activeModelStatuses" :key="`${item.kind}-${item.model}`" class="group relative mb-1 break-inside-avoid">
        ...
        <span
          ...
          class="inline-flex items-center gap-1 rounded bg-[#bf616a]/12 px-1.5 py-0.5 text-xs font-medium text-white dark:bg-[#bf616a]/12 dark:text-[#bf616a]"
        >
          <Icon name="exclamationTriangle" size="xs" :stroke-width="2" />
        ...
        <span
          v-else
          class="inline-flex items-center gap-1 rounded bg-[#b48ead]/12 px-1.5 py-0.5 text-xs font-medium text-white dark:bg-[#b48ead]/12 dark:text-[#b48ead]"
        >
          <Icon name="exclamationTriangle" size="xs" :stroke-width="2" />
          {{ formatScopeName(item.model) }}
          <span class="text-[10px] opacity-70">{{ formatModelResetTime(item.reset_at) }}</span>
