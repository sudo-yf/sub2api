<template>
  <div class="space-y-4">
    ...
    <div v-else-if="initError" class="card p-6 text-center">
      <p class="text-sm text-[#bf616a]/80 dark:text-[#bf616a]">{{ initError }}</p>
    ...
    <template v-else-if="success">
      <div class="card p-6">
        <div class="flex flex-col items-center space-y-4 py-4">
          ...
          <div class="w-full rounded-xl bg-gray-50 p-4 dark:bg-neutral-800">
            <div class="space-y-2 text-sm">
              <div class="flex justify-between">
                <span class="text-neutral-500 dark:text-neutral-400">{{ t('payment.orders.orderId') }}</span>
                <span class="font-medium text-neutral-100 dark:text-white">#{{ orderId }}</span>
              </div>
              <div v-if="amount > 0" class="flex justify-between">
                ...
                <span class="font-medium text-neutral-100 dark:text-white">{{ orderType === 'balance' ? '$' : '¥' }}{{ amount.toFixed(2) }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-neutral-500 dark:text-neutral-400">{{ t('payment.orders.payAmount') }}</span>
                <span class="font-medium text-neutral-100 dark:text-white">¥{{ payAmount.toFixed(2) }}</span>
              </div>
            </div>
          </div>
    ...
    <template v-else>
      ...
      <div class="card p-6">
        ...
        <p v-if="error" class="mt-4 text-sm text-[#bf616a]/80 dark:text-[#bf616a]">{{ error }}</p>
