    ...
    <form id="provider-form" @submit.prevent="handleSave" class="space-y-4">
      ...
      <div class="flex flex-wrap items-center gap-x-5 gap-y-2">
        ...
        <div v-if="supportsPaymentMode" class="flex items-center gap-2">
          <span class="text-xs font-medium text-neutral-500 dark:text-neutral-400">{{ t('admin.settings.payment.paymentMode') }}</span>
            ...
            <button
              ...
              :class="[
                ...
                form.payment_mode === mode.value
                  ? 'border-[#81a1c1]/60 bg-[#81a1c1] text-white shadow-sm'
        ...
        <div v-if="availableTypes.length > 1" class="flex items-center gap-2">
          <span class="text-xs font-medium text-neutral-500 dark:text-neutral-400">{{ t('admin.settings.payment.supportedTypes') }}</span>
      ...
      <div class="border-t border-gray-200 pt-4 dark:border-neutral-800/60">
        <div class="mb-3 flex items-center gap-2">
          <h4 class="text-sm font-semibold text-neutral-100 dark:text-white">
          ...
          <HelpTooltip v-if="paymentGuide" trigger="click" width-class="w-80">
            ...
            <div class="space-y-3">
              ...
              >
                ...
                <p><span class="text-neutral-400">{{ t('admin.settings.payment.guideOpenLabel') }}</span>{{ item.open }}</p>
                <p><span class="text-neutral-400">{{ t('admin.settings.payment.guideCallLabel') }}</span>{{ item.call }}</p>
                <p><span class="text-neutral-400">{{ t('admin.settings.payment.guideFallbackLabel') }}</span>{{ item.fallback }}</p>
              </div>
              <p v-if="paymentGuide.note" class="border-t border-white/10 pt-2 text-[11px] text-neutral-400">
                {{ paymentGuide.note }}
              </p>
            </div>
          </HelpTooltip>
        </div>
        <p v-if="paymentGuide" class="mb-3 text-xs text-neutral-500 dark:text-neutral-400">
        ...
        <div class="space-y-3">
          <div v-for="field in resolvedFields" :key="field.key">
            <label class="input-label">
              {{ field.label }}
              <span v-if="field.optional" class="text-xs text-neutral-400">({{ t('common.optional') }})</span>
              <span v-else class="text-[#bf616a]/80"> *</span>
            </label>
            <textarea
            ...
            <div v-else-if="field.sensitive" class="relative">
              ...
              <button
                ...
                class="absolute inset-y-0 right-0 flex items-center pr-3 text-neutral-400 hover:text-gray-600 dark:hover:text-neutral-400"
              >
            ...
            />
            <p v-if="field.hintKey" class="mt-1 text-xs leading-relaxed text-neutral-500 dark:text-neutral-400">
              {{ t(field.hintKey) }}
            </p>
          </div>
        </div>
        ...
        <div v-if="callbackPaths" class="mt-4 space-y-3">
          <div v-if="callbackPaths.notifyUrl">
            <label class="input-label">{{ t('admin.settings.payment.field_notifyUrl') }} <span class="text-[#bf616a]/80">*</span></label>
            <div class="flex">
              <input v-model="notifyBaseUrl" type="text" class="input min-w-0 flex-1 !rounded-r-none !border-r-0" :placeholder="defaultBaseUrl" />
              <span class="inline-flex items-center whitespace-nowrap rounded-r-lg border border-gray-300 bg-gray-50 px-3 text-xs text-neutral-500 dark:border-neutral-800/60 dark:bg-neutral-900 dark:text-neutral-400">{{ callbackPaths.notifyUrl }}</span>
            </div>
          </div>
          <div v-if="callbackPaths.returnUrl">
            <label class="input-label">{{ t('admin.settings.payment.field_returnUrl') }} <span class="text-[#bf616a]/80">*</span></label>
            <div class="flex">
              <input v-model="returnBaseUrl" type="text" class="input min-w-0 flex-1 !rounded-r-none !border-r-0" :placeholder="defaultBaseUrl" />
              <span class="inline-flex items-center whitespace-nowrap rounded-r-lg border border-gray-300 bg-gray-50 px-3 text-xs text-neutral-500 dark:border-neutral-800/60 dark:bg-neutral-900 dark:text-neutral-400">{{ callbackPaths.returnUrl }}</span>
            </div>
          </div>
        </div>

        <!-- 服务商 Webhook 提示 -->
        <div v-if="providerWebhookUrl" class="mt-3 rounded-lg border border-[#81a1c1]/20 bg-[#81a1c1] p-3 dark:border-[#81a1c1]/25 dark:bg-[#81a1c1]/12">
          <p class="text-xs text-[#81a1c1]/80 dark:text-[#81a1c1]">
            {{ t(providerWebhookHint) }}
          </p>
          <code class="mt-1 block break-all rounded bg-[#81a1c1]/12 px-2 py-1 text-xs text-white dark:bg-[#81a1c1]/12 dark:text-[#81a1c1]/70">
            {{ providerWebhookUrl }}
          </code>
          <p v-if="form.provider_key === 'stripe'" class="mt-2 text-xs leading-relaxed text-[#81a1c1]/80 dark:text-[#81a1c1]">
      ...
      <div v-if="limitableTypes.length" class="border-t border-gray-200 pt-4 dark:border-neutral-800/60">
        <button type="button" @click="limitsExpanded = !limitsExpanded" class="flex w-full items-center justify-between">
          ...
          <svg :class="['h-4 w-4 text-neutral-400 transition-transform', limitsExpanded && 'rotate-180']" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" /></svg>
        </button>
        <div v-show="limitsExpanded" class="mt-3 space-y-3">
          <div
            v-for="lt in limitableTypes"
            :key="lt.value"
            class="rounded-lg border border-gray-100 p-3 dark:border-neutral-800/60"
          >
            <p class="mb-2 text-xs font-medium text-neutral-600 dark:text-neutral-300">{{ lt.label }}</p>
            <div class="grid grid-cols-3 gap-3">
              <div>
                <label class="text-xs text-neutral-500 dark:text-neutral-400">{{ t('admin.settings.payment.limitSingleMin') }}</label>
                <input
                  type="number"
              ...
              <div>
                <label class="text-xs text-neutral-500 dark:text-neutral-400">{{ t('admin.settings.payment.limitSingleMax') }}</label>
                <input
                  type="number"
              ...
              <div>
                <label class="text-xs text-neutral-500 dark:text-neutral-400">{{ t('admin.settings.payment.limitDaily') }}</label>
                <input
                  type="number"
          ...
          <p class="text-xs text-neutral-400 dark:text-neutral-500">{{ t('admin.settings.payment.limitsHint') }}</p>
        </div>
      </div>
    </form>
