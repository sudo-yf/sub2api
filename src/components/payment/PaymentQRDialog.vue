<template>
  <BaseDialog :show="show" :title="dialogTitle" width="narrow" @close="handleClose">
    ...
    <div v-if="!success" class="flex flex-col items-center space-y-4">
      <!-- QR Code mode -->
      <template v-if="qrUrl">
        <div class="rounded-2xl bg-white p-4 shadow-sm dark:bg-neutral-800">
          <canvas ref="qrCanvas" class="mx-auto"></canvas>
        </div>
        <p v-if="scanHint" class="text-center text-sm text-neutral-500 dark:text-neutral-400">
          {{ scanHint }}
        </p>
      </template>
      ...
      <template v-else>
        <div class="flex flex-col items-center py-4">
          ...
          <p class="mt-4 text-sm text-neutral-500 dark:text-neutral-400">{{ t('payment.qr.payInNewWindowHint') }}</p>
      ...
      <div v-if="expired" class="text-center">
        <p class="text-lg font-medium text-[#bf616a]/80">{{ t('payment.qr.expired') }}</p>
      </div>
      <div v-else class="text-center">
        <p class="text-sm text-neutral-500 dark:text-neutral-400">{{ qrUrl ? t('payment.qr.expiresIn') : '' }}</p>
      ...
      <div v-if="paidOrder" class="w-full rounded-xl bg-gray-50 p-4 dark:bg-neutral-800">
        <div class="space-y-2 text-sm">
          ...
          <div class="flex justify-between">
            ...
            <span class="font-medium text-neutral-100 dark:text-white">{{ paidOrder.order_type === 'balance' ? '$' : '¥' }}{{ paidOrder.amount.toFixed(2) }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-neutral-500 dark:text-neutral-400">{{ t('payment.orders.payAmount') }}</span>
            <span class="font-medium text-neutral-100 dark:text-white">¥{{ paidOrder.pay_amount.toFixed(2) }}</span>
          </div>
        </div>
      </div>
    </div>
