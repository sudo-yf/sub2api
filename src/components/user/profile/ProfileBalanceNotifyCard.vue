<template>
  <div class="card">
    <div class="border-b border-gray-100 px-6 py-4 dark:border-neutral-800/60">
      <h2 class="text-lg font-medium text-neutral-100 dark:text-white">
    ...
    <div class="px-6 py-6 space-y-6">
      ...
      <div class="flex items-center justify-between">
        ...
        <label class="relative inline-flex items-center cursor-pointer">
          ...
          <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-neutral-400 dark:peer-focus:ring-neutral-700 rounded-full peer dark:bg-[#252525] peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:after:border-gray-600 peer-checked:bg-[#81a1c1]"></div>
        </label>
      </div>
      ...
      <template v-if="notifyEnabled">
        ...
        <div>
          <label class="input-label">
            ...
            <span class="text-xs text-neutral-400 ml-2">{{ t('profile.balanceNotify.thresholdHint') }}</span>
          </label>
        ...
        <div>
          ...
          <p class="mb-2 text-xs text-[#ebcb8b]/80 dark:text-[#ebcb8b]">{{ t('profile.balanceNotify.extraEmailsHint') }}</p>

          <!-- Saved email entries -->
          <div v-if="emailEntries.length > 0" class="space-y-2 mb-3">
            <div v-for="(entry, idx) in emailEntries" :key="idx"
              class="flex items-center justify-between px-3 py-2 bg-gray-50 dark:bg-neutral-900 rounded-lg">
              <div class="flex items-center gap-2 min-w-0 flex-1">
                <label class="relative inline-flex items-center cursor-pointer shrink-0">
                  <input type="checkbox" :checked="!entry.disabled" @change="handleEmailToggle(entry)" class="sr-only peer" />
                  <div class="w-9 h-5 bg-gray-200 peer-focus:outline-none rounded-full peer dark:bg-gray-600 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all dark:after:border-gray-500 peer-checked:bg-[#81a1c1]"></div>
                </label>
                <span class="text-sm text-neutral-600 dark:text-neutral-300 truncate">{{ entry.email }}</span>
              </div>
                ...
                <template v-if="!entry.verified">
                  ...
                  <template v-if="verifyingEmail === entry.email">
                    <input
                      ...
                      type="text"
                      maxlength="6"
                      class="w-20 rounded border border-gray-300 px-2 py-1 text-xs dark:border-neutral-700/60 dark:bg-neutral-900"
                      :placeholder="t('profile.balanceNotify.codePlaceholder')"
                    />
                    <button @click="verifySavedEmail(entry.email)" :disabled="!verifyCode || verifyCode.length !== 6 || verifyingSaved" class="text-xs text-neutral-300 hover:text-neutral-200">
                      {{ t('profile.balanceNotify.verify') }}
                    </button>
                    <span v-if="verifyCountdown > 0" class="text-xs text-neutral-400">{{ verifyCountdown }}s</span>
                    <button v-else @click="sendCodeForSaved(entry.email)" :disabled="sendingSavedCode" class="text-xs text-neutral-500 hover:text-neutral-400">
                      {{ t('profile.balanceNotify.resend') }}
                    </button>
                    <button @click="verifyingEmail = ''" class="text-xs text-neutral-400 hover:text-gray-600">
                      {{ t('common.cancel') }}
                    </button>
                  </template>
                  <template v-else>
                    <button @click="sendCodeForSaved(entry.email)" :disabled="sendingSavedCode" class="text-xs text-neutral-300 hover:text-neutral-200">
                      {{ t('profile.balanceNotify.verify') }}
                    </button>
                    <span class="text-xs text-[#ebcb8b]/80">{{ t('profile.balanceNotify.unverified') }}</span>
                  </template>
                </template>
                <span v-else class="text-xs text-[#a3be8c]/80">{{ t('profile.balanceNotify.verified') }}</span>
                <button @click="handleRemoveEmail(entry.email)" class="text-[#bf616a]/80 hover:text-red-400/80 text-xs">
          ...
          <div v-if="pendingEmails.length > 0" class="space-y-2 mb-3">
            <div v-for="(pe, idx) in pendingEmails" :key="pe.email"
              class="flex items-center gap-2 px-3 py-2 bg-[#ebcb8b] dark:bg-[#ebcb8b]/12 rounded-lg border border-[#ebcb8b]/20 dark:border-[#ebcb8b]/25">
              <span class="flex-1 text-sm text-neutral-600 dark:text-neutral-300">{{ pe.email }}</span>
              <div v-if="!pe.codeSent" class="flex items-center gap-1">
                <button @click="sendCodeFor(idx)" :disabled="pe.sending" class="text-xs text-neutral-300 hover:text-neutral-200">
                  {{ t('profile.balanceNotify.sendCode') }}
                </button>
                <button @click="pendingEmails.splice(idx, 1)" class="text-xs text-[#bf616a]/80 hover:text-red-400/80 ml-1">
                  {{ t('profile.balanceNotify.removeEmail') }}
                </button>
              </div>
              <div v-else class="flex items-center gap-1">
                <input
                  ...
                  :placeholder="t('profile.balanceNotify.codePlaceholder')"
                />
                <button @click="verifyPending(idx)" :disabled="!pe.code || pe.code.length !== 6 || pe.verifying" class="text-xs text-neutral-300 hover:text-neutral-200">
                  {{ t('profile.balanceNotify.verify') }}
                </button>
                <span v-if="pe.countdown > 0" class="text-xs text-neutral-400">{{ pe.countdown }}s</span>
                <button v-else @click="sendCodeFor(idx)" :disabled="pe.sending" class="text-xs text-neutral-500 hover:text-neutral-400">
                  {{ t('profile.balanceNotify.resend') }}
                </button>
              </div>
            </div>
