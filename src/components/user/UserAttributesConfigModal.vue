  ...
  <BaseDialog :show="show" :title="t('admin.users.attributes.title')" width="wide" @close="emit('close')">
    <div class="space-y-4">
      ...
      <div v-else-if="attributes.length === 0" class="py-12 text-center">
        <svg class="mx-auto h-12 w-12 text-neutral-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1">
      ...
      <div v-else class="max-h-96 space-y-2 overflow-y-auto">
        <div
          ...
          :key="attr.id"
          class="flex items-center gap-3 rounded-lg border border-gray-200 bg-white p-3 dark:border-neutral-800/60 dark:bg-neutral-800"
        >
          <!-- Drag Handle -->
          <div class="cursor-move text-neutral-400 hover:text-gray-600 dark:hover:text-neutral-400" :title="t('admin.users.attributes.dragToReorder')">
          ...
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-2">
              <span class="font-medium text-neutral-100">{{ attr.name }}</span>
              <span class="rounded bg-gray-100 px-1.5 py-0.5 font-mono text-xs text-neutral-500 dark:bg-neutral-900 dark:text-neutral-400">
                {{ attr.key }}
              </span>
          ...
          <div class="flex items-center gap-1">
            <button
              ...
              class="rounded-lg p-1.5 text-neutral-500 hover:bg-gray-100 hover:text-neutral-200 dark:hover:bg-neutral-900 dark:hover:text-neutral-300"
              :title="t('common.edit')"
            >
            ...
            <button
              ...
              class="rounded-lg p-1.5 text-neutral-500 hover:bg-[#bf616a] hover:text-red-400/70 dark:hover:bg-[#bf616a]/15 dark:hover:text-red-400"
    ...
    <form id="attribute-form" @submit.prevent="handleSave" class="space-y-4">
      ...
      <div v-if="form.type === 'select' || form.type === 'multi_select'" class="space-y-2">
        ...
        <div v-for="(option, index) in form.options" :key="getOptionKey(option)" class="flex items-center gap-2">
          ...
          <button
            ...
            class="rounded-lg p-1.5 text-neutral-500 hover:bg-[#bf616a] hover:text-red-400/70"
          >
      ...
      <div class="flex items-center gap-6">
        <label class="flex items-center gap-2">
          <input v-model="form.required" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-neutral-300" />
          <span class="text-sm text-neutral-300">{{ t('admin.users.attributes.required') }}</span>
        </label>
        <label class="flex items-center gap-2">
          <input v-model="form.enabled" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-neutral-300" />
          <span class="text-sm text-neutral-300">{{ t('admin.users.attributes.enabled') }}</span>
        </label>
      </div>
    </form>
