  ...
  <div v-if="attributes.length > 0" class="space-y-4">
    <div v-for="attr in attributes" :key="attr.id">
      <label class="input-label">
        {{ attr.name }}
        <span v-if="attr.required" class="text-[#bf616a]/80">*</span>
      </label>
      ...
      <div v-else-if="attr.type === 'multi_select'" class="space-y-2">
        ...
        >
          <input
            ...
            class="h-4 w-4 rounded border-gray-300 text-neutral-300"
          />
          <span class="text-sm text-neutral-600 dark:text-neutral-300">{{ opt.label }}</span>
        </label>
      </div>
