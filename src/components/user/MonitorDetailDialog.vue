<template>
  ...
  >
    ...
    <div v-else-if="!detail" class="py-8 text-center text-sm text-neutral-400">
      ...
      <table class="w-full text-left text-sm">
        ...
        <tbody>
          <tr
            ...
            :key="m.model"
            class="border-b border-neutral-800/60"
          >
            <td class="py-2 pr-3 font-medium text-gray-900 dark:text-neutral-500">{{ m.model }}</td>
            <td class="py-2 pr-3">
              <span
            ...
            <td class="py-2 pr-3 text-neutral-300">{{ formatLatency(m.latest_latency_ms) }}</td>
            <td class="py-2 pr-3 text-neutral-300">{{ formatPercent(m.availability_7d) }}</td>
            <td class="py-2 pr-3 text-neutral-300">{{ formatPercent(m.availability_15d) }}</td>
            <td class="py-2 pr-3 text-neutral-300">{{ formatPercent(m.availability_30d) }}</td>
            <td class="py-2 pr-3 text-neutral-300">{{ formatLatency(m.avg_latency_7d_ms) }}</td>
          </tr>
        </tbody>
      </table>
    </div>
