<template>
  <button
    type="button"
    class="group text-left p-5 rounded-2xl min-h-[280px] w-full bg-neutral-800/40 backdrop-blur-xl border border-neutral-800/60 shadow-card dark:bg-neutral-800/60 dark:border-neutral-800/60 hover:-translate-y-1 hover:shadow-card-hover dark:hover:border-neutral-700/40 hover:border-neutral-700/60 transition-all duration-300 ease-out flex flex-col"
    @click="emit('click')"
  >
    ...
    <div class="flex items-start gap-3">
      ...
      <div class="flex-1 min-w-0">
        <div class="text-base font-semibold truncate text-gray-900 dark:text-neutral-500">
          {{ item.name }}
        </div>
        <div class="mt-0.5 flex items-center gap-1.5 min-w-0">
          ...
          <span class="font-mono text-xs truncate text-neutral-500 dark:text-neutral-400">
            {{ item.primary_model }}
          </span>
          <span
            v-if="item.group_name"
            class="inline-flex items-center rounded-md px-1.5 py-0.5 text-[10px] font-medium bg-gray-100 text-gray-600 dark:bg-neutral-900 dark:text-neutral-300 flex-shrink-0"
          >
            {{ item.group_name }}
          </span>
        </div>
    ...
    <!-- Divider -->
    <div class="mt-4 border-t border-gray-100 dark:border-neutral-800/60"></div>
...
const PROVIDER_TINT: Record<string, string> = {
  openai: 'text-[#88c0d0]/80 dark:text-[#88c0d0]/80',
...
const providerTintClass = computed(() =>
  PROVIDER_TINT[props.item.provider] ?? 'text-neutral-500 dark:text-neutral-300'
)
