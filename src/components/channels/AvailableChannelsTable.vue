<template>
  <div class="card overflow-hidden">
    <table class="w-full table-fixed border-collapse text-sm">
      <thead>
        <tr class="border-b border-neutral-800/60 text-xs font-medium uppercase tracking-wide text-neutral-400" style="background-color: rgb(10 10 10 / 0.30);">
      ...
      <tbody v-if="loading">
        <tr>
          <td colspan="5" class="py-10 text-center">
            <Icon name="refresh" size="lg" class="inline-block animate-spin text-neutral-500" />
          </td>
        </tr>
      </tbody>
      <tbody v-else-if="rows.length === 0">
        <tr>
          <td colspan="5" class="py-12 text-center">
            <Icon name="inbox" size="xl" class="mx-auto mb-3 h-12 w-12 text-neutral-500" />
            <p class="text-sm text-neutral-400">{{ emptyLabel }}</p>
          </td>
        </tr>
      </tbody>
      ...
      >
        <tr
          ...
          :class="{ 'border-t border-neutral-800/60': secIdx > 0 }"
        >
          <!-- 渠道名：只在第一行渲染并用 rowspan 纵向合并 -->
          <td
            v-if="secIdx === 0"
            :rowspan="channel.platforms.length"
            class="px-4 py-3 text-center align-middle font-medium text-neutral-200"
          >
            {{ channel.name }}
          </td>
          ...
          <td
            v-if="secIdx === 0"
            :rowspan="channel.platforms.length"
            class="px-4 py-3 align-middle text-xs text-neutral-400"
          >
          ...
          <td class="align-top px-4 py-3">
            <div class="flex flex-col gap-1.5">
              ...
              >
                <span
                  class="inline-flex items-center gap-0.5 text-[10px] font-medium uppercase text-[#b48ead]/80 dark:text-[#b48ead]"
              ...
              >
                <span
                  class="inline-flex items-center gap-0.5 text-[10px] font-medium uppercase text-neutral-500 dark:text-neutral-400"
              ...
              <span v-if="section.groups.length === 0" class="text-xs text-neutral-400">-</span>
            </div>
          </td>
          ...
          <td class="align-top px-4 py-3">
            <div class="flex flex-wrap gap-1">
              ...
              />
              <span v-if="section.supported_models.length === 0" class="text-xs text-neutral-400">
                {{ noModelsLabel }}
              </span>
            </div>
          </td>
