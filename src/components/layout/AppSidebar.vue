<template>
  ...
  >
    ...
    <div class="sidebar-header" :class="{ 'sidebar-header-collapsed': sidebarCollapsed }">
      <!-- Logo icon — click to expand when collapsed -->
      <div
        class="sidebar-logo flex h-8 w-8 flex-shrink-0 items-center justify-center overflow-hidden rounded-lg border border-neutral-700/60 bg-neutral-800/80"
        :class="{ 'cursor-pointer hover:border-neutral-500': sidebarCollapsed }"
        :title="sidebarCollapsed ? t('nav.expand') : undefined"
        @click="sidebarCollapsed ? toggleSidebar() : undefined"
      >
        <img v-if="settingsLoaded" :src="siteLogo || '/logo.png'" alt="Logo" class="h-full w-full object-contain" />
      </div>
      <!-- Site name — hidden when collapsed -->
      <div class="sidebar-brand" :class="{ 'sidebar-brand-collapsed': sidebarCollapsed }" :aria-hidden="sidebarCollapsed ? 'true' : 'false'">
        <span class="sidebar-brand-title text-sm font-semibold text-neutral-200 leading-none">{{ siteName }}</span>
      </div>
      <!-- Collapse button — fixed right, always visible on desktop -->
      <button
        v-if="!sidebarCollapsed"
        @click="toggleSidebar"
        class="ml-auto hidden flex-shrink-0 items-center justify-center rounded-md border border-neutral-700/60 bg-neutral-800/60 p-1.5 text-neutral-400 transition-colors hover:border-neutral-600 hover:bg-neutral-800 hover:text-neutral-200 lg:flex"
        ...
        <div class="sidebar-section">
          <template v-for="item in adminNavItems" :key="item.path">
            ...
            <template v-if="item.children?.length">
              ...
              <!-- Children -->
              <div v-if="!sidebarCollapsed && isGroupExpanded(item)" class="mb-1 ml-4 border-l border-neutral-800/60 pl-2 dark:border-neutral-800/60">
                <router-link
                  v-for="child in item.children"
                  :key="child.path"
                  :to="getDevPreviewRoute(child.path)"
            ...
            <router-link
              v-else
              :to="getDevPreviewRoute(item.path)"
    ...
    <!-- Bottom Section: no theme toggle (moved to header) -->
    <div class="mt-auto p-3">
      <!-- User profile shortcut or blank space -->
    </div>
  </aside>
...
// import VersionBadge from '@/components/common/VersionBadge.vue' // disabled: CCF brand mark
import { sanitizeSvg } from '@/utils/sanitize'
import { getDevPreviewRoute } from '@/utils/devPreview'
...
// const siteVersion = computed(() => appStore.siteVersion) // disabled
...
// SunIcon/MoonIcon/toggleTheme/isDark moved to AppHeader
// const SunIcon = {
//   render: () => h('svg', { fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor', 'stroke-width': '1.5' }, [h('path', { 'stroke-linecap': 'round', 'stroke-linejoin': 'round', d: 'M12 3v2.25m6.364.386l-1.591 1.591M21 12h-2.25m-.386 6.364l-1.591-1.591M12 18.75V21m-4.773-4.227l-1.591 1.591M5.25 12H3m4.227-4.773L5.636 5.636M15.75 12a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0z' })])
// }
// const MoonIcon = {
//   render: () => h('svg', { fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor', 'stroke-width': '1.5' }, [h('path', { 'stroke-linecap': 'round', 'stroke-linejoin': 'round', d: 'M21.752 15.002A9.718 9.718 0 0118 15.75c-5.385 0-9.75-4.365-9.75-9.75 0-1.33.266-2.597.748-3.752A9.753 9.753 0 003 11.25C3 16.635 7.365 21 12.75 21a9.753 9.753 0 009.002-5.998z' })])
// }
...
// ChevronDoubleRightIcon no longer used (collapsed state: click logo to expand)
// const ChevronDoubleRightIcon = { render: () => h('svg', ...) }
...
// const flagAvailableChannels = makeSidebarFlag(FeatureFlags.availableChannels) // disabled
...
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  items.push(
    ...
    // { path: '/available-channels', label: t('nav.availableChannels'), icon: SignalIcon, hideInSimpleMode: true, featureFlag: flagAvailableChannels },
...
// toggleTheme moved to AppHeader
...
function handleGroupClick(item: NavItem) {
  ...
  if (route.path !== item.path) {
    router.push(getDevPreviewRoute(item.path))
  }
  if (!expandedGroups.value.has(item.path)) {
    expandedGroups.value.add(item.path)
  }
