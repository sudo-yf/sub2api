<template>
  ...
  >
    <div class="space-y-4">
      ...
      <div v-if="!platform" class="flex items-start gap-3 p-4 rounded-lg bg-[#ebcb8b]/12 dark:bg-[#ebcb8b]/12 border border-[#ebcb8b]/20 dark:border-[#ebcb8b]/25">
        <svg class="w-5 h-5 text-[#ebcb8b]/80 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
          <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" />
        </svg>
        <div>
          <p class="text-sm font-medium text-[#ebcb8b]/80 dark:text-[#ebcb8b]">
            {{ t('keys.useKeyModal.noGroupTitle') }}
          </p>
          <p class="text-sm text-[#ebcb8b]/80 dark:text-[#ebcb8b] mt-1">
      ...
      <template v-else>
        ...
        <!-- Client Tabs -->
        <div v-if="clientTabs.length" class="border-b border-gray-200 dark:border-neutral-800/60">
            ...
            <button
              ...
              :class="[
                ...
                activeClientTab === tab.id
                  ...
                  : 'border-transparent text-neutral-500 hover:text-neutral-400 hover:border-gray-300 dark:text-neutral-400 dark:hover:text-neutral-400'
              ]"
            >
        ...
        <!-- OS/Shell Tabs -->
        <div v-if="showShellTabs" class="border-b border-gray-200 dark:border-neutral-800/60">
        ...
        <div class="space-y-4">
          ...
          >
            <!-- File Hint (if exists) -->
            <p v-if="file.hint" class="text-xs text-[#d08770]/80 dark:text-[#d08770] mb-1.5 flex items-center gap-1">
              <Icon name="exclamationCircle" size="sm" class="flex-shrink-0" />
              {{ file.hint }}
            </p>
            <div class="bg-neutral-900 dark:bg-[#0d0d0d] rounded-xl overflow-hidden">
              <!-- Code Header -->
              <div class="flex items-center justify-between px-4 py-2 bg-[#1e1e1e] dark:bg-neutral-800 border-b border-gray-700 dark:border-neutral-800/60">
                <span class="text-xs text-neutral-400 font-mono">{{ file.path }}</span>
                <button
                  @click="copyContent(file.content, index)"
                  class="flex items-center gap-1.5 px-2.5 py-1 text-xs font-medium rounded-lg transition-colors"
                  :class="copiedIndex === index
                    ? 'bg-[#a3be8c]/12 text-[#a3be8c]/80'
                    : 'bg-[#252525] hover:bg-gray-600 text-neutral-400 hover:text-white'"
                >
              ...
              <!-- Code Content -->
              <pre class="p-4 text-sm font-mono text-neutral-500 overflow-x-auto"><code v-if="file.highlighted" v-html="file.highlighted"></code><code v-else v-text="file.content"></code></pre>
            </div>
          </div>
        </div>

        <!-- Usage Note -->
        <div v-if="showPlatformNote" class="flex items-start gap-3 p-3 rounded-lg bg-[#81a1c1]/12 dark:bg-[#81a1c1]/12 border border-[#81a1c1]/20 dark:border-[#81a1c1]/25">
          <Icon name="infoCircle" size="md" class="text-[#81a1c1]/80 flex-shrink-0 mt-0.5" />
...
const keyword = (value: string) => wrapToken('text-[#88c0d0]/80', value)
const variable = (value: string) => wrapToken('text-[#81a1c1]/80', value)
const operator = (value: string) => wrapToken('text-slate-400', value)
const string = (value: string) => wrapToken('text-[#d08770]/80', value)
