<template>
  <div v-if="hasActiveSubscriptions" class="relative" ref="containerRef">
    ...
    <button
      @click="toggleTooltip"
      class="flex cursor-pointer items-center gap-2 rounded-lg border border-neutral-700/30 bg-[#0d0d0d] px-3 py-1.5 transition-colors hover:bg-neutral-800 dark:bg-[#0d0d0d] dark:hover:bg-neutral-800"
      :title="t('subscriptionProgress.viewDetails')"
    >
      <Icon name="creditCard" size="sm" class="text-neutral-300 dark:text-neutral-300" />
      <div class="flex items-center gap-1.5">
        ...
        <span class="font-mono text-xs font-medium text-neutral-300 dark:text-neutral-300">
    ...
    <transition name="dropdown">
      <div
        v-if="tooltipOpen"
        class="absolute right-0 z-50 mt-2 w-[340px] overflow-hidden rounded-xl border border-neutral-800/60 bg-[#0d0d0d] shadow-xl dark:border-neutral-800/60 dark:bg-[#0d0d0d]"
      >
        <div class="border-b border-neutral-800/60 p-3 dark:border-neutral-800/60">
          <h3 class="font-headline text-sm font-semibold text-accent-50 dark:text-accent-50">
            {{ t('subscriptionProgress.title') }}
          </h3>
          <p class="mt-0.5 text-xs text-neutral-400/80 dark:text-neutral-400/80">
        ...
        <div class="max-h-64 overflow-y-auto">
          <div
            ...
            :key="subscription.id"
            class="border-b border-neutral-800/60 p-3 last:border-b-0 dark:border-neutral-800/60"
          >
            <div class="mb-2 flex items-center justify-between">
              <span class="text-sm font-medium text-accent-50 dark:text-accent-50">
            ...
            <div class="space-y-1.5">
              ...
              <div
                ...
                class="flex items-center gap-2 rounded-lg bg-emerald-900/20 px-2.5 py-1.5 dark:bg-[#88c0d0]/12"
              >
              ...
              <template v-else>
                <div v-if="subscription.group?.daily_limit_usd" class="flex items-center gap-2">
                  <span class="w-8 flex-shrink-0 text-[10px] text-neutral-400/80">{{
                    t('subscriptionProgress.daily')
                  }}</span>
                  <div class="h-1.5 min-w-0 flex-1 rounded-full bg-neutral-900 dark:bg-neutral-900">
                    <div
                  ...
                  <span class="w-24 flex-shrink-0 text-right text-[10px] text-neutral-400/80">
                    {{
        ...
        <div class="border-t border-neutral-800/60 p-2 dark:border-neutral-800/60">
          <router-link
            to="/subscriptions"
            @click="closeTooltip"
            class="block w-full py-1 text-center text-xs text-neutral-300 hover:underline dark:text-neutral-300"
          >
...
function getProgressDotClass(sub: UserSubscription): string {
  ...
  if (maxPercentage >= 90) return 'bg-[#bf616a]'
  if (maxPercentage >= 70) return 'bg-[#ebcb8b]'
  return 'bg-[#a3be8c]'
}
...
function getDaysRemainingClass(expiresAt: string): string {
  ...
  const diff = expires.getTime() - now.getTime()
  const days = Math.ceil(diff / (1000 * 60 * 60 * 24))
  if (days <= 3) return 'text-[#bf616a]/80 dark:text-[#bf616a]'
  if (days <= 7) return 'text-[#d08770]/80 dark:text-[#d08770]'
