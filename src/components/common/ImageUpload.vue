<template>
  <div class="flex items-start gap-4">
    ...
    <div class="flex-shrink-0">
      <div
        class="flex items-center justify-center overflow-hidden rounded-xl border-2 border-dashed border-gray-300 bg-gray-50 dark:border-neutral-800/60 dark:bg-neutral-800"
        :class="[previewSizeClass, { 'border-solid': !!modelValue }]"
      >
        <!-- SVG mode: render inline -->
        <span
          v-if="mode === 'svg' && modelValue"
          class="text-gray-600 dark:text-neutral-300 [&>svg]:h-full [&>svg]:w-full"
    ...
    <div class="flex-1 space-y-2">
      ...
      <p v-if="error" class="text-xs text-[#bf616a]/80">{{ error }}</p>
    </div>
  </div>
</template>
