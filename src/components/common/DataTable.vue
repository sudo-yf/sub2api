<template>
  <div v-if="!isDesktopViewport" class="space-y-3">
    <template v-if="loading">
      <div v-for="i in 5" :key="i" class="rounded-lg border border-neutral-800/60 bg-[#111111] p-4">
        <div class="space-y-3">
          <div v-for="column in dataColumns" :key="column.key" class="flex justify-between">
            <div class="h-4 w-20 animate-pulse rounded bg-neutral-800"></div>
            <div class="h-4 w-32 animate-pulse rounded bg-neutral-800"></div>
          </div>
          <div v-if="hasActionsColumn" class="border-t border-neutral-800/60 pt-3">
            <div class="h-8 w-full animate-pulse rounded bg-neutral-800"></div>
          </div>
        </div>
      </div>
    </template>
    ...
    <template v-else-if="!data || data.length === 0">
      <div class="rounded-lg border border-neutral-800/60 bg-[#111111] p-12 text-center">
        <slot name="empty">
          <div class="flex flex-col items-center">
            <Icon
              name="inbox"
              size="xl"
              class="mb-4 h-12 w-12 text-neutral-500"
            />
    ...
    <template v-else>
      <div
        ...
        class="rounded-lg border border-neutral-800/60 bg-[#111111] p-4"
      >
        <div class="space-y-3">
          <div
  ...
  >
    <table class="w-full min-w-max divide-y divide-neutral-800/60">
      <thead class="table-header bg-[#0d0d0d]">
        <tr>
          <th
            ...
            scope="col"
            :class="[
              'sticky-header-cell py-3 text-left text-xs font-medium uppercase tracking-wider text-neutral-400',
              getAdaptivePaddingClass(),
              { 'cursor-pointer hover:bg-[#161616]': column.sortable },
          ...
          >
            ...
            >
              <div class="flex items-center space-x-1">
                ...
                <span v-if="column.sortable" class="text-neutral-500">
                  <svg
      ...
      <tbody class="table-body divide-y divide-neutral-800/60 bg-[#0d0d0d]">
        <!-- Loading skeleton -->
        <tr v-if="loading" v-for="i in 5" :key="i">
          <td v-for="column in columns" :key="column.key" :class="['whitespace-nowrap py-4', getAdaptivePaddingClass()]">
            <div class="animate-pulse">
              <div class="h-4 w-3/4 rounded bg-neutral-800"></div>
            </div>
          </td>
        </tr>
        ...
        <tr v-else-if="!data || data.length === 0">
          <td
            ...
            :class="['py-12 text-center text-neutral-400', getAdaptivePaddingClass()]"
          >
            <slot name="empty">
              <div class="flex flex-col items-center">
                <Icon
        ...
        <template v-else>
          ...
          <tr
            ...
            :ref="measureElement"
            class="hover:bg-[#161616]"
          >
            <td
              v-for="(column, colIndex) in columns"
              :key="column.key"
              :class="[
                'whitespace-nowrap py-4 text-sm text-neutral-200',
                getAdaptivePaddingClass(),
