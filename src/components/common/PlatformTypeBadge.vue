<template>
  <div class="inline-flex flex-col gap-0.5 text-xs font-medium">
    ...
    <div v-if="expiresLabel" class="text-[10px] leading-tight text-neutral-400 dark:text-neutral-500 pl-0.5" :title="subscriptionExpiresAt">
      {{ expiresLabel }}
    </div>
  </div>
</template>
...
const platformClass = computed(() => {
  if (props.platform === 'anthropic') {
    return 'bg-[#ebcb8b]/15 text-[#d08770] dark:bg-[#ebcb8b]/15 dark:text-[#ebcb8b]'
  }
  if (props.platform === 'openai') {
    return 'bg-[#a3be8c]/15 text-[#a3be8c] dark:bg-[#a3be8c]/15 dark:text-[#a3be8c]'
  }
  if (props.platform === 'antigravity') {
    return 'bg-[#b48ead]/15 text-[#b48ead] dark:bg-[#b48ead]/15 dark:text-[#b48ead]'
  }
  return 'bg-[#81a1c1]/15 text-[#81a1c1] dark:bg-[#81a1c1]/15 dark:text-[#81a1c1]'
})
...
const privacyBadge = computed(() => {
  ...
  switch (props.privacyMode) {
    // OpenAI states
    case 'training_off':
      return { label: 'Private', icon: shieldCheck, title: t('admin.accounts.privacyTrainingOff'), class: 'bg-[#a3be8c] text-white dark:bg-[#a3be8c]/12 dark:text-[#a3be8c]' }
    case 'training_set_cf_blocked':
      return { label: 'CF', icon: shieldX, title: t('admin.accounts.privacyCfBlocked'), class: 'bg-[#ebcb8b] text-neutral-900 dark:bg-[#ebcb8b]/12 dark:text-[#ebcb8b]' }
    case 'training_set_failed':
      return { label: 'Fail', icon: shieldX, title: t('admin.accounts.privacyFailed'), class: 'bg-[#bf616a] text-white dark:bg-[#bf616a]/12 dark:text-[#bf616a]' }
    // Antigravity states
    case 'privacy_set':
      return { label: 'Private', icon: shieldCheck, title: t('admin.accounts.privacyAntigravitySet'), class: 'bg-[#a3be8c] text-white dark:bg-[#a3be8c]/12 dark:text-[#a3be8c]' }
    case 'privacy_set_failed':
      return { label: 'Fail', icon: shieldX, title: t('admin.accounts.privacyAntigravityFailed'), class: 'bg-[#bf616a] text-white dark:bg-[#bf616a]/12 dark:text-[#bf616a]' }
    default:
      return null
  }
})
