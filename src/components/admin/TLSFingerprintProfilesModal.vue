      ...
      <div v-else class="max-h-96 overflow-auto rounded-lg border border-neutral-800/60">
        <table class="min-w-full divide-y divide-neutral-800/60">
          ...
          <tbody class="divide-y divide-neutral-800/60 bg-[#0d0d0d]">
            <tr v-for="profile in profiles" :key="profile.id" class="hover:bg-[#161616]">
              <td class="px-3 py-2">
                <div class="font-medium text-neutral-100 text-sm">{{ profile.name }}</div>
              </td>
              <td class="px-3 py-2">
                <div v-if="profile.description" class="text-sm text-neutral-400 max-w-xs truncate">
                  {{ profile.description }}
                </div>
                <div v-else class="text-xs text-neutral-500 dark:text-gray-600">—</div>
              </td>
              <td class="px-3 py-2">
                <Icon
                  :name="profile.enable_grease ? 'check' : 'lock'"
                  size="sm"
                  :class="profile.enable_grease ? 'text-[#a3be8c]/80' : 'text-neutral-500'"
                />
              </td>
              <td class="px-3 py-2">
                <div v-if="profile.alpn_protocols?.length" class="flex flex-wrap gap-1">
                  ...
                  <span v-if="profile.alpn_protocols.length > 3" class="text-xs text-neutral-400">
      ...
      <form @submit.prevent="handleSubmit" class="space-y-4">
        ...
        <div>
          ...
          <div class="mt-1 flex items-center gap-2">
            ...
            <p class="text-xs text-neutral-400">
              ...
              <a href="https://tls.sub2api.org" target="_blank" rel="noopener noreferrer" class="text-neutral-300 hover:text-neutral-200 dark:text-neutral-400 dark:hover:text-neutral-300 underline">{{ t('admin.tlsFingerprintProfiles.form.openCollector') }}</a>
            </p>
          </div>
        </div>

        <hr class="border-neutral-800/60" />
        ...
        <div class="flex items-center gap-3">
          <button
            ...
            :class="[
              'relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-neutral-500 focus:ring-offset-2',
              form.enable_grease ? 'bg-[#81a1c1]' : 'bg-gray-200 dark:bg-neutral-800'
            ]"
          >
            <span
              :class="[
