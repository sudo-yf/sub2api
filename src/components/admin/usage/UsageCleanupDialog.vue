<template>
  <BaseDialog :show="show" :title="t('admin.usage.cleanup.title')" width="wide" @close="handleClose">
    <div class="space-y-4">
      ...
      <div class="rounded-xl border border-[#d08770]/20 bg-[#ebcb8b] px-4 py-3 text-sm text-neutral-800 dark:border-[#d08770]/25 dark:bg-[#1a1200]/10 dark:text-[#ebcb8b]">
        {{ t('admin.usage.cleanup.warning') }}
      </div>

      <div class="rounded-xl border border-gray-200 p-4 dark:border-neutral-800/60">
        <div class="flex items-center justify-between">
          <h4 class="text-sm font-semibold text-gray-700 dark:text-neutral-500">
        ...
        <div class="mt-3 space-y-2">
          <div v-if="tasksLoading" class="text-sm text-neutral-500 dark:text-neutral-400">
            {{ t('admin.usage.cleanup.loadingTasks') }}
          </div>
          <div v-else-if="tasks.length === 0" class="text-sm text-neutral-500 dark:text-neutral-400">
          ...
          <div v-else class="space-y-2">
            <div
              v-for="task in tasks"
              :key="task.id"
              class="flex flex-col gap-2 rounded-lg border border-gray-100 px-3 py-2 text-sm text-gray-600 dark:border-neutral-800/60 dark:text-neutral-300"
            >
              <div class="flex flex-wrap items-center justify-between gap-2">
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-xs text-neutral-400">#{{ task.id }}</span>
                  <button
                    v-if="canCancel(task)"
                    type="button"
                    class="btn btn-ghost btn-xs text-[#b48ead]/80 hover:text-rose-700 dark:text-[#b48ead]"
              ...
              <div v-if="task.error_message" class="text-xs text-[#b48ead]/80">
                {{ task.error_message }}
              </div>
            </div>
          </div>
...
const statusClass = (status: string) => {
  const map: Record<string, string> = {
    pending: 'bg-[#ebcb8b] text-neutral-800 dark:bg-[#1a1200]/20 dark:text-[#ebcb8b]',
    running: 'bg-[#81a1c1] text-white dark:bg-[#090e1a]/20 dark:text-[#81a1c1]/70',
    succeeded: 'bg-[#a3be8c] text-neutral-800 dark:bg-[#0a1a0f]/20 dark:text-[#a3be8c]',
    failed: 'bg-[#b48ead]/8 text-[#b48ead]/80 dark:bg-[#1a0a0d]/20 dark:text-[#b48ead]',
    canceled: 'bg-gray-200 text-gray-600 dark:bg-neutral-800 dark:text-neutral-300'
  }
