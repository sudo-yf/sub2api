  ...
  >
    <div class="space-y-4">
      ...
      <div v-if="loading" class="flex items-center justify-center py-8">
        <Icon name="refresh" size="lg" class="animate-spin text-neutral-500" />
      </div>
      ...
      <div v-else-if="rules.length === 0" class="py-8 text-center">
        <div class="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-neutral-900">
          <Icon name="shield" size="lg" class="text-neutral-500" />
        </div>
      ...
      <div v-else class="max-h-96 overflow-auto rounded-lg border border-neutral-800/60">
        <table class="min-w-full divide-y divide-neutral-800/60">
          <thead class="sticky top-0 bg-[#0d0d0d]">
            <tr>
              <th class="px-3 py-2 text-left text-xs font-medium uppercase text-neutral-400">
          ...
          <tbody class="divide-y divide-neutral-800/60 bg-[#0d0d0d]">
            <tr v-for="rule in rules" :key="rule.id" class="hover:bg-[#161616]">
              <td class="whitespace-nowrap px-3 py-2">
                <span class="inline-flex h-5 w-5 items-center justify-center rounded bg-neutral-800 text-xs font-medium text-neutral-300">
                  {{ rule.priority }}
                </span>
              </td>
              <td class="px-3 py-2">
                <div class="font-medium text-neutral-100 text-sm">{{ rule.name }}</div>
                <div v-if="rule.description" class="mt-0.5 text-xs text-neutral-400 max-w-xs truncate">
                  {{ rule.description }}
                </div>
              </td>
              <td class="px-3 py-2">
              ...
              <td class="px-3 py-2">
                <div v-if="rule.platforms.length === 0" class="text-xs text-neutral-400">
                ...
                <div v-else class="flex flex-wrap gap-1">
                  ...
                  <span v-if="rule.platforms.length > 2" class="text-xs text-neutral-400">
                ...
                <div class="text-xs space-y-0.5">
                  <div class="flex items-center gap-1">
                    <Icon
                      ...
                      size="xs"
                      :class="rule.passthrough_code ? 'text-[#a3be8c]/80' : 'text-neutral-500'"
                    />
                  ...
                  <div class="flex items-center gap-1">
                    <Icon
                      ...
                      size="xs"
                      :class="rule.passthrough_body ? 'text-[#a3be8c]/80' : 'text-neutral-500'"
                    />
                    <span class="text-neutral-400">
              ...
              <td class="px-3 py-2">
                <button
                  ...
                  :class="[
                    'relative inline-flex h-4 w-7 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-neutral-500 focus:ring-offset-2',
                    rule.enabled ? 'bg-[#81a1c1]' : 'bg-neutral-800'
                  ]"
                >
                  <span
                    :class="[
                      'pointer-events-none inline-block h-3 w-3 transform rounded-full bg-neutral-200 shadow ring-0 transition duration-200 ease-in-out',
              ...
              <td class="px-3 py-2">
                <div class="flex items-center gap-1">
                  <button
                    ...
                    class="p-1 text-neutral-400 hover:text-neutral-200 dark:hover:text-neutral-300"
                    :title="t('common.edit')"
                  >
                  ...
                  <button
                    ...
                    class="p-1 text-neutral-400 hover:text-red-400/70 dark:hover:text-red-400"
    ...
    >
      <form @submit.prevent="handleSubmit" class="space-y-4">
        ...
        <div class="rounded-lg border border-neutral-800/60 p-3">
          <h4 class="mb-2 text-sm font-medium text-neutral-100">
            ...
            <div class="mt-1 space-y-2">
              ...
              >
                <input
                  ...
                  class="mt-0.5 h-3.5 w-3.5 border-gray-300 text-neutral-300 focus:ring-neutral-500"
                />
                <div class="flex-1">
                  <span class="text-xs font-medium text-neutral-300">{{ option.label }}</span>
                  <p class="text-xs text-neutral-400">{{ option.description }}</p>
                </div>
              </label>
            </div>
          </div>
          ...
          <div class="mt-3">
            ...
            <div class="flex flex-wrap gap-3">
              ...
              >
                ...
                />
                <span class="text-xs text-neutral-300">{{ platform.label }}</span>
              </label>
            </div>
        ...
        <div class="flex items-center gap-1.5">
          <input
            ...
            class="h-3.5 w-3.5 rounded border-gray-300 text-[#ebcb8b]/80 focus:ring-yellow-500"
          />
          <span class="text-xs font-medium text-neutral-300">
