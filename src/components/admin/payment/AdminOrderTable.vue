  ...
  <div class="space-y-4">
    ...
    <DataTable :columns="columns" :data="orders" :loading="loading">
      ...
      <template #cell-user_id="{ value }">
        <span class="text-sm text-gray-600 dark:text-neutral-400">#{{ value }}</span>
      </template>

      <template #cell-pay_amount="{ value, row }">
        <div class="text-sm">
          <span class="font-medium text-neutral-100 dark:text-white">¥{{ value.toFixed(2) }}</span>
          <span v-if="row.fee_rate > 0" class="ml-1 text-xs text-neutral-400" :title="t('payment.orders.fee') + ': ' + row.fee_rate + '%'">
            ({{ row.fee_rate }}%)
          </span>
          <div v-if="row.amount !== row.pay_amount" class="text-xs text-neutral-500">
      ...
      <template #cell-created_at="{ value }">
        <span class="text-xs text-neutral-500 dark:text-neutral-400">{{ formatDateTime(value) }}</span>
      </template>
      ...
      <template #cell-actions="{ row }">
        <div class="flex items-center gap-2">
          <button
            ...
            class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-neutral-500 transition-colors hover:bg-gray-50 hover:text-neutral-400 dark:hover:bg-[#1e1e1e]/50 dark:hover:text-neutral-400"
          >
          ...
          <button
            ...
            class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-neutral-500 transition-colors hover:bg-[#ebcb8b] hover:text-yellow-500/70 dark:hover:bg-[#ebcb8b]/15 dark:hover:text-yellow-500/80"
          >
            <Icon name="x" size="sm" />
