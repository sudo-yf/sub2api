<template>
  <div class="flex items-start gap-2 rounded border p-2"
       :class="isEmpty ? 'border-[#bf616a]/20 bg-[#bf616a] dark:border-[#bf616a]/25 dark:bg-[#bf616a]/12' : 'border-gray-200 bg-white dark:border-neutral-700/60 dark:bg-neutral-900'">
    ...
    <template v-if="mode === 'token'">
      ...
      <div class="w-20">
        <label class="text-xs text-neutral-400">Max <span class="text-neutral-400">(含)</span></label>
      ...
      <div class="flex-1">
        <label class="text-xs text-neutral-400">{{ t('admin.channels.form.inputPrice', '输入') }} <span v-if="isEmpty" class="text-[#bf616a]/80">*</span> <span class="text-neutral-400">$/M</span></label>
      ...
      <div class="flex-1">
        <label class="text-xs text-neutral-400">{{ t('admin.channels.form.outputPrice', '输出') }} <span v-if="isEmpty" class="text-[#bf616a]/80">*</span> <span class="text-neutral-400">$/M</span></label>
        <input :value="interval.output_price" @input="emitField('output_price', ($event.target as HTMLInputElement).value)"
          type="number" step="any" min="0" class="input mt-0.5 text-xs" />
      </div>
      <div class="flex-1">
        <label class="text-xs text-neutral-400">{{ t('admin.channels.form.cacheWritePrice', '缓存W') }} <span class="text-neutral-400">$/M</span></label>
        <input :value="interval.cache_write_price" @input="emitField('cache_write_price', ($event.target as HTMLInputElement).value)"
          type="number" step="any" min="0" class="input mt-0.5 text-xs" />
      </div>
      <div class="flex-1">
        <label class="text-xs text-neutral-400">{{ t('admin.channels.form.cacheReadPrice', '缓存R') }} <span class="text-neutral-400">$/M</span></label>
    ...
    <template v-else>
      ...
      <div class="flex-1">
        <label class="text-xs text-neutral-400">{{ t('admin.channels.form.perRequestPrice', '单次价格') }} <span v-if="isEmpty" class="text-[#bf616a]/80">*</span> <span class="text-neutral-400">$</span></label>
        <input :value="interval.per_request_price" @input="emitField('per_request_price', ($event.target as HTMLInputElement).value)"
          type="number" step="any" min="0" class="input mt-0.5 text-xs" />
      </div>
    </template>

    <button type="button" @click="emit('remove')" class="mt-4 rounded p-0.5 text-neutral-400 hover:text-red-400/80">
      <Icon name="x" size="sm" />
    </button>
  </div>
</template>
