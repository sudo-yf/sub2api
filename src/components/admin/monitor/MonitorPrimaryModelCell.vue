<template>
  <div class="flex items-center gap-2">
    <span class="text-sm text-gray-900 dark:text-neutral-500">{{ row.primary_model }}</span>
    <HelpTooltip>
      <template #trigger>
        <span
      ...
      <div class="space-y-2">
        ...
        <div v-if="(row.extra_models?.length ?? 0) === 0" class="text-[11px] text-neutral-400">
          ...
          <table class="w-full text-left text-[11px]">
            ...
            <tbody>
              <tr v-for="m in (row.extra_models_status || [])" :key="m.model">
                <td class="py-0.5 pr-2 text-neutral-500">{{ m.model }}</td>
                ...
                <td class="py-0.5 text-neutral-500">{{ formatLatency(m.latency_ms) }}</td>
              </tr>
            </tbody>
          </table>
        </div>
