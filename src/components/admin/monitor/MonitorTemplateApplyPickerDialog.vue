<template>
  ...
  >
    ...
    <div v-if="loading" class="py-6 text-center text-sm text-neutral-400">
      {{ t('common.loading') }}
    </div>

    <div v-else-if="monitors.length === 0" class="py-6 text-center text-sm text-neutral-400">
    ...
    <div v-else>
      ...
      <div class="mb-2 flex items-center gap-3 text-xs">
        <button
          type="button"
          class="text-neutral-300 hover:underline dark:text-neutral-400"
          @click="selectAll"
        >
          {{ t('common.selectAll') }}
        </button>
      ...
      <ul class="max-h-80 divide-y divide-gray-100 overflow-y-auto rounded-lg border border-gray-200 dark:divide-dark-700 dark:border-neutral-800/60">
        <li
          v-for="m in monitors"
          :key="m.id"
          class="flex cursor-pointer items-center gap-3 px-3 py-2 hover:bg-gray-50 dark:hover:bg-neutral-800"
          @click="toggle(m.id)"
        >
          <input
            type="checkbox"
          ...
          />
          <span class="font-medium text-neutral-100 dark:text-white">{{ m.name }}</span>
          <span class="text-xs text-neutral-400">{{ m.provider }}</span>
          <span v-if="m.provider === 'openai'" class="text-xs text-neutral-400">{{ m.api_mode }}</span>
          <span
            v-if="!m.enabled"
            class="ml-auto rounded bg-gray-100 px-1.5 py-0.5 text-xs text-neutral-500 dark:bg-neutral-900 dark:text-neutral-400"
          >
