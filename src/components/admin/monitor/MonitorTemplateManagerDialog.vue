<template>
  ...
  >
    <!-- provider tabs -->
    <div class="mb-4 border-b border-gray-200 dark:border-neutral-800/60">
      <div role="tablist" class="flex gap-1">
        ...
        >
          ...
          <span
            ...
            class="ml-1.5 rounded-full bg-gray-100 px-2 py-0.5 text-xs dark:bg-neutral-900"
          >
    ...
    <div v-if="!editing" class="space-y-2">
      ...
      <div
        ...
        v-else
        :key="tpl.id"
        class="rounded-lg border border-gray-200 bg-white p-4 dark:border-neutral-800/60 dark:bg-neutral-800"
      >
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-2">
              <span class="font-medium text-neutral-100 dark:text-white">{{ tpl.name }}</span>
              <span
            ...
            <p v-if="tpl.description" class="mt-0.5 text-xs text-neutral-500 dark:text-neutral-400">
              {{ tpl.description }}
            </p>
          ...
          <div class="flex flex-shrink-0 gap-2">
            ...
            <button class="btn btn-secondary btn-sm text-[#bf616a]/80" @click="handleDelete(tpl)">
              {{ t('common.delete') }}
            </button>
          </div>
        </div>
...
function tabClass(value: Provider): string {
  return activeProvider.value === value
    ? 'border-b-2 border-neutral-600/60 text-neutral-300 dark:text-neutral-400'
    : 'border-b-2 border-transparent text-neutral-500 hover:text-neutral-400 dark:text-neutral-400 dark:hover:text-neutral-500'
}
...
function modeBadgeClass(mode: BodyOverrideMode): string {
  switch (mode) {
    ...
    case 'replace':
      return 'bg-[#b48ead] text-white dark:bg-[#0f0a1a]/15 dark:text-[#b48ead]'
    default:
...
function apiModeBadgeClass(mode: APIMode): string {
  if (normalizeAPIMode(mode) === API_MODE_RESPONSES) {
    return 'bg-[#81a1c1] text-white dark:bg-[#090e1a]/15 dark:text-[#81a1c1]'
  }
