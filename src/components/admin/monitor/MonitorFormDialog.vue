<template>
  ...
  >
    <form id="channel-monitor-form" @submit.prevent="handleSubmit" class="space-y-5">
      <div>
        <label class="input-label">{{ t('admin.channelMonitor.form.name') }} <span class="text-[#bf616a]/80">*</span></label>
        <input v-model="form.name" type="text" required class="input" :placeholder="t('admin.channelMonitor.form.namePlaceholder')" />
      </div>

      <div>
        <label class="input-label">{{ t('admin.channelMonitor.form.provider') }} <span class="text-[#bf616a]/80">*</span></label>
      ...
      <div v-if="form.provider === PROVIDER_OPENAI" class="rounded-lg border border-[#81a1c1]/20 bg-[#090e1a]/50 p-3 dark:border-[#81a1c1]/25 dark:bg-[#090e1a]/10">
      ...
      <div>
        <label class="input-label">{{ t('admin.channelMonitor.form.endpoint') }} <span class="text-[#bf616a]/80">*</span></label>
      ...
      <div>
        <label class="input-label">
          {{ t('admin.channelMonitor.form.apiKey') }}<span v-if="!editing" class="text-[#bf616a]/80"> *</span>
        </label>
        <div class="flex gap-2">
          <input
        ...
        <p v-if="editing && editing.api_key_masked" class="mt-1 text-xs text-neutral-400">{{ editing.api_key_masked }}</p>
      </div>

      <div>
        <label class="input-label">{{ t('admin.channelMonitor.form.primaryModel') }} <span class="text-[#bf616a]/80">*</span></label>
        <input
      ...
      <div>
        <label class="input-label">{{ t('admin.channelMonitor.form.intervalSeconds') }} <span class="text-[#bf616a]/80">*</span></label>
        <input v-model.number="form.interval_seconds" type="number" min="15" max="3600" required class="input" />
        <p class="mt-1 text-xs text-neutral-400">{{ t('admin.channelMonitor.form.intervalSecondsHint') }}</p>
      </div>
      ...
      <details class="rounded-lg border border-gray-200 bg-gray-50/50 p-3 dark:border-neutral-800/60 dark:bg-[#0d0d0d]/30">
        ...
        <p class="mt-1 text-xs text-neutral-400">{{ t('admin.channelMonitor.advanced.sectionHint') }}</p>
        ...
        <div class="mt-4 space-y-4">
          <div>
            ...
            />
            <p class="mt-1 text-xs text-neutral-400">{{ t('admin.channelMonitor.templateField.applyHint') }}</p>
          </div>
...
function apiModeButtonClass(mode: APIMode): string {
  ...
  if (active) {
    return 'border-neutral-600/60 bg-white text-neutral-200 shadow-sm dark:border-neutral-600/50 dark:bg-neutral-600/15 dark:text-neutral-300'
  }
  return 'border-[#81a1c1]/20 bg-white/70 text-gray-600 hover:border-neutral-600/50 dark:border-neutral-800/60 dark:bg-neutral-800 dark:text-neutral-400'
}
