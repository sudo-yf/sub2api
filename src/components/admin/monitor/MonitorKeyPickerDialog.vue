  ...
  >
    <div class="space-y-3">
      ...
      <div class="relative">
        ...
        />
        <svg class="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-neutral-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      ...
      <div v-else-if="filteredKeys.length === 0" class="py-6 text-center text-sm text-neutral-400">
      ...
      <div v-else class="max-h-96 overflow-auto rounded-lg border border-neutral-800/60">
        <table class="w-full text-sm">
          <thead class="bg-[#0d0d0d] sticky top-0 z-10">
            <tr class="text-left text-xs font-medium uppercase tracking-wider text-neutral-400">
          ...
          <tbody class="divide-y divide-neutral-800/60">
            <tr
              ...
              :key="k.id"
              class="cursor-pointer hover:bg-[#161616]"
              @click="$emit('pick', k)"
            >
              <td class="px-3 py-2 font-medium text-neutral-100">{{ k.name }}</td>
              <td class="px-3 py-2 font-mono text-xs text-neutral-400">{{ maskApiKey(k.key) }}</td>
              <td class="px-3 py-2">
                <GroupBadge
                  v-if="k.group"
                  :name="k.group.name"
