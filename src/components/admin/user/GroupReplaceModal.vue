    ...
    <div v-if="oldGroup" class="space-y-4">
      ...
      <div class="rounded-lg border border-gray-200 bg-gray-50 p-3 dark:border-neutral-800/60 dark:bg-neutral-800">
        <div class="flex items-center gap-2">
          <Icon name="shield" size="sm" class="text-[#b48ead]/80" />
          <span class="font-medium text-neutral-100 dark:text-white">{{ oldGroup.name }}</span>
          <Icon name="arrowRight" size="sm" class="ml-auto text-neutral-400" />
          <span v-if="selectedGroupId" class="font-medium text-neutral-300 dark:text-neutral-400">
      ...
      <div v-if="availableGroups.length > 0" class="max-h-64 space-y-2 overflow-y-auto">
        <label
          ...
          :class="selectedGroupId === group.id
            ? 'border-neutral-600/60 bg-neutral-900/40 dark:border-neutral-700/50 dark:bg-neutral-800/30'
            : 'border-gray-200 hover:border-gray-300 dark:border-neutral-800/60 dark:hover:border-neutral-600/60'"
        >
          <input
            type="radio"
            :value="group.id"
          ...
          <div
            ...
            :class="selectedGroupId === group.id
              ? 'border-[#81a1c1]/60 bg-[#81a1c1]'
              : 'border-gray-300 dark:border-neutral-700/60'"
          >
            <div v-if="selectedGroupId === group.id" class="h-2 w-2 rounded-full bg-white"></div>
          </div>
          <div class="flex-1">
            <span class="font-medium text-neutral-100 dark:text-white">{{ group.name }}</span>
            <span class="ml-2 text-xs text-neutral-400">{{ group.platform }}</span>
          </div>
        </label>
      </div>

      <!-- 无可选分组 -->
      <div v-else class="py-6 text-center text-sm text-neutral-400">
