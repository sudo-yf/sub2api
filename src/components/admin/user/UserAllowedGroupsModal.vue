<template>
  <BaseDialog :show="show" :title="t('admin.users.groupConfig')" width="wide" @close="$emit('close')">
    <div v-if="user" class="space-y-6">
      <!-- 用户信息头部 -->
      <div class="flex items-center gap-4 rounded-2xl bg-gradient-to-r from-neutral-900/30 to-neutral-800/30 p-5 dark:from-neutral-900/30 dark:to-neutral-800/20">
        <div class="flex h-14 w-14 items-center justify-center rounded-full bg-white shadow-sm dark:bg-neutral-900">
          <span class="text-2xl font-semibold text-neutral-300 dark:text-neutral-400">{{ user.email.charAt(0).toUpperCase() }}</span>
        </div>
        <div class="flex-1">
          <p class="text-lg font-semibold text-neutral-100 dark:text-white">{{ user.email }}</p>
          <p class="mt-1 text-sm text-gray-600 dark:text-neutral-400">{{ t('admin.users.groupConfigHint', { email: user.email }) }}</p>
        </div>
      </div>

      <!-- 加载状态 -->
      <div v-if="loading" class="flex justify-center py-12">
        <svg class="h-10 w-10 animate-spin text-neutral-500" fill="none" viewBox="0 0 24 24">
      ...
      <div v-else class="space-y-6">
        ...
        <div v-if="exclusiveGroups.length > 0">
          <div class="mb-3 flex items-center gap-2">
            <div class="h-1.5 w-1.5 rounded-full bg-[#b48ead]"></div>
            <h4 class="text-sm font-semibold text-neutral-600 dark:text-neutral-300">{{ t('admin.users.exclusiveGroups') }}</h4>
            <span class="text-xs text-neutral-400">({{ exclusiveGroupConfigs.filter(c => c.isSelected).length }}/{{ exclusiveGroupConfigs.length }})</span>
          </div>
          <div class="grid gap-3">
            <div
              ...
              :class="config.isSelected
                ? 'border-neutral-600/60 bg-neutral-900/40 shadow-sm dark:border-neutral-700/50 dark:bg-neutral-800/30'
                : 'border-gray-200 bg-white hover:border-gray-300 dark:border-neutral-800/60 dark:bg-neutral-800 dark:hover:border-neutral-600/60'"
            >
              <div class="flex items-center gap-4">
                ...
                <div class="flex-shrink-0">
                  <label class="relative flex h-6 w-6 cursor-pointer items-center justify-center">
                    <input
                      ...
                      class="peer sr-only"
                    />
                    <div class="h-5 w-5 rounded-md border-2 border-gray-300 transition-all peer-checked:border-neutral-600/60 peer-checked:bg-[#81a1c1] dark:border-neutral-700/60 peer-checked:dark:border-neutral-700/50">
                ...
                <div class="min-w-0 flex-1">
                  <div class="flex items-center gap-2">
                    <span class="text-base font-semibold text-neutral-100 dark:text-white">{{ config.groupName }}</span>
                    <span class="inline-flex items-center rounded-full bg-[#b48ead]/12 px-2 py-0.5 text-xs font-medium text-white dark:bg-[#b48ead]/12 dark:text-[#b48ead]">
                  ...
                  <div class="mt-1.5 flex items-center gap-3 text-sm">
                    ...
                    <span class="text-neutral-500 dark:text-neutral-400">
                      {{ t('admin.users.defaultRate') }}: <span class="font-medium text-neutral-600 dark:text-neutral-300">{{ config.defaultRate }}x</span>
                    </span>
                  </div>
                </div>
                ...
                <div class="flex flex-shrink-0 items-center gap-3">
                  <label class="text-sm font-medium text-gray-600 dark:text-neutral-400">{{ t('admin.users.customRate') }}</label>
                  <input
                    type="number"
                    step="0.001"
                    min="0.001"
                    ...
                    class="hide-spinner w-24 rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm font-medium transition-colors focus:border-neutral-500 focus:outline-none focus:ring-2 focus:ring-neutral-500/20 dark:border-neutral-700/60 dark:bg-neutral-900 dark:focus:border-neutral-500"
                  />
                </div>
              </div>
            </div>
        ...
        <div v-if="publicGroups.length > 0">
          <div class="mb-3 flex items-center gap-2">
            <div class="h-1.5 w-1.5 rounded-full bg-[#a3be8c]"></div>
            <h4 class="text-sm font-semibold text-neutral-600 dark:text-neutral-300">{{ t('admin.users.publicGroups') }}</h4>
            <span class="text-xs text-neutral-400">({{ publicGroupConfigs.length }})</span>
          </div>
          <div class="grid gap-3">
            <div
              v-for="config in publicGroupConfigs"
              :key="config.groupId"
              class="relative overflow-hidden rounded-xl border-2 border-[#a3be8c]/20 bg-[#a3be8c]/50 p-4 dark:border-[#a3be8c]/25 dark:bg-[#a3be8c]/12"
            >
              <div class="flex items-center gap-4">
                <!-- 复选框（禁用状态） -->
                <div class="flex-shrink-0">
                  <div class="flex h-5 w-5 items-center justify-center rounded-md border-2 border-[#a3be8c]/20 bg-[#a3be8c] dark:border-[#a3be8c]/25 dark:bg-[#a3be8c]/12">
        ...
        <div v-if="groups.length === 0" class="flex flex-col items-center justify-center py-12 text-center">
          <div class="mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-gray-100 dark:bg-neutral-900">
          ...
          <p class="text-neutral-500 dark:text-neutral-400">{{ t('common.noGroupsAvailable') }}</p>
        </div>
      </div>
    </div>
