<template>
  ...
  >
    <div class="space-y-4">
      ...
      <div
        v-if="showAddForm"
        class="rounded-xl border border-neutral-700/50 bg-neutral-900/40 p-4 dark:border-neutral-700/50 dark:bg-neutral-800/30"
      >
        <div class="mb-3 text-sm font-medium text-neutral-600 dark:text-neutral-300">
          {{ t('admin.scheduledTests.addPlan') }}
        </div>
        ...
        <div class="mt-3 flex justify-end gap-2">
          <button
            ...
            class="rounded-lg bg-gray-100 px-3 py-1.5 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-200 dark:bg-neutral-800 dark:text-neutral-300 dark:hover:bg-neutral-700"
          >
            {{ t('common.cancel') }}
          </button>
          <button
            ...
            class="flex items-center gap-1.5 rounded-lg bg-[#81a1c1] px-3 py-1.5 text-sm font-medium text-white transition-colors hover:bg-[#81a1c1]/90 disabled:cursor-not-allowed disabled:opacity-50"
          >
      ...
      <div v-if="loading" class="flex items-center justify-center py-8">
        <Icon name="refresh" size="md" class="animate-spin text-neutral-400" :stroke-width="2" />
        <span class="ml-2 text-sm text-neutral-500">{{ t('common.loading') }}...</span>
      </div>

      <!-- Empty State -->
      <div
        v-else-if="plans.length === 0"
        class="rounded-xl border border-dashed border-gray-300 py-10 text-center dark:border-neutral-800/60"
      >
        <Icon name="calendar" size="lg" class="mx-auto mb-2 text-neutral-400" :stroke-width="1.5" />
        <p class="text-sm text-neutral-500 dark:text-neutral-400">
      ...
      <div v-else class="space-y-3">
        <div
          v-for="plan in plans"
          :key="plan.id"
          class="rounded-xl border border-gray-200 bg-white transition-all dark:border-neutral-800/60 dark:bg-neutral-800"
        >
          <!-- Plan Header -->
          <div
          ...
          >
            <div class="flex flex-1 items-center gap-4">
              <!-- Model -->
              <div class="min-w-0">
                <div class="text-sm font-medium text-gray-900 dark:text-neutral-500">
                  {{ plan.model_id }}
                </div>
              ...
              <span
                ...
                class="inline-flex items-center rounded-full bg-[#a3be8c] px-2 py-0.5 text-xs font-medium text-neutral-800 dark:bg-[#0a1a0f]/20 dark:text-[#a3be8c]"
              >
                {{ t('admin.scheduledTests.autoRecover') }}
              </span>
            </div>
            ...
            <div class="flex items-center gap-3">
              <!-- Last Run -->
              <div v-if="plan.last_run_at" class="hidden text-right text-xs text-neutral-500 dark:text-neutral-400 sm:block">
                <div>{{ t('admin.scheduledTests.lastRun') }}</div>
                <div>{{ formatDateTime(plan.last_run_at) }}</div>
              </div>

              <!-- Next Run -->
              <div v-if="plan.next_run_at" class="hidden text-right text-xs text-neutral-500 dark:text-neutral-400 sm:block">
              ...
              <div class="flex items-center gap-1" @click.stop>
                <button
                  ...
                  class="rounded-lg p-1.5 text-neutral-400 transition-colors hover:bg-[#81a1c1] hover:text-blue-400/80 dark:hover:bg-[#81a1c1]/15"
                ...
                <button
                  ...
                  class="rounded-lg p-1.5 text-neutral-400 transition-colors hover:bg-[#bf616a] hover:text-red-400/80 dark:hover:bg-[#bf616a]/15"
          ...
          <div
            ...
            class="border-t border-[#81a1c1]/20 bg-[#090e1a]/50 px-4 py-3 dark:border-[#81a1c1]/25 dark:bg-[#81a1c1]/12"
            @click.stop
          >
            <div class="mb-2 text-xs font-medium text-gray-600 dark:text-neutral-400">
          ...
          <div
            ...
            class="border-t border-gray-100 px-4 py-3 dark:border-neutral-800/60"
          >
            <div class="mb-2 text-xs font-medium text-gray-600 dark:text-neutral-400">
            ...
            <div v-if="loadingResults" class="flex items-center justify-center py-4">
              <Icon name="refresh" size="sm" class="animate-spin text-neutral-400" :stroke-width="2" />
              <span class="ml-2 text-xs text-neutral-500">{{ t('common.loading') }}...</span>
            </div>
            ...
            <div v-else class="max-h-64 space-y-2 overflow-y-auto">
              <div
                ...
                :key="result.id"
                class="rounded-lg border border-gray-100 bg-gray-50 p-3 dark:border-neutral-800/60 dark:bg-[#0d0d0d]"
              >
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-2">
                    ...
                    <span
                      :class="[
                        ...
                        result.status === 'success'
                          ? 'bg-[#a3be8c] text-white dark:bg-[#0a1a0e]/20 dark:text-[#a3be8c]'
                          : result.status === 'running'
                            ? 'bg-[#81a1c1] text-white dark:bg-[#090e1a]/20 dark:text-[#81a1c1]'
                            : 'bg-[#bf616a] text-white dark:bg-[#1a0a0a]/20 dark:text-[#bf616a]'
                      ]"
                    >
                      {{
                        result.status === 'success'
                    ...
                    <!-- Latency -->
                    <span v-if="result.latency_ms > 0" class="text-xs text-neutral-500 dark:text-neutral-400">
                ...
                <div v-if="result.error_message" class="mt-2">
                  <div
                    class="cursor-pointer text-xs font-medium text-[#bf616a]/80 dark:text-[#bf616a]"
                  ...
                  <pre
                    ...
                    class="mt-1 max-h-32 overflow-auto whitespace-pre-wrap rounded bg-[#bf616a]/12 p-2 text-xs text-white dark:bg-[#bf616a]/12 dark:text-[#bf616a]"
                  >{{ result.error_message }}</pre>
                </div>
                <div v-else-if="result.response_text" class="mt-2">
                  <div
                    class="cursor-pointer text-xs font-medium text-gray-600 dark:text-neutral-400"
                    @click="toggleResultDetail(result.id)"
                  >
                  ...
                  <pre
                    v-if="expandedResultIds.has(result.id)"
                    class="mt-1 max-h-32 overflow-auto whitespace-pre-wrap rounded bg-gray-100 p-2 text-xs text-gray-700 dark:bg-neutral-800 dark:text-neutral-300"
