  ...
  <div class="relative flex min-h-screen flex-col bg-gray-50 dark:bg-dark-950">
    ...
    <header class="relative z-20 px-6 py-4">
      <nav class="mx-auto flex max-w-6xl items-center justify-between">
        <router-link to="/home" class="flex items-center gap-3">
          ...
          <span class="text-lg font-semibold tracking-tight text-neutral-100">{{ siteName }}</span>
        </router-link>
        <div class="flex items-center gap-3">
          ...
          <a
            ...
            class="rounded-lg p-2 text-neutral-400 transition-colors hover:bg-gray-100 hover:text-neutral-400 dark:hover:bg-neutral-800 dark:hover:text-white"
    ...
    <main class="flex-1 w-full max-w-5xl mx-auto px-6 py-12">
      ...
      <div class="text-center mb-12">
        <h1 class="text-3xl sm:text-4xl font-bold tracking-tight mb-3 text-neutral-100">
          {{ t('keyUsage.title') }}
        </h1>
        <p class="text-neutral-400 text-base max-w-md mx-auto">
          {{ t('keyUsage.subtitle') }}
        </p>
      </div>
      ...
      <div class="max-w-xl mx-auto mb-14">
        <div class="flex gap-3">
          <div class="flex-1 relative">
            ...
            <input
              ...
              class="input-ring w-full h-12 pl-12 pr-12 rounded-xl border border-gray-200 bg-white text-sm text-gray-900 placeholder:text-neutral-500 transition-all dark:border-neutral-800/60 dark:bg-[#0d0d0d] dark:text-white dark:placeholder:text-dark-500"
              @keydown.enter="queryKey"
            />
            <button
              @click="keyVisible = !keyVisible"
              class="absolute right-4 top-1/2 -translate-y-1/2 text-neutral-500 hover:text-neutral-400 dark:text-neutral-500 dark:hover:text-white transition-colors"
            >
          ...
          <button
            @click="queryKey"
            :disabled="isQuerying"
            class="h-12 px-7 rounded-xl bg-neutral-600 hover:bg-neutral-600 text-white font-medium text-sm transition-all active:scale-[0.97] flex items-center gap-2 whitespace-nowrap disabled:opacity-60"
          >
        ...
        <div v-if="showDatePicker" class="mt-4">
          <div class="flex flex-wrap items-center gap-2 justify-center">
            <span class="text-xs text-neutral-400">{{ t('keyUsage.dateRange') }}</span>
            <button
              ...
              :class="currentRange === range.key
                ? 'bg-neutral-600 text-white border-neutral-600/60'
                : 'border-gray-200 bg-white text-gray-700 dark:border-neutral-800/60 dark:bg-[#0d0d0d] dark:text-neutral-200 hover:border-neutral-600/50 dark:hover:border-dark-600'"
            >{{ range.label }}</button>
            <div v-if="currentRange === 'custom'" class="flex items-center gap-2 ml-1">
              <input
                v-model="customStartDate"
                type="date"
                class="input-ring text-xs px-2 py-1.5 rounded-lg border border-gray-200 bg-white text-gray-900 dark:border-neutral-800/60 dark:bg-[#0d0d0d] dark:text-white"
              />
              ...
              <button
                @click="queryKey"
                class="text-xs px-3 py-1.5 rounded-lg bg-neutral-600 text-white hover:bg-neutral-600"
      ...
      <div v-if="showResults">
        ...
        <div v-if="showLoading" class="space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="rounded-2xl border border-gray-200 bg-white p-8 dark:border-neutral-800/60 dark:bg-[#0d0d0d]">
        ...
        <div v-else-if="resultData" class="space-y-6">
          ...
          <div v-if="statusInfo" class="fade-up flex items-center justify-center mb-2">
            <div class="inline-flex items-center gap-2 px-5 py-2.5 rounded-full border border-gray-200 bg-white/90 shadow-sm backdrop-blur-sm dark:border-neutral-800/60 dark:bg-[#0d0d0d]/90">
              <span
                class="w-2.5 h-2.5 rounded-full pulse-dot"
                :class="statusInfo.isActive ? 'bg-[#a3be8c]' : 'bg-[#bf616a]'"
              ></span>
              <span class="text-sm font-medium text-neutral-100">{{ statusInfo.label }}</span>
              <span class="text-xs text-neutral-500">|</span>
              <span class="text-xs text-neutral-400">{{ statusInfo.statusText }}</span>
            </div>
          </div>
          ...
          <div v-if="ringItems.length > 0" :class="ringGridClass">
            <div
              ...
              :key="i"
              class="fade-up rounded-2xl border border-gray-200 bg-white/90 p-8 backdrop-blur-sm transition-all duration-300 hover:shadow-lg dark:border-neutral-800/60 dark:bg-[#0d0d0d]/90"
            ...
            >
              <div class="flex items-center justify-between mb-6">
                ...
                <!-- Clock icon -->
                <svg v-if="ring.iconType === 'clock'" class="w-5 h-5 text-neutral-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                  <circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>
                </svg>
                <!-- Calendar icon -->
                <svg v-else-if="ring.iconType === 'calendar'" class="w-5 h-5 text-neutral-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                  <rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/>
                </svg>
                <!-- Dollar icon -->
                <svg v-else class="w-5 h-5 text-neutral-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              ...
              <div class="flex justify-center">
                <div class="relative">
                  ...
                  <div class="absolute inset-0 flex flex-col items-center justify-center">
                    ...
                    <template v-else>
                      <span class="text-3xl font-bold tabular-nums text-neutral-100">
                        {{ displayPcts[i] ?? 0 }}%
                      </span>
                      <span class="text-xs text-neutral-400 mt-0.5">{{ t('keyUsage.used') }}</span>
                      <span
                        class="text-sm font-semibold mt-1 tabular-nums"
                        :style="{ color: RING_GRADIENTS[i % 4].from }"
                      >{{ ring.amount }}</span>
                      <p v-if="ring.resetAt && formatResetTime(ring.resetAt)" class="flex items-center gap-0.5 text-xs text-neutral-500 dark:text-neutral-400 mt-0.5 tabular-nums">
                        <Icon name="refresh" size="xs" class="opacity-50 shrink-0" />{{ formatResetTime(ring.resetAt) }}
                      </p>
                    </template>
                  </div>
                </div>
          ...
          <div
            ...
            class="fade-up fade-up-delay-3 rounded-2xl border border-gray-200 bg-white/90 backdrop-blur-sm overflow-hidden dark:border-neutral-800/60 dark:bg-[#0d0d0d]/90"
          >
            <div class="px-8 py-5 border-b border-neutral-800/60">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-neutral-400">{{ t('keyUsage.detailInfo') }}</h3>
            </div>
            <div class="divide-y divide-gray-100 dark:divide-dark-800">
              ...
              >
                <div class="flex items-center gap-3">
                  ...
                  <span class="text-sm text-neutral-200">{{ row.label }}</span>
                </div>
                <span class="text-sm font-semibold tabular-nums" :class="row.valueClass || 'text-neutral-100'">
                  {{ row.value }}
                </span>
              </div>
            </div>
          ...
          >
            <div class="px-8 py-5 border-b border-neutral-800/60">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-neutral-400">{{ t('keyUsage.tokenStats') }}</h3>
            </div>
            <div class="grid grid-cols-2 md:grid-cols-4 gap-px bg-neutral-800">
              <div
                v-for="(cell, i) in usageStatCells"
                :key="i"
                class="bg-white px-6 py-4 dark:bg-[#0d0d0d]"
              >
                <div class="text-xs text-neutral-400 mb-1">{{ cell.label }}</div>
                <div class="text-sm font-semibold tabular-nums text-neutral-100">{{ cell.value }}</div>
              </div>
            </div>
          </div>
          ...
          <div
            v-if="showDailyUsage"
            class="fade-up fade-up-delay-4 rounded-2xl border border-gray-200 bg-white/90 backdrop-blur-sm overflow-hidden dark:border-neutral-800/60 dark:bg-[#0d0d0d]/90"
          >
            <div class="flex flex-col gap-3 px-8 py-5 border-b border-neutral-800/60 sm:flex-row sm:items-center sm:justify-between">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-neutral-400">{{ t('keyUsage.dailyDetail') }}</h3>
              <div class="inline-flex rounded-lg border border-gray-200 bg-white p-0.5 dark:border-neutral-800/60 dark:bg-dark-950">
                <button
            ...
            <div v-if="dailyUsageRows.length > 0" class="overflow-x-auto">
              <table class="w-full">
                <thead>
                  <tr class="border-b border-neutral-800/60 bg-[#0d0d0d]">
                    <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-neutral-400">{{ t('keyUsage.date') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-neutral-400">{{ t('keyUsage.requests') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-neutral-400">{{ t('keyUsage.inputTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-neutral-400">{{ t('keyUsage.outputTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-neutral-400">{{ t('keyUsage.cacheReadTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-neutral-400">{{ t('keyUsage.cacheWriteTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-neutral-400">{{ t('keyUsage.cost') }}</th>
                  </tr>
                </thead>
                <tbody>
                  <tr
                    ...
                    :key="row.date"
                    class="border-b border-gray-100 last:border-b-0 dark:border-neutral-800/60"
                  >
                    <td class="px-4 py-3 text-sm font-medium whitespace-nowrap text-neutral-100">{{ row.date }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-neutral-200">{{ fmtNum(row.requests) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-neutral-200">{{ fmtNum(row.input_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-neutral-200">{{ fmtNum(row.output_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-neutral-200">{{ fmtNum(row.cache_read_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-neutral-200">{{ fmtNum(row.cache_write_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right font-medium text-neutral-100">{{ usd(row.actual_cost != null ? row.actual_cost : row.cost) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            <div v-else class="px-8 py-8 text-center text-sm text-neutral-400">
          ...
          >
            <div class="px-8 py-5 border-b border-neutral-800/60">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-neutral-400">{{ t('keyUsage.modelStats') }}</h3>
            </div>
            <div class="overflow-x-auto">
              <table class="w-full">
                <thead>
                  <tr class="border-b border-neutral-800/60 bg-[#0d0d0d]">
                    <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-neutral-400">{{ t('keyUsage.model') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-neutral-400">{{ t('keyUsage.requests') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-neutral-400">{{ t('keyUsage.inputTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-neutral-400">{{ t('keyUsage.outputTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-neutral-400">{{ t('keyUsage.cacheCreationTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-neutral-400">{{ t('keyUsage.cacheReadTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-neutral-400">{{ t('keyUsage.totalTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-neutral-400">{{ t('keyUsage.cost') }}</th>
                  </tr>
                </thead>
                <tbody>
                  <tr
                    ...
                    class="border-b border-gray-100 last:border-b-0 dark:border-neutral-800/60"
                  >
                    <td class="px-4 py-3 text-sm font-medium whitespace-nowrap text-neutral-100">{{ m.model || '-' }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-neutral-200">{{ fmtNum(m.requests) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-neutral-200">{{ fmtNum(m.input_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-neutral-200">{{ fmtNum(m.output_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-neutral-200">{{ fmtNum(m.cache_creation_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-neutral-200">{{ fmtNum(m.cache_read_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-neutral-200">{{ fmtNum(m.total_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right font-medium text-neutral-100">{{ usd(m.actual_cost != null ? m.actual_cost : m.cost) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
    ...
    <footer class="relative z-10 border-t border-gray-200/50 px-6 py-8 dark:border-neutral-800/60">
      <div class="mx-auto flex max-w-6xl flex-col items-center justify-center gap-4 text-center sm:flex-row sm:text-left">
        ...
        <div class="flex items-center gap-4">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            class="text-sm text-neutral-400 transition-colors hover:text-neutral-400 dark:hover:text-white"
          >{{ t('home.docs') }}</a>
          <a
            :href="githubUrl"
            target="_blank"
...
const siteName = computed(() => appStore.cachedPublicSettings?.site_name || appStore.siteName || 'CCF')
...
function getUsageColor(pct: number): string {
  if (pct > 90) return 'text-[#b48ead]/80'
  if (pct > 70) return 'text-[#d08770]/80'
  return 'text-[#88c0d0]/80'
}
...
const detailRows = computed<DetailRow[]>(() => {
  ...
  if (data.mode === 'quota_limited') {
    if (data.quota) {
      const remainColor = data.quota.remaining <= 0 ? 'text-[#b48ead]/80'
        : data.quota.remaining < data.quota.limit * 0.1 ? 'text-[#d08770]/80'
        : 'text-[#88c0d0]/80'
      rows.push({
        iconBg: 'bg-[#88c0d0]/10', iconColor: 'text-[#88c0d0]/80', iconSvg: ICON_SHIELD,
    ...
    if (data.expires_at) {
      ...
      rows.push({
        iconBg: 'bg-[#d08770]/10', iconColor: 'text-[#d08770]/80', iconSvg: ICON_CALENDAR,
    ...
    if (data.rate_limits) {
      ...
      for (const rl of data.rate_limits) {
        ...
        rows.push({
          iconBg: 'bg-neutral-600/10', iconColor: 'text-neutral-500', iconSvg: ICON_DOLLAR,
    ...
    rows.push({
      iconBg: 'bg-[#88c0d0]/10', iconColor: 'text-[#88c0d0]/80', iconSvg: ICON_CHECK,
    ...
    if (data.subscription) {
      ...
      if (sub.weekly_limit_usd > 0) {
        ...
        rows.push({
          iconBg: 'bg-[#5e81ac]/10', iconColor: 'text-[#5e81ac]/80', iconSvg: ICON_DOLLAR,
      ...
      if (sub.monthly_limit_usd > 0) {
        ...
        rows.push({
          iconBg: 'bg-[#88c0d0]/10', iconColor: 'text-[#88c0d0]/80', iconSvg: ICON_DOLLAR,
    ...
    const remainColor = data.remaining != null
      ? (data.remaining <= 0 ? 'text-[#b48ead]/80' : data.remaining < 10 ? 'text-[#d08770]/80' : 'text-[#88c0d0]/80')
      : ''
    rows.push({
      iconBg: 'bg-[#88c0d0]/10', iconColor: 'text-[#88c0d0]/80', iconSvg: ICON_SHIELD,
