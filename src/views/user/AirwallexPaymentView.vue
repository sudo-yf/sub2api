  ...
  <AppLayout>
    <div class="mx-auto max-w-lg space-y-6 py-8">
      <div v-if="loading" class="flex items-center justify-center py-20">
        <div class="h-8 w-8 animate-spin rounded-full border-4 border-[#88c0d0]/20 border-t-transparent"></div>
      </div>
      ...
      <div v-else-if="errorMessage" class="card p-8 text-center">
        ...
        <h3 class="text-lg font-semibold text-neutral-100 dark:text-white">{{ t('payment.airwallexLoadFailed') }}</h3>
        <p class="mt-2 text-sm text-neutral-500 dark:text-neutral-400">{{ errorMessage }}</p>
        <button class="btn btn-primary mt-6" @click="router.push('/purchase')">{{ t('payment.result.backToRecharge') }}</button>
      </div>

      <div v-else class="card p-6">
        <div class="flex flex-col items-center space-y-4 py-4">
          <div class="h-10 w-10 animate-spin rounded-full border-4 border-[#88c0d0]/20 border-t-transparent"></div>
