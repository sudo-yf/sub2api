<template>
  <div class="flex min-h-screen items-center justify-center bg-gray-50 px-4 dark:bg-[#0d0d0d]">
    <div class="w-full max-w-md space-y-6">
      ...
      <template v-else>
        ...
        <div class="text-center">
          <div v-if="isSuccess"
            class="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-[#a3be8c]/12 dark:bg-[#a3be8c]/12">
            <svg class="h-10 w-10 text-[#a3be8c]/80" fill="none" viewBox="0 0 24 24" stroke="currentColor"
              stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
            </svg>
          </div>
          <div v-else-if="isPending"
            class="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-[#ebcb8b]/12 dark:bg-[#ebcb8b]/12">
            <div class="h-10 w-10 animate-spin rounded-full border-4 border-[#ebcb8b]/20 border-t-transparent"></div>
          </div>
          <div v-else
            class="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-[#bf616a]/12 dark:bg-[#bf616a]/12">
            <svg class="h-10 w-10 text-[#bf616a]/80" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
            </svg>
          </div>
          <h2 class="mt-4 text-2xl font-bold text-neutral-100 dark:text-white">
            {{ statusTitle }}
          </h2>
          <p v-if="isPending" class="mt-2 text-sm text-neutral-500 dark:text-neutral-400">
            {{ t('payment.result.processingHint') }}
          </p>
        </div>
        <!-- Order Info -->
        <div v-if="order" class="rounded-xl bg-white p-5 shadow-sm dark:bg-neutral-800">
          <div class="space-y-3 text-sm">
            <div class="flex justify-between">
              <span class="text-neutral-500 dark:text-neutral-400">{{ t('payment.orders.orderId') }}</span>
              <span class="font-medium text-neutral-100 dark:text-white">#{{ order.id }}</span>
            </div>
            <div v-if="order.out_trade_no" class="flex justify-between">
              <span class="text-neutral-500 dark:text-neutral-400">{{ t('payment.orders.orderNo') }}</span>
              <span class="font-medium text-neutral-100 dark:text-white">{{ order.out_trade_no }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-neutral-500 dark:text-neutral-400">{{ t('payment.orders.baseAmount') }}</span>
              <span class="font-medium text-neutral-100 dark:text-white">{{ formatGatewayAmount(baseAmount) }}</span>
            </div>
            <div v-if="order.fee_rate > 0" class="flex justify-between">
              <span class="text-neutral-500 dark:text-neutral-400">{{ t('payment.orders.fee') }} ({{ order.fee_rate }}%)</span>
              <span class="font-medium text-neutral-100 dark:text-white">{{ formatGatewayAmount(feeAmount) }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-neutral-500 dark:text-neutral-400">{{ t('payment.orders.payAmount') }}</span>
              <span class="font-bold text-neutral-300 dark:text-neutral-400">{{ formatGatewayAmount(order.pay_amount) }}</span>
            </div>
            <div v-if="order.amount !== order.pay_amount" class="flex justify-between">
              <span class="text-neutral-500 dark:text-neutral-400">{{ t('payment.orders.creditedAmount') }}</span>
              <span class="font-medium text-neutral-100 dark:text-white">{{ order.order_type === 'balance' ? '$' + order.amount.toFixed(2) : formatGatewayAmount(order.amount) }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-neutral-500 dark:text-neutral-400">{{ t('payment.orders.paymentMethod') }}</span>
              <span class="font-medium text-neutral-100 dark:text-white">{{ t(paymentMethodI18nKey(order.payment_type), normalizedOrderPaymentType(order.payment_type)) }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-neutral-500 dark:text-neutral-400">{{ t('payment.orders.status') }}</span>
              <OrderStatusBadge :status="order.status" />
            </div>
          </div>
        </div>
        <!-- EasyPay return info (when no order loaded) -->
        <div v-else-if="returnInfo" class="rounded-xl bg-white p-5 shadow-sm dark:bg-neutral-800">
          <div class="space-y-3 text-sm">
            <div v-if="returnInfo.outTradeNo" class="flex justify-between">
              <span class="text-neutral-500 dark:text-neutral-400">{{ t('payment.orders.orderId') }}</span>
              <span class="font-medium text-neutral-100 dark:text-white">{{ returnInfo.outTradeNo }}</span>
            </div>
            <div v-if="returnInfo.money" class="flex justify-between">
              <span class="text-neutral-500 dark:text-neutral-400">{{ t('payment.orders.payAmount') }}</span>
              <span class="font-medium text-neutral-100 dark:text-white">{{ formatGatewayAmount(Number(returnInfo.money) || 0) }}</span>
            </div>
            <div v-if="returnInfo.type" class="flex justify-between">
              <span class="text-neutral-500 dark:text-neutral-400">{{ t('payment.orders.paymentMethod') }}</span>
              <span class="font-medium text-neutral-100 dark:text-white">{{ t(paymentMethodI18nKey(returnInfo.type), normalizedOrderPaymentType(returnInfo.type)) }}</span>
            </div>
          </div>
        </div>
        <!-- Actions -->
