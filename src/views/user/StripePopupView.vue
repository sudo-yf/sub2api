  ...
  <div class="flex min-h-screen items-center justify-center bg-slate-50 p-4 dark:bg-slate-950">
    ...
    >
      ...
      <div v-if="amount" class="text-center">
        ...
        <p v-if="orderId" class="mt-1 text-sm text-neutral-500 dark:text-slate-400">
      ...
      <div v-if="error" class="space-y-3">
        <div
          class="rounded-lg border border-[#bf616a]/20 bg-[#bf616a] p-3 text-sm text-white dark:border-[#bf616a]/25 dark:bg-[#bf616a]/12 dark:text-[#bf616a]"
        >
          {{ error }}
        </div>
        <button
          class="w-full text-sm underline dark:text-[#81a1c1] dark:hover:text-blue-400/70"
      ...
      <div v-else-if="success" class="space-y-3 py-4 text-center">
        <div class="flex justify-center text-[#a3be8c]/80 dark:text-[#a3be8c]">
          <Icon name="checkCircle" size="xl" />
        </div>
        <p class="text-sm text-neutral-500 dark:text-slate-400">{{ t('payment.result.success') }}</p>
        <button
          class="text-sm underline dark:text-[#81a1c1] dark:hover:text-blue-400/70"
          :style="{ color: methodColor }"
          @click="closeWindow"
        >
          {{ t('common.close') }}
      ...
      <div v-else class="flex items-center justify-center py-8">
        ...
        />
        <span class="ml-3 text-sm text-neutral-500 dark:text-slate-400">{{ hint }}</span>
      </div>
    </div>
  </div>
</template>
