<template>
  <component :is="isPopup ? 'div' : AppLayout" :class="isPopup ? 'min-h-screen bg-gray-50 dark:bg-[#0d0d0d]' : ''">
    <div class="mx-auto max-w-lg space-y-6 py-8" :class="isPopup ? 'px-4' : ''">
      ...
      <div v-else-if="initError" class="card p-8 text-center">
        <div class="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-[#bf616a]/12 dark:bg-[#bf616a]/12">
          <Icon name="exclamationCircle" size="xl" class="text-[#bf616a]/80" />
        </div>
        <h3 class="text-lg font-semibold text-neutral-100 dark:text-white">{{ t('payment.stripeLoadFailed') }}</h3>
        <p class="mt-2 text-sm text-neutral-500 dark:text-neutral-400">{{ initError }}</p>
      ...
      <template v-else>
        ...
        <div v-if="order" class="card overflow-hidden">
          <div class="bg-gradient-to-br from-[#635bff] to-[#4f46e5] px-6 py-6 text-center">
            <p class="text-sm font-medium text-[#5e81ac]/80">{{ t('payment.actualPay') }}</p>
        ...
        <template v-if="wechatQrUrl">
          <div class="card p-6">
            <div class="flex flex-col items-center space-y-4">
              <p class="text-lg font-semibold text-neutral-100 dark:text-white">{{ t('payment.qr.scanWxpay') }}</p>
              <div class="relative rounded-lg border-2 border-[#2BB741] bg-[#a3be8c] p-4 dark:border-[#2BB741]/70 dark:bg-[#a3be8c]/12">
              ...
              <p class="text-center text-sm text-neutral-500 dark:text-neutral-400">{{ t('payment.qr.scanWxpayHint') }}</p>
            </div>
          </div>
          <div class="card p-4 text-center">
            <p class="text-sm text-neutral-500 dark:text-neutral-400">{{ t('payment.qr.waitingPayment') }}</p>
          </div>
        </template>
        ...
        <template v-else-if="stripeSuccess">
          <div class="card p-6 text-center">
            <div class="flex flex-col items-center gap-3 py-4">
              ...
              <p class="text-lg font-bold text-neutral-100 dark:text-white">{{ t('payment.result.success') }}</p>
              <p class="text-sm text-neutral-500 dark:text-neutral-400">{{ t('payment.stripeSuccessProcessing') }}</p>
            </div>
          </div>
        </template>
        ...
        <template v-else-if="showPaymentElement">
          <div class="card p-6">
            ...
            <p v-if="stripeError" class="mt-4 text-sm text-[#bf616a]/80 dark:text-[#bf616a]">{{ stripeError }}</p>
        ...
        <div v-if="stripeError && !showPaymentElement" class="card p-4">
          <p class="text-sm text-[#bf616a]/80 dark:text-[#bf616a]">{{ stripeError }}</p>
