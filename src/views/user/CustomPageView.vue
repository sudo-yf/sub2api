    ...
    <div class="custom-page-layout">
      <div class="card flex-1 min-h-0 overflow-hidden">
        ...
        >
          <div class="max-w-md">
            <div
              class="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-neutral-900"
            >
              <Icon name="link" size="lg" class="text-neutral-400" />
            </div>
...
async function fetchAndRenderMarkdown(slug: string) {
  ...
  try {
    ...
    if (!resp.ok) {
      renderedHtml.value = '<p class="text-[#bf616a]/80">Page not found</p>'
      return
    }
    ...
    renderedHtml.value = '<p class="text-[#bf616a]/80">Failed to load page</p>'
  } finally {
    loading.value = false
    await nextTick()
    await nextTick()
...
.toc-sidebar {
  @apply flex flex-col h-full border-r border-neutral-800/60 bg-[#0d0d0d];
  width: min(240px, 30%);
  min-width: 160px;
  max-width: 280px;
  overflow: hidden;
...
.toc-header {
  @apply flex items-center justify-between px-4 py-3 border-b border-neutral-800/60;
}

.toc-title {
  @apply text-sm font-semibold text-neutral-200;
}

.toc-close-btn {
  @apply p-1 rounded text-gray-400 hover:text-gray-600 dark:hover:text-neutral-200 hover:bg-gray-200 dark:hover:bg-dark-600 transition-colors;
}
...
.toc-item {
  ...
  @apply text-neutral-300 hover:text-gray-900 dark:hover:text-white hover:bg-gray-200 dark:hover:bg-dark-600;
}

.toc-item.toc-active {
  @apply text-neutral-300 dark:text-neutral-400 bg-neutral-900/40 dark:bg-neutral-800/30 font-medium;
}
...
.toc-toggle-btn {
  ...
  @apply bg-[#0d0d0d] border border-gray-200 dark:border-neutral-700/60;
  @apply text-neutral-300 hover:bg-gray-100 dark:hover:bg-dark-600;
...
.custom-embed-shell {
  ...
  @apply bg-gradient-to-b from-gray-50 to-white dark:from-dark-900 dark:to-[#0a0a0a];
  @apply p-0;
}

.custom-open-fab {
  @apply absolute right-3 top-3 z-10;
  @apply shadow-sm backdrop-blur supports-[backdrop-filter]:bg-white/80 dark:supports-[backdrop-filter]:bg-neutral-800/80;
}
...
.markdown-page-content h1 { @apply text-3xl font-bold mt-8 mb-4 pb-2 border-b border-neutral-800/60; }
...
.markdown-page-content a { @apply text-neutral-500 hover:text-neutral-200 underline; }
.markdown-page-content blockquote { @apply border-l-4 border-gray-300 dark:border-neutral-700/60 pl-4 italic text-neutral-300 my-4; }
...
.markdown-page-content th { @apply border border-gray-300 dark:border-neutral-700/60 px-3 py-2 bg-[#0d0d0d] font-semibold text-left; }
.markdown-page-content td { @apply border border-gray-300 dark:border-neutral-700/60 px-3 py-2; }
.markdown-page-content code { @apply bg-neutral-900 px-1.5 py-0.5 rounded text-sm font-mono; }
.markdown-page-content pre { @apply bg-gray-900 dark:bg-[#0d0d0d] text-gray-100 p-4 rounded-lg overflow-x-auto my-4 relative; }
.markdown-page-content pre code { @apply bg-transparent p-0 text-inherit; }
.markdown-page-content hr { @apply my-6 border-neutral-800/60; }
