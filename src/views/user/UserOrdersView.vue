<template>
  <AppLayout>
      ...
      <OrderTable :orders="orders" :loading="loading">
        <template #actions="{ row }">
          <div class="flex items-center gap-2">
            <button v-if="row.status === 'PENDING'" @click="handleCancel(row.id)" class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-[#ebcb8b]/80 hover:bg-[#ebcb8b] dark:text-[#ebcb8b] dark:hover:bg-[#ebcb8b]/15">
              <Icon name="x" size="sm" />
              <span>{{ t('payment.orders.cancel') }}</span>
            </button>
            <button v-if="canRequestRefund(row)" @click="openRefundDialog(row)" class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-[#b48ead]/80 hover:bg-[#b48ead] dark:text-[#b48ead] dark:hover:bg-[#b48ead]/15">
    ...
    <BaseDialog :show="!!cancelTargetId" :title="t('payment.orders.cancel')" width="narrow" @close="cancelTargetId = null">
      <p class="text-sm text-gray-600 dark:text-neutral-300">{{ t('payment.confirmCancel') }}</p>
      <template #footer>
      ...
      <div v-if="refundTarget" class="space-y-4">
        <div class="rounded-xl bg-gray-50 p-4 dark:bg-neutral-800">
          <div class="flex justify-between text-sm">
            ...
            <span class="font-mono text-neutral-100 dark:text-white">#{{ refundTarget.id }}</span>
          </div>
          <div class="mt-2 flex justify-between text-sm">
            <span class="text-neutral-500 dark:text-neutral-400">{{ t('payment.orders.amount') }}</span>
            <span class="text-neutral-100 dark:text-white">${{ refundTarget.amount.toFixed(2) }}</span>
          </div>
        </div>
        <div>
