<template>
  <AppLayout>
    <div class="mx-auto flex max-w-md flex-col items-center space-y-6 py-8">
      ...
      <div v-if="qrUrl" class="rounded-2xl bg-white p-6 shadow-lg dark:bg-neutral-800">
        <canvas ref="qrCanvas" class="mx-auto"></canvas>
      </div>
      <!-- Scan prompt for QR code -->
      <p v-if="qrUrl && !expired && scanHint" class="text-center text-sm text-neutral-500 dark:text-neutral-400">
        {{ scanHint }}
      </p>
      ...
      <div v-else class="text-center">
        <p class="text-sm text-neutral-500 dark:text-neutral-400">{{ qrUrl ? t('payment.qr.expiresIn') : t('payment.qr.payInNewWindowHint') }}</p>
        <p class="mt-1 text-2xl font-bold tabular-nums text-neutral-100 dark:text-white">{{ countdownDisplay }}</p>
        <p class="mt-2 text-sm text-neutral-400 dark:text-neutral-500">{{ t('payment.qr.waitingPayment') }}</p>
      </div>
