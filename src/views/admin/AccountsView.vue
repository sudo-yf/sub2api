  ...
  <AppLayout>
    <TablePageLayout>
      <template #filters>
            ...
            <template #after>
              ...
              <div class="relative" ref="autoRefreshDropdownRef">
                ...
                <div
                  ...
                  class="absolute right-0 z-50 mt-2 w-56 origin-top-right rounded-lg border border-gray-200 bg-white shadow-lg dark:border-gray-700 dark:bg-[#1e1e1e]"
                >
                  <div class="p-2">
                    <button
                      @click="setAutoRefreshEnabled(!autoRefreshEnabled)"
                      class="flex w-full items-center justify-between rounded-md px-3 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-neutral-500 dark:hover:bg-[#252525]"
                    >
                      <span>{{ t('admin.accounts.enableAutoRefresh') }}</span>
                      <Icon v-if="autoRefreshEnabled" name="check" size="sm" class="text-neutral-500" />
                    </button>
                    ...
                    >
                      ...
                      <Icon v-if="autoRefreshIntervalSeconds === sec" name="check" size="sm" class="text-neutral-500" />
                    </button>
                  </div>
                </div>
              </div>
              ...
              <div class="relative" ref="accountToolsDropdownRef">
                ...
                <div
                  ...
                  class="absolute right-0 z-50 mt-2 w-[min(20rem,calc(100vw-2rem))] origin-top-right overflow-hidden rounded-lg border border-gray-200 bg-white shadow-xl dark:border-gray-700 dark:bg-[#1e1e1e]"
                >
                  <div class="max-h-[70vh] overflow-y-auto p-2">
                    <div class="px-2 py-2">
                      <div class="text-xs font-semibold uppercase tracking-wide text-neutral-400 dark:text-neutral-500">
                        {{ t('admin.accounts.dataActions') }}
                      </div>
                    </div>
                    <button class="account-tools-menu-item" @click="openSyncFromCrs">
                      <span class="account-tools-menu-icon bg-[#81a1c1] text-white dark:bg-[#81a1c1]/12 dark:text-[#81a1c1]">
                        <Icon name="sync" size="sm" />
                      </span>
                      <span class="flex-1 text-left">{{ t('admin.accounts.syncFromCrs') }}</span>
                    </button>
                    <button class="account-tools-menu-item" @click="openImportData">
                      <span class="account-tools-menu-icon bg-[#a3be8c] text-neutral-800 dark:bg-[#a3be8c]/15 dark:text-[#a3be8c]">
                        <Icon name="upload" size="sm" />
                      </span>
                      <span class="flex-1 text-left">{{ t('admin.accounts.dataImport') }}</span>
                    </button>
                    <button class="account-tools-menu-item" @click="openExportDataDialogFromMenu">
                      <span class="account-tools-menu-icon bg-[#b48ead] text-white dark:bg-[#b48ead]/12 dark:text-[#b48ead]">
                      ...
                      <span
                        v-if="selIds.length"
                        class="rounded-full bg-neutral-800 px-2 py-0.5 text-xs font-medium text-neutral-200 dark:bg-neutral-800/40 dark:text-neutral-300"
                      >
                    ...
                    <button class="account-tools-menu-item" @click="openErrorPassthrough">
                      <span class="account-tools-menu-icon bg-[#ebcb8b] text-neutral-800 dark:bg-[#ebcb8b]/15 dark:text-[#ebcb8b]">
                    ...
                    <div class="px-2 py-2">
                      <div class="flex items-center justify-between gap-3">
                        ...
                        <Icon name="grid" size="sm" class="text-neutral-400" />
                      </div>
                    </div>
                    <div class="grid grid-cols-1 gap-1">
                      <button
                        ...
                        class="flex w-full items-center justify-between rounded-md px-3 py-2 text-sm text-gray-700 transition-colors hover:bg-gray-100 dark:text-neutral-500 dark:hover:bg-[#252525]"
                      >
                        <span class="truncate">{{ col.label }}</span>
                        <Icon v-if="isColumnVisible(col.key)" name="check" size="sm" class="text-neutral-500" />
                      </button>
                    </div>
                  </div>
                </div>
        ...
        <div
          ...
          class="mt-2 flex items-center justify-between rounded-lg border border-[#d08770]/20 bg-[#ebcb8b] px-3 py-2 text-sm text-[#d08770] dark:border-[#d08770]/25 dark:bg-[#ebcb8b]/15 dark:text-[#ebcb8b]"
        >
      ...
      <template #table>
        ...
        >
          <template #header-select>
            <input
              type="checkbox"
              class="h-4 w-4 cursor-pointer rounded border-gray-300 text-neutral-300 focus:ring-neutral-500"
          ...
          <template #cell-select="{ row }">
            <input type="checkbox" :checked="isSelected(row.id)" @change="toggleSel(row.id)" class="rounded border-gray-300 text-neutral-300 focus:ring-neutral-500" />
          </template>
          <template #cell-name="{ row, value }">
            <div class="flex flex-col">
              ...
              <span
                ...
                class="text-xs text-neutral-400 truncate max-w-[200px]"
          ...
          <template #cell-notes="{ value }">
            <span v-if="value" :title="value" class="block max-w-xs truncate text-sm text-neutral-300">{{ value }}</span>
          ...
          <template #cell-schedulable="{ row }">
            <button @click="handleToggleSchedulable(row)" :disabled="togglingSchedulable === row.id" class="relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-neutral-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 dark:focus:ring-offset-dark-800" :class="[row.schedulable ? 'bg-[#a3be8c] hover:bg-[#a3be8c]' : 'bg-gray-200 hover:bg-gray-300 dark:bg-neutral-800 dark:hover:bg-neutral-700']" :title="row.schedulable ? t('admin.accounts.schedulableEnabled') : t('admin.accounts.schedulableDisabled')">
          ...
          <template #cell-proxy="{ row }">
            <div v-if="row.proxy" class="flex items-center gap-2">
              <span class="text-sm text-neutral-300">{{ row.proxy.name }}</span>
              <span v-if="row.proxy.country_code" class="text-xs text-neutral-400">
          ...
          <template #cell-last_used_at="{ value }">
            <span class="text-sm text-neutral-400">{{ formatRelativeTime(value) }}</span>
          </template>
          <template #cell-expires_at="{ row, value }">
            <div class="flex flex-col items-start gap-1">
              <span class="text-sm text-neutral-400">{{ formatExpiresAt(value) }}</span>
              <div v-if="isExpired(value) || (row.auto_pause_on_expired && value)" class="flex items-center gap-1">
                <span
                  v-if="isExpired(value)"
                  class="inline-flex items-center rounded-md bg-[#ebcb8b] px-2 py-0.5 text-xs font-medium text-[#d08770] dark:bg-[#ebcb8b]/15 dark:text-[#ebcb8b]"
                >
                  {{ t('admin.accounts.expired') }}
                </span>
                <span
                  v-if="row.auto_pause_on_expired && value"
                  class="inline-flex items-center rounded-md bg-[#a3be8c] px-2 py-0.5 text-xs font-medium text-neutral-800 dark:bg-[#a3be8c]/15 dark:text-[#a3be8c]"
                >
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              <button @click="handleEdit(row)" class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-neutral-500 transition-colors hover:bg-gray-100 hover:text-neutral-200 dark:hover:bg-neutral-900 dark:hover:text-neutral-300">
                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" /></svg>
                <span class="text-xs">{{ t('common.edit') }}</span>
              </button>
              <button @click="handleDelete(row)" class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-neutral-500 transition-colors hover:bg-[#bf616a] hover:text-red-400/70 dark:hover:bg-[#bf616a]/15 dark:hover:text-red-400">
                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" /></svg>
                <span class="text-xs">{{ t('common.delete') }}</span>
              </button>
              <button @click="openMenu(row, $event)" class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-neutral-500 transition-colors hover:bg-gray-100 hover:text-gray-900 dark:hover:bg-neutral-900 dark:hover:text-white">
    ...
    <ConfirmDialog :show="showExportDataDialog" :title="t('admin.accounts.dataExport')" :message="t('admin.accounts.dataExportConfirmMessage')" :confirm-text="t('admin.accounts.dataExportConfirm')" :cancel-text="t('common.cancel')" @confirm="handleExportData" @cancel="showExportDataDialog = false">
      <label class="flex items-center gap-2 text-sm text-neutral-300">
        <input type="checkbox" class="h-4 w-4 rounded border-gray-300 text-neutral-300 focus:ring-neutral-500" v-model="includeProxyOnExport" />
  ...
  switch (state) {
    case 'active':
      return {
        ...
        className: 'text-[#88c0d0]/80 dark:text-[#88c0d0]/80',
        dotClass: 'bg-[#a3be8c] shadow-[0_0_0_2px_rgba(16,185,129,0.14)]'
      }
    case 'blocked':
      return {
        label: t('admin.accounts.openai.compactUnsupported'),
        className: 'text-[#b48ead]/80 dark:text-[#b48ead]',
        dotClass: 'bg-[#bf616a] shadow-[0_0_0_2px_rgba(244,63,94,0.14)]'
      }
    case 'auto':
      return {
...
function getAntigravityTierClass(row: any): string {
  const tier = getAntigravityTierFromRow(row)
  switch (tier) {
    case 'free-tier': return 'bg-gray-100 text-gray-600 dark:bg-[#252525] dark:text-neutral-300'
    case 'g1-pro-tier': return 'bg-[#81a1c1] text-white dark:bg-[#81a1c1]/12 dark:text-[#81a1c1]'
    case 'g1-ultra-tier': return 'bg-[#b48ead] text-white dark:bg-[#b48ead]/12 dark:text-[#b48ead]'
    default: return ''
  }
}
