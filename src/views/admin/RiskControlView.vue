  ...
  <AppLayout>
    <div class="space-y-6">
      <div v-if="loading" class="flex items-center justify-center py-16">
        <div class="h-8 w-8 animate-spin rounded-full border-b-2 border-neutral-600"></div>
      </div>

      <template v-else>
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h1 class="text-2xl font-semibold text-neutral-100">{{ t('admin.riskControl.title') }}</h1>
            <p class="mt-1 text-sm text-neutral-400">{{ t('admin.riskControl.description') }}</p>
          </div>
        ...
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
          <div
            ...
            :key="item.key"
            class="rounded-lg border border-gray-100 bg-white px-4 py-3 shadow-sm dark:border-neutral-800/60 dark:bg-neutral-800"
          >
            <div class="flex min-w-0 items-center gap-3">
              ...
              <div class="min-w-0 flex-1">
                <div class="flex min-w-0 items-center justify-between gap-2">
                  <p class="truncate text-xs font-medium text-neutral-400">{{ item.label }}</p>
                  <span
                    v-if="item.badge"
                ...
                <div class="mt-1 flex min-w-0 items-baseline gap-2">
                  <p class="truncate text-xl font-semibold leading-7 text-neutral-100">{{ item.value }}</p>
                  <p v-if="item.meta" class="truncate text-xs text-neutral-400">{{ item.meta }}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="card">
          <div class="flex flex-col gap-4 border-b border-neutral-800/60 px-6 py-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <h2 class="text-lg font-semibold text-neutral-100">{{ t('admin.riskControl.workerStatus') }}</h2>
              <p class="mt-1 text-sm text-neutral-400">{{ t('admin.riskControl.workerStatusHint') }}</p>
            </div>
          ...
          <div class="grid grid-cols-1 gap-6 p-6 xl:grid-cols-[minmax(0,360px)_1fr]">
            <div class="space-y-4">
              <div class="rounded-lg border border-gray-100 p-4 dark:border-neutral-800/60">
                <div class="flex items-center justify-between gap-3">
                  <div>
                    <p class="text-sm font-medium text-neutral-100">{{ t('admin.riskControl.queueUsage') }}</p>
                    <p class="mt-1 text-xs text-neutral-400">
                      {{ formatNumber(status?.queue_length ?? 0) }} / {{ formatNumber(status?.queue_size ?? configForm.queue_size) }}
                    </p>
                  </div>
                  <span class="text-sm font-semibold text-neutral-100">{{ queueUsagePercent }}</span>
                </div>
                <div class="mt-4 h-2 overflow-hidden rounded-full bg-neutral-900">
                  <div class="h-full rounded-full bg-[#81a1c1] transition-all duration-300" :style="queueUsageStyle"></div>
                </div>
              </div>

              <div class="grid grid-cols-2 gap-3">
                <div class="rounded-lg bg-gray-50 p-4 dark:bg-neutral-900/50">
                  <p class="text-xs text-neutral-400">{{ t('admin.riskControl.activeWorkers') }}</p>
                  <p class="mt-2 text-2xl font-semibold text-neutral-100">{{ status?.active_workers ?? 0 }}</p>
                </div>
                <div class="rounded-lg bg-[#a3be8c]/12 p-4 dark:bg-[#a3be8c]/15">
                  <p class="text-xs text-neutral-400">{{ t('admin.riskControl.idleWorkers') }}</p>
                  <p class="mt-2 text-2xl font-semibold text-[#88c0d0]/80 dark:text-[#88c0d0]/80">{{ status?.idle_workers ?? configForm.worker_count }}</p>
                </div>
                <div class="rounded-lg bg-gray-50 p-4 dark:bg-neutral-900/50">
                  <p class="text-xs text-neutral-400">{{ t('admin.riskControl.processed') }}</p>
                  <p class="mt-2 text-2xl font-semibold text-neutral-100">{{ formatNumber(status?.processed ?? 0) }}</p>
                </div>
                <div class="rounded-lg bg-gray-50 p-4 dark:bg-neutral-900/50">
                  <p class="text-xs text-neutral-400">{{ t('admin.riskControl.droppedErrors') }}</p>
                  <p class="mt-2 text-2xl font-semibold text-neutral-100">{{ formatNumber((status?.dropped ?? 0) + (status?.errors ?? 0)) }}</p>
                </div>
              </div>
            </div>
            ...
            <div>
              <div class="mb-3 flex items-center justify-between gap-3">
                <div>
                  <p class="text-sm font-medium text-neutral-100">{{ t('admin.riskControl.workerPool') }}</p>
                  <p class="mt-1 text-xs text-neutral-400">
                ...
                <span class="inline-flex items-center rounded-full bg-gray-100 px-2.5 py-1 text-xs font-medium text-gray-600 dark:bg-neutral-900 dark:text-neutral-300">
        ...
        <div class="card">
          <div class="flex flex-col gap-4 border-b border-neutral-800/60 px-6 py-4">
            <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h2 class="text-lg font-semibold text-neutral-100">{{ t('admin.riskControl.records') }}</h2>
                <p class="mt-1 text-sm text-neutral-400">{{ t('admin.riskControl.recordsHint') }}</p>
              </div>
            ...
            <div class="flex flex-col gap-2 rounded-lg border border-gray-100 bg-gray-50 px-3 py-2 dark:border-neutral-800/60 dark:bg-[#0d0d0d]/30 sm:flex-row sm:items-center sm:justify-between">
              <div class="flex min-w-0 items-center gap-2 text-sm text-gray-700 dark:text-neutral-500">
                <Icon name="filter" size="sm" class="flex-shrink-0 text-neutral-500" />
                <span class="font-medium">{{ t('admin.riskControl.modelFilter') }}</span>
                <span class="truncate text-neutral-400">{{ modelFilterSummary }}</span>
              </div>
              <div v-if="modelFilterPreviewModels.length > 0" class="flex flex-wrap gap-1.5">
                <span
                  v-for="model in modelFilterPreviewModels"
                  :key="model"
                  class="inline-flex max-w-[180px] items-center truncate rounded-md bg-white px-2 py-1 font-mono text-xs text-gray-600 shadow-sm dark:bg-neutral-800 dark:text-neutral-300"
                >
                  {{ model }}
                </span>
                <span v-if="hiddenModelFilterModelCount > 0" class="inline-flex rounded-md bg-white px-2 py-1 text-xs text-neutral-400 shadow-sm dark:bg-neutral-800 dark:text-neutral-400">
          ...
          <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-neutral-800/60">
              <thead class="bg-[#0d0d0d]">
                <tr>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-neutral-400">{{ t('admin.riskControl.table.time') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-neutral-400">{{ t('admin.riskControl.table.group') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-neutral-400">{{ t('admin.riskControl.table.user') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-neutral-400">{{ t('admin.riskControl.table.apiKey') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-neutral-400">{{ t('admin.riskControl.table.endpoint') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-neutral-400">{{ t('admin.riskControl.table.result') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-neutral-400">{{ t('admin.riskControl.table.highest') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-neutral-400">{{ t('admin.riskControl.table.actionMeta') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-neutral-400">{{ t('admin.riskControl.table.latency') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-neutral-400">{{ t('admin.riskControl.table.input') }}</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-neutral-800/60 bg-[#0d0d0d]">
                <tr v-if="logsLoading">
                  <td colspan="10" class="px-5 py-12 text-center text-sm text-neutral-400">{{ t('common.loading') }}</td>
                </tr>
                <tr v-else-if="logs.length === 0">
                  <td colspan="10" class="px-5 py-12 text-center text-sm text-neutral-400">{{ t('admin.riskControl.emptyLogs') }}</td>
                </tr>
                <template v-else>
                  <tr v-for="row in logs" :key="row.id" class="hover:bg-[#161616]/60">
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-neutral-300">{{ formatDateTime(row.created_at) }}</td>
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-neutral-300">{{ row.group_name || '-' }}</td>
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-neutral-300">
                      <div>{{ row.user_email || '-' }}</div>
                      <div v-if="row.user_id" class="text-xs text-neutral-500">UID {{ row.user_id }}</div>
                    </td>
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-neutral-300">{{ row.api_key_name || '-' }}</td>
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-neutral-300">
                      <div>{{ row.endpoint || '-' }}</div>
                      <div class="text-xs text-neutral-500">{{ row.provider || '-' }} / {{ row.model || '-' }}</div>
                    </td>
                    ...
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-neutral-300">
                      ...
                      <div class="text-xs text-neutral-500">{{ percent(row.highest_score) }}</div>
                    </td>
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-neutral-300">
                      ...
                      <button
                        ...
                        type="button"
                        class="mt-2 inline-flex items-center gap-1 rounded-md border border-[#88c0d0]/20 bg-[#a3be8c] px-2 py-1 text-xs font-medium text-neutral-800 transition-colors hover:bg-[#a3be8c] disabled:cursor-not-allowed disabled:opacity-60 dark:border-[#88c0d0]/25 dark:bg-[#a3be8c]/15 dark:text-[#a3be8c] dark:hover:bg-[#88c0d0]/15"
                    ...
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-neutral-300">
                      ...
                      <div v-if="row.queue_delay_ms !== null && row.queue_delay_ms !== undefined" class="text-xs text-neutral-500">
                        {{ t('admin.riskControl.queueDelay', { ms: row.queue_delay_ms }) }}
                      </div>
                    </td>
                    <td class="w-[320px] max-w-sm px-5 py-4 text-sm text-neutral-300">
                      <button
                        type="button"
                        class="group flex w-full min-w-0 items-center gap-2 rounded-lg px-2 py-1.5 text-left transition-colors hover:bg-[#161616]"
                        :title="inputSummaryText(row)"
                        @click="openInputDetail(row)"
                      >
                        <span class="min-w-0 flex-1 truncate">{{ inputSummaryText(row) }}</span>
                        <Icon name="eye" size="xs" class="flex-shrink-0 text-neutral-400 transition-colors group-hover:text-neutral-500 dark:text-neutral-400" />
                      </button>
                    </td>
                  </tr>
                </template>
      ...
      <BaseDialog :show="settingsOpen" :title="t('admin.riskControl.settingsTitle')" width="extra-wide" @close="settingsOpen = false">
        <div class="space-y-6">
          <div class="flex gap-2 overflow-x-auto border-b border-gray-100 pb-3 dark:border-neutral-800/60">
            <button
              v-for="tab in settingsTabs"
              :key="tab.id"
              type="button"
              class="inline-flex whitespace-nowrap rounded-lg px-3 py-2 text-sm font-medium transition-colors"
              :class="activeSettingsTab === tab.id ? 'bg-neutral-900/40 text-neutral-200 dark:bg-neutral-800/40 dark:text-neutral-300' : 'text-neutral-400 hover:bg-gray-50 hover:text-gray-900 dark:text-neutral-400 dark:hover:bg-neutral-900 dark:hover:text-white'"
          ...
          <div v-if="activeSettingsTab === 'basic'" class="space-y-5">
            <div class="grid grid-cols-1 gap-5 lg:grid-cols-2">
              <div class="flex items-center justify-between rounded-lg border border-gray-100 p-4 dark:border-neutral-800/60">
                <div>
                  <p class="text-sm font-medium text-neutral-100">{{ t('admin.riskControl.enabled') }}</p>
                  <p class="mt-1 text-xs text-neutral-400">{{ t('admin.riskControl.enabledHint') }}</p>
                </div>
              ...
              <div>
                ...
                <p class="mt-2 text-xs leading-5 text-neutral-400">{{ modeDescription(configForm.mode) }}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <div class="relative">
                  ...
                  <span class="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-neutral-500">%</span>
                </div>
              </div>
            </div>

            <div class="overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm dark:border-neutral-800/60 dark:bg-neutral-800">
              <div class="flex flex-col gap-4 border-b border-gray-100 bg-gray-50 px-4 py-4 dark:border-neutral-800/60 dark:bg-neutral-800/60 lg:flex-row lg:items-center lg:justify-between">
                <div class="flex items-start gap-3">
                  <span class="mt-0.5 flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg bg-neutral-900/40 text-neutral-300 dark:bg-neutral-800/40 dark:text-neutral-300">
                    <Icon name="key" size="md" />
                  </span>
                  <div>
                    <label class="text-sm font-semibold text-neutral-100">{{ t('admin.riskControl.apiKeys') }}</label>
                    <p class="mt-1 max-w-3xl text-xs leading-5 text-neutral-400">
              ...
              <div class="grid grid-cols-1 gap-4 p-4 xl:grid-cols-[minmax(0,1fr)_minmax(360px,440px)]">
                <div class="space-y-3">
                  <div class="flex flex-col gap-2 rounded-lg border border-gray-100 bg-gray-50 p-2 dark:border-neutral-800/60 dark:bg-[#0d0d0d]/30 sm:flex-row sm:items-center sm:justify-between">
                    <div class="text-xs leading-5 text-neutral-400">
                      <span class="font-medium text-gray-700 dark:text-neutral-500">{{ t('admin.riskControl.apiKeysWriteMode') }}</span>
                      <span class="ml-2">{{ apiKeysModeHint }}</span>
                    </div>
                    <div class="inline-flex rounded-lg bg-white p-1 shadow-sm dark:bg-neutral-800">
                      <button
                        type="button"
                        class="rounded-md px-3 py-1.5 text-xs font-medium transition-colors"
                        :class="configForm.api_keys_mode === 'append' ? 'bg-[#81a1c1] text-white shadow-sm' : 'text-gray-600 hover:bg-gray-100 dark:text-neutral-300 dark:hover:bg-neutral-900'"
                      ...
                      <button
                        type="button"
                        class="rounded-md px-3 py-1.5 text-xs font-medium transition-colors"
                        :class="configForm.api_keys_mode === 'replace' ? 'bg-[#ebcb8b] text-white shadow-sm' : 'text-gray-600 hover:bg-gray-100 dark:text-neutral-300 dark:hover:bg-neutral-900'"
                        :disabled="configForm.clear_api_key"
                  ...
                  <div class="flex flex-wrap items-center gap-2 text-xs text-neutral-400">
                    <span class="inline-flex rounded-md bg-gray-100 px-2 py-1 dark:bg-neutral-900">
                      {{ t('admin.riskControl.inputApiKeyCount', { count: inputApiKeyCount }) }}
                    </span>
                    <span v-if="configForm.api_key_configured" class="inline-flex rounded-md bg-gray-100 px-2 py-1 dark:bg-neutral-900">
                      {{ t('admin.riskControl.storedApiKeyCount', { count: configForm.api_key_count }) }}
                    </span>
                    <span v-if="configForm.clear_api_key" class="inline-flex rounded-md bg-[#bf616a]/12 px-2 py-1 text-white dark:bg-[#bf616a]/12 dark:text-[#bf616a]">
                      {{ t('admin.riskControl.apiKeyWillClear') }}
                    </span>
                    <span v-else-if="pendingDeletedApiKeyCount > 0" class="inline-flex rounded-md bg-[#ebcb8b] px-2 py-1 text-[#d08770] dark:bg-[#ebcb8b]/15 dark:text-[#ebcb8b]">
                      {{ t('admin.riskControl.apiKeyPendingDeleteCount', { count: pendingDeletedApiKeyCount }) }}
                    </span>
                    <span v-if="configForm.api_keys_mode === 'replace'" class="inline-flex rounded-md bg-[#ebcb8b] px-2 py-1 text-[#d08770] dark:bg-[#ebcb8b]/15 dark:text-[#ebcb8b]">
                      {{ t('admin.riskControl.apiKeysReplaceWarning') }}
                    </span>
                  </div>

                  <div class="rounded-lg border border-gray-100 bg-gray-50 p-3 dark:border-neutral-800/60 dark:bg-[#0d0d0d]/30" @paste="handleModerationImagePaste">
                    <div class="mb-3 flex items-center justify-between gap-3">
                      <div>
                        <p class="text-sm font-semibold text-neutral-100">{{ t('admin.riskControl.auditTestInput') }}</p>
                        <p class="mt-1 text-xs text-neutral-400">{{ t('admin.riskControl.auditTestInputHint') }}</p>
                      </div>
                      <button
                        v-if="moderationTestPrompt || moderationTestImages.length > 0 || moderationTestResult"
                        type="button"
                        class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-neutral-400 hover:bg-white hover:text-gray-900 dark:text-neutral-400 dark:hover:bg-neutral-800 dark:hover:text-white"
                    ...
                    ></textarea>
                    <div
                      class="mt-3 rounded-lg border border-dashed border-gray-200 bg-white p-3 dark:border-neutral-800/60 dark:bg-neutral-800"
                      @dragover.prevent
                      @drop.prevent="handleModerationImageDrop"
                    >
                      <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        <div class="flex items-start gap-2">
                          <Icon name="upload" size="md" class="mt-0.5 text-neutral-500" />
                          <div>
                            <p class="text-sm font-medium text-neutral-300 dark:text-neutral-500">{{ t('admin.riskControl.auditTestImages') }}</p>
                            <p class="mt-1 text-xs text-neutral-400">{{ t('admin.riskControl.auditTestImagesHint') }}</p>
                          </div>
                        </div>
                      ...
                      <div v-if="moderationTestImages.length > 0" class="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
                        <div
                          ...
                          class="group relative aspect-square overflow-hidden rounded-lg border border-gray-100 bg-gray-100 dark:border-neutral-800/60 dark:bg-neutral-900"
                        >
                ...
                <div class="rounded-lg border border-gray-100 bg-gray-50 p-3 dark:border-neutral-800/60 dark:bg-[#0d0d0d]/30">
                  <div class="mb-3 flex items-start justify-between gap-3">
                    <div class="min-w-0">
                      <p class="text-sm font-semibold text-neutral-100">{{ t('admin.riskControl.apiKeyHealth') }}</p>
                      <p class="mt-1 text-xs text-neutral-400">{{ t('admin.riskControl.apiKeyFreezeRule') }}</p>
                    </div>
                    <span class="inline-flex shrink-0 items-center whitespace-nowrap rounded-full bg-white px-2 py-0.5 text-[11px] font-medium leading-5 text-gray-600 shadow-sm dark:bg-neutral-800 dark:text-neutral-300">
                      {{ t('admin.riskControl.apiKeyRows', { count: apiKeyRows.length }) }}
                    </span>
                  </div>

                  <div v-if="apiKeyRows.length === 0" class="flex min-h-32 flex-col items-center justify-center rounded-lg border border-dashed border-gray-200 bg-white px-4 py-6 text-center dark:border-neutral-800/60 dark:bg-neutral-800">
                    <Icon name="infoCircle" size="lg" class="text-neutral-500" />
                    <p class="mt-2 text-sm font-medium text-gray-700 dark:text-neutral-500">{{ t('admin.riskControl.apiKeyHealthEmpty') }}</p>
                    <p class="mt-1 text-xs text-neutral-400">{{ t('admin.riskControl.apiKeyHealthEmptyHint') }}</p>
                  </div>
                  <div v-else class="space-y-2">
                    <div class="space-y-2" :class="apiKeyRowsExpanded ? 'max-h-72 overflow-y-auto pr-1' : ''">
                      <div
                        ...
                        class="rounded-lg border bg-white p-2.5 shadow-sm dark:bg-neutral-800"
                        :class="isStoredApiKeyPendingDelete(row) ? 'border-[#d08770]/20 opacity-70 dark:border-[#d08770]/25' : 'border-neutral-800/60'"
                      >
                        <div class="flex items-start justify-between gap-2">
                          <div class="min-w-0">
                            <div class="flex min-w-0 flex-wrap items-center gap-2">
                              <span class="truncate font-mono text-sm font-semibold text-neutral-100">{{ row.masked || '-' }}</span>
                              <span
                                class="inline-flex rounded-md px-1.5 py-0.5 text-[11px] font-medium"
                                :class="row.configured ? 'bg-neutral-900/40 text-neutral-200 dark:bg-neutral-800/40 dark:text-neutral-300' : 'bg-[#b48ead] text-white dark:bg-[#b48ead]/12 dark:text-[#b48ead]'"
                              >
                            ...
                            <p class="mt-1 text-xs leading-5 text-neutral-400">{{ apiKeyStatusMeta(row) }}</p>
                          </div>
                          <div class="flex flex-shrink-0 items-center gap-1.5">
                            ...
                            <button
                              ...
                              type="button"
                              class="inline-flex h-7 w-7 items-center justify-center rounded-md text-neutral-500 transition-colors hover:bg-gray-100 hover:text-neutral-400 dark:hover:bg-neutral-900 dark:hover:text-neutral-500"
                        ...
                        <p v-if="row.last_error" class="mt-1.5 rounded-md bg-[#ebcb8b] px-2 py-1.5 text-xs leading-5 text-[#d08770] dark:bg-[#ebcb8b]/15 dark:text-[#ebcb8b]">
                          {{ row.last_error }}
                        </p>
                      </div>
                    </div>

                    <div v-if="canToggleApiKeyRows" class="flex items-center justify-between gap-3 rounded-lg border border-dashed border-gray-200 bg-white px-3 py-2 text-xs text-neutral-400 dark:border-neutral-800/60 dark:bg-neutral-800 dark:text-neutral-400">
                      ...
                      <button
                        type="button"
                        class="inline-flex shrink-0 items-center gap-1 rounded-md px-2 py-1 font-medium text-neutral-300 transition-colors hover:bg-neutral-900/40 hover:text-neutral-200 dark:text-neutral-300 dark:hover:bg-neutral-800/30"
                  ...
                  <div v-if="moderationTestResult" class="mt-4 rounded-lg border border-gray-100 bg-white p-3 dark:border-neutral-800/60 dark:bg-neutral-800">
                    <div class="flex items-start justify-between gap-3">
                      <div>
                        <p class="text-sm font-semibold text-neutral-100">{{ t('admin.riskControl.auditTestResult') }}</p>
                        <p class="mt-1 text-xs text-neutral-400">
                      ...
                      <span class="inline-flex rounded-full px-2 py-1 text-xs font-medium" :class="moderationTestResult.flagged ? 'bg-[#bf616a] text-white dark:bg-[#bf616a]/12 dark:text-[#bf616a]' : 'bg-[#a3be8c] text-neutral-800 dark:bg-[#a3be8c]/15 dark:text-[#a3be8c]'">
                    ...
                    <div class="mt-3">
                      <div class="mb-2 flex items-center justify-between text-xs text-neutral-400">
                        ...
                        <span class="font-semibold text-neutral-100">{{ percent(moderationTestResult.composite_score) }}</span>
                      </div>
                      <div class="h-2 overflow-hidden rounded-full bg-neutral-900">
                        <div class="h-full rounded-full" :class="moderationTestResult.flagged ? 'bg-[#bf616a]' : 'bg-[#a3be8c]'" :style="{ width: percentWidth(moderationTestResult.composite_score) }"></div>
                      </div>
                    </div>
                    <div class="mt-3 max-h-52 space-y-2 overflow-y-auto pr-1">
                      <div v-for="score in moderationScoreRows" :key="score.category">
                        <div class="mb-1 flex items-center justify-between gap-3 text-xs">
                          <span class="truncate text-neutral-300">{{ score.category }}</span>
                          <span class="font-mono text-neutral-400">{{ percent(score.score) }} / {{ percent(score.threshold) }}</span>
                        </div>
                        <div class="h-1.5 overflow-hidden rounded-full bg-neutral-900">
                          <div class="h-full rounded-full" :class="score.hit ? 'bg-[#bf616a]' : 'bg-[#81a1c1]'" :style="{ width: percentWidth(score.score) }"></div>
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          <div v-else-if="activeSettingsTab === 'scope'" class="space-y-5">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h3 class="text-base font-semibold text-neutral-100">{{ t('admin.riskControl.groupScope') }}</h3>
                <p class="mt-1 text-sm text-neutral-400">{{ t('admin.riskControl.groupScopeHint') }}</p>
              </div>
              <div class="inline-flex rounded-lg bg-gray-100 p-1 dark:bg-neutral-900">
                <button
                  type="button"
                  class="rounded-md px-3 py-1.5 text-sm font-medium transition-colors"
                  :class="configForm.all_groups ? 'bg-white text-gray-900 shadow-sm dark:bg-neutral-800 dark:text-white' : 'text-neutral-400'"
            ...
            <div v-if="!configForm.all_groups" class="space-y-4">
              <div class="relative">
                <Icon name="search" size="sm" class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-neutral-500" />
              ...
              <div class="grid max-h-[420px] grid-cols-1 gap-3 overflow-y-auto pr-1 md:grid-cols-2 xl:grid-cols-3">
                <button
                  ...
                  :class="isGroupSelected(group.id) ? 'border-neutral-600/60 bg-neutral-900/40 dark:border-neutral-700/60 dark:bg-neutral-800/30' : 'border-gray-100 hover:bg-gray-50 dark:border-neutral-800/60 dark:hover:bg-neutral-900/60'"
                  @click="toggleGroup(group.id)"
                >
                  <span class="min-w-0">
                    <span class="block truncate text-sm font-semibold text-neutral-100">{{ group.name }}</span>
                    <span class="mt-1 inline-flex rounded-md bg-gray-100 px-2 py-0.5 text-xs text-neutral-400 dark:bg-neutral-900 dark:text-neutral-400">{{ group.platform }}</span>
                  </span>
                  <span
                    class="flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full border"
                    :class="isGroupSelected(group.id) ? 'border-[#81a1c1]/60 bg-[#81a1c1] text-white' : 'border-gray-300 text-transparent dark:border-neutral-700/60'"
                  >
                    <Icon name="check" size="xs" :stroke-width="2" />
                  </span>
                </button>
                <p v-if="filteredGroups.length === 0" class="text-sm text-neutral-400">{{ t('admin.riskControl.noGroups') }}</p>
              </div>
            </div>

            <div class="space-y-4 rounded-lg border border-gray-100 p-4 dark:border-neutral-800/60">
              <div class="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
                <div>
                  <h3 class="text-base font-semibold text-neutral-100">{{ t('admin.riskControl.modelFilter') }}</h3>
                  <p class="mt-1 text-sm text-neutral-400">{{ t('admin.riskControl.modelFilterHint') }}</p>
                </div>
                <span class="inline-flex w-fit rounded-md bg-gray-100 px-2.5 py-1 text-xs font-medium text-gray-600 dark:bg-neutral-900 dark:text-neutral-300">
                  {{ modelFilterSummary }}
                </span>
              </div>
              ...
              <div class="grid grid-cols-1 gap-2 md:grid-cols-3">
                <button
                  ...
                  :class="configForm.model_filter_type === option.value
                    ? 'border-neutral-600/60 bg-neutral-900/40 text-neutral-100 shadow-sm dark:border-neutral-700/60 dark:bg-neutral-800/30 dark:text-neutral-100'
                    : 'border-gray-100 hover:bg-gray-50 dark:border-neutral-800/60 dark:hover:bg-neutral-900/60'"
                ...
                >
                  <div class="flex items-center justify-between gap-2">
                    ...
                    <span
                      ...
                      :class="configForm.model_filter_type === option.value
                        ? 'border-[#81a1c1]/60 bg-[#81a1c1] text-white'
                        : 'border-gray-300 text-transparent dark:border-neutral-700/60'"
                    >
                      <Icon name="check" size="xs" :stroke-width="2" />
                    </span>
                  </div>
                  <p class="mt-1 text-xs leading-5 text-neutral-400">{{ option.description }}</p>
                </button>
              </div>
          ...
          <div v-else-if="activeSettingsTab === 'runtime'" class="grid grid-cols-1 gap-5 lg:grid-cols-2">
            ...
            <div class="flex items-center justify-between rounded-lg border border-gray-100 p-4 dark:border-neutral-800/60 lg:col-span-2">
              <div>
                <p class="text-sm font-medium text-neutral-100">{{ t('admin.riskControl.recordNonHits') }}</p>
                <p class="mt-1 text-xs text-neutral-400">{{ t('admin.riskControl.recordNonHitsHint') }}</p>
              </div>
              <Toggle v-model="configForm.record_non_hits" />
            </div>
            <div class="space-y-4 rounded-lg border border-gray-100 p-4 dark:border-neutral-800/60 lg:col-span-2">
              <div class="flex items-center justify-between gap-4">
                <div>
                  <p class="text-sm font-medium text-neutral-100">{{ t('admin.riskControl.preHashCheck') }}</p>
                  <p class="mt-1 text-xs text-neutral-400">{{ t('admin.riskControl.preHashCheckHint') }}</p>
                </div>
                <Toggle v-model="configForm.pre_hash_check_enabled" />
              </div>
              <div class="rounded-lg bg-gray-50 p-3 dark:bg-[#0d0d0d]/30">
                <div class="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
                  <div>
                    ...
                    <p class="mt-1 text-xs text-neutral-400">{{ t('admin.riskControl.flaggedHashHint') }}</p>
                  </div>
                  <button
                    type="button"
                    class="btn btn-secondary inline-flex items-center justify-center gap-2 text-[#bf616a]/80 hover:text-red-400/80 dark:text-[#bf616a]"
          ...
          <div v-else-if="activeSettingsTab === 'response'" class="space-y-5">
            <div class="grid grid-cols-1 gap-5 lg:grid-cols-2">
              ...
              <div class="flex items-center justify-between rounded-lg border border-gray-100 p-4 dark:border-neutral-800/60">
                <div>
                  <p class="text-sm font-medium text-neutral-100">{{ t('admin.riskControl.emailOnHit') }}</p>
                  <p class="mt-1 text-xs text-neutral-400">{{ t('admin.riskControl.emailOnHitHint') }}</p>
                </div>
                <Toggle v-model="configForm.email_on_hit" />
              </div>
              <div class="flex items-center justify-between rounded-lg border border-gray-100 p-4 dark:border-neutral-800/60">
                <div>
                  <p class="text-sm font-medium text-neutral-100">{{ t('admin.riskControl.autoBan') }}</p>
                  <p class="mt-1 text-xs text-neutral-400">{{ t('admin.riskControl.autoBanHint') }}</p>
                </div>
          ...
          <div v-else-if="activeSettingsTab === 'keywords'" class="space-y-5">
            ...
            >
              ...
              <div class="text-sm leading-6">
                ...
                <p class="mt-1 text-xs text-neutral-400">{{ keywordNotice.description }}</p>
              </div>
            </div>
            ...
            <div>
              <div class="mb-2 flex items-center justify-between">
                ...
                <span class="inline-flex rounded-md bg-gray-100 px-2 py-1 text-xs text-neutral-400 dark:bg-neutral-900 dark:text-neutral-300">
          ...
          <div v-else class="grid grid-cols-1 gap-5 lg:grid-cols-2">
            ...
            <div class="rounded-lg border border-gray-100 p-4 text-sm text-neutral-400 dark:border-neutral-800/60 dark:text-neutral-400 lg:col-span-2">
              <div class="flex flex-wrap items-center gap-3">
                <Icon name="database" size="md" class="text-neutral-500" />
      ...
      >
        <div v-if="inputDetailRow" class="space-y-5">
          <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <div class="rounded-lg border border-gray-100 bg-gray-50 p-4 dark:border-neutral-800/60 dark:bg-neutral-800/70">
              <p class="text-xs font-medium text-neutral-400">{{ t('admin.riskControl.table.time') }}</p>
              <p class="mt-1 truncate text-sm font-semibold text-neutral-100">{{ formatDateTime(inputDetailRow.created_at) }}</p>
            </div>
            <div class="rounded-lg border border-gray-100 bg-gray-50 p-4 dark:border-neutral-800/60 dark:bg-neutral-800/70">
              <p class="text-xs font-medium text-neutral-400">{{ t('admin.riskControl.table.user') }}</p>
              <p class="mt-1 truncate text-sm font-semibold text-neutral-100">{{ inputDetailRow.user_email || '-' }}</p>
            </div>
            <div class="rounded-lg border border-gray-100 bg-gray-50 p-4 dark:border-neutral-800/60 dark:bg-neutral-800/70">
              <p class="text-xs font-medium text-neutral-400">{{ t('admin.riskControl.table.result') }}</p>
            ...
            <div class="rounded-lg border border-gray-100 bg-gray-50 p-4 dark:border-neutral-800/60 dark:bg-neutral-800/70">
              <p class="text-xs font-medium text-neutral-400">{{ t('admin.riskControl.table.highest') }}</p>
              <p class="mt-1 truncate text-sm font-semibold text-neutral-100">
                {{ inputDetailRow.highest_category || '-' }} / {{ percent(inputDetailRow.highest_score) }}
              </p>
            </div>
          </div>

          <div class="rounded-xl border border-gray-100 bg-white p-4 shadow-sm dark:border-neutral-800/60 dark:bg-neutral-800">
            <div class="flex flex-wrap items-center justify-between gap-3">
              <div>
                <p class="text-sm font-semibold text-neutral-100">{{ t('admin.riskControl.inputDetailContent') }}</p>
                <p class="mt-1 text-xs text-neutral-400">
                  {{ inputDetailRow.endpoint || '-' }} · {{ inputDetailRow.provider || '-' }} / {{ inputDetailRow.model || '-' }}
                </p>
              </div>
              <span v-if="inputDetailRow.group_name" class="inline-flex rounded-md bg-sky-50 px-2.5 py-1 text-xs font-medium text-[#81a1c1]/80 dark:bg-[#81a1c1]/12 dark:text-[#81a1c1]">
                {{ inputDetailRow.group_name }}
              </span>
            </div>
            <pre class="mt-4 max-h-[420px] overflow-auto whitespace-pre-wrap break-words rounded-lg bg-gray-950 p-4 text-sm leading-6 text-neutral-500 shadow-inner dark:bg-black/50">{{ inputDetailText }}</pre>
          </div>
        </div>
...
const keywordNoticeTones = {
  info: {
    ...
    toneClass: 'border-neutral-800/60 bg-neutral-900/40 dark:border-neutral-800/60 dark:bg-neutral-900/10',
    iconClass: 'mt-0.5 flex-shrink-0 text-neutral-500 dark:text-neutral-300',
    titleClass: 'text-neutral-200 dark:text-neutral-200',
  },
  warning: {
    icon: 'exclamationTriangle' as const,
    toneClass: 'border-[#d08770]/20 bg-[#ebcb8b] dark:border-[#d08770]/25 dark:bg-[#ebcb8b]/15',
    iconClass: 'mt-0.5 flex-shrink-0 text-[#d08770]/80 dark:text-[#d08770]/70',
    titleClass: 'text-[#d08770]/80 dark:text-[#d08770]/60',
  },
}
...
const overviewItems = computed<OverviewItem[]>(() => [
  {
    ...
    iconClass: configForm.enabled
      ? 'bg-[#a3be8c] text-neutral-800 dark:bg-[#a3be8c]/15 dark:text-[#a3be8c]'
      : 'bg-gray-100 text-neutral-400 dark:bg-neutral-900 dark:text-neutral-400',
  ...
  },
  {
    ...
    icon: 'key',
    iconClass: 'bg-sky-50 text-[#81a1c1]/80 dark:bg-[#81a1c1]/12 dark:text-[#81a1c1]',
  },
  {
    key: 'scope',
    ...
    icon: 'users',
    iconClass: 'bg-[#b48ead] text-white dark:bg-[#b48ead]/12 dark:text-[#b48ead]',
  },
  {
    key: 'logs',
    ...
    icon: 'document',
    iconClass: 'bg-[#ebcb8b] text-neutral-800 dark:bg-[#ebcb8b]/15 dark:text-[#ebcb8b]',
  },
])
...
const runtimeBadgeClass = computed(() => {
  ...
  return 'bg-[#a3be8c] text-neutral-800 dark:bg-[#a3be8c]/15 dark:text-[#a3be8c]'
})
...
function resultBadgeClass(row: ContentModerationLog): string {
  if (row.action === 'block' || row.action === 'keyword_block') return 'bg-[#bf616a] text-white dark:bg-[#bf616a]/12 dark:text-[#bf616a]'
  if (row.action === 'error' || row.error) return 'bg-[#ebcb8b] text-neutral-800 dark:bg-[#ebcb8b]/15 dark:text-[#ebcb8b]'
  if (row.flagged) return 'bg-[#b48ead]/8 text-[#b48ead]/80 dark:bg-[#b48ead]/12 dark:text-[#b48ead]'
  return 'bg-[#a3be8c] text-white dark:bg-[#a3be8c]/12 dark:text-[#a3be8c]'
}

function workerSlotClass(state: WorkerSlotState): string {
  if (state === 'active') {
    return 'border-[#81a1c1]/20 bg-sky-50 text-[#81a1c1]/80 dark:border-[#81a1c1]/25 dark:bg-[#81a1c1]/12 dark:text-[#81a1c1]'
  }
  if (state === 'idle') {
    return 'border-[#88c0d0]/20 bg-[#a3be8c] text-neutral-800 dark:border-[#88c0d0]/25 dark:bg-[#a3be8c]/15 dark:text-[#a3be8c]'
  }
  return 'border-gray-100 bg-white text-neutral-500 dark:border-neutral-800/60 dark:bg-neutral-800 dark:text-neutral-400'
}

function workerDotClass(state: WorkerSlotState): string {
  if (state === 'active') return 'bg-sky-500'
  if (state === 'idle') return 'bg-[#a3be8c]'
...
function apiKeyStatusBadgeClass(statusValue: ContentModerationAPIKeyStatus['status']): string {
  const classes: Record<ContentModerationAPIKeyStatus['status'], string> = {
    ok: 'bg-[#a3be8c] text-neutral-800 dark:bg-[#a3be8c]/15 dark:text-[#a3be8c]',
    error: 'bg-[#ebcb8b] text-neutral-800 dark:bg-[#ebcb8b]/15 dark:text-[#ebcb8b]',
    frozen: 'bg-[#bf616a] text-white dark:bg-[#bf616a]/12 dark:text-[#bf616a]',
    unknown: 'bg-gray-100 text-gray-600 dark:bg-neutral-900 dark:text-neutral-300',
  }
