  ...
  <AppLayout>
      ...
      <template #filters>
        <div class="flex flex-wrap items-center gap-3">
          <div class="relative w-full md:w-80">
            <Icon name="search" size="md" class="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-400" />
      ...
      <template #table>
        ...
        >
          ...
          <template #cell-aff_code="{ row }">
            <span class="font-mono text-sm text-neutral-300">{{ row.aff_code || '-' }}</span>
          </template>
          <template #cell-order="{ row }">
            <div class="space-y-0.5">
              <div class="font-mono text-sm text-neutral-100">#{{ row.order_id }}</div>
              <div class="max-w-56 truncate text-sm text-neutral-400">{{ row.out_trade_no }}</div>
            </div>
          </template>
          ...
          <template #cell-pay_amount="{ row }">
            <span class="text-sm text-neutral-100">¥{{ formatAmount(row.pay_amount) }}</span>
          </template>
          ...
          <template #cell-created_at="{ row }">
            <span class="text-sm text-neutral-300">{{ formatDateTime(row.created_at) }}</span>
          </template>
        </DataTable>
      </template>
    ...
    >
      <div v-if="overviewLoading" class="flex justify-center py-8">
        <div class="h-6 w-6 animate-spin rounded-full border-2 border-neutral-600/60 border-t-transparent"></div>
      </div>
      <div v-else-if="selectedOverview" class="space-y-4">
        <div class="rounded-lg border border-gray-100 bg-gray-50 p-4 dark:border-neutral-800/60 dark:bg-neutral-800">
          <div class="font-mono text-sm text-neutral-100">#{{ selectedOverview.user_id }}</div>
          <div class="mt-1 text-sm font-medium text-neutral-100">{{ selectedOverview.email || '-' }}</div>
          <div class="mt-0.5 text-sm text-neutral-400">{{ selectedOverview.username || '-' }}</div>
        </div>
...
const UserCell = defineComponent({
  ...
  setup(cellProps, { emit }) {
    return () => h('div', { class: 'space-y-0.5' }, [
      h('div', { class: 'font-mono text-sm text-neutral-100' }, `#${cellProps.id}`),
      h(cellProps.clickable ? 'button' : 'div', {
        class: cellProps.clickable
          ? 'max-w-56 truncate text-left text-sm font-medium text-neutral-300 hover:text-neutral-200 hover:underline dark:text-neutral-400 dark:hover:text-neutral-300'
          : 'max-w-56 truncate text-sm text-neutral-300',
        type: cellProps.clickable ? 'button' : undefined,
        onClick: cellProps.clickable ? () => emit('open', cellProps.id) : undefined,
      }, cellProps.email || '-'),
      h('div', { class: 'max-w-56 truncate text-sm text-neutral-400' }, cellProps.username || '-'),
    ])
  },
})
  ...
  setup(amountProps) {
    return () => h('span', {
      class: amountProps.strong
        ? 'text-sm font-semibold text-[#88c0d0]/80 dark:text-[#88c0d0]/80'
  ...
  setup(amountProps) {
    return () => {
      ...
      if (value === null || value === undefined) {
        return h('span', { class: 'text-sm text-neutral-500' }, '-')
      }
...
const OverviewStat = defineComponent({
  ...
  setup(statProps) {
    return () => h('div', { class: 'rounded-lg border border-gray-100 bg-white p-3 dark:border-neutral-800/60 dark:bg-[#0d0d0d]' }, [
      h('div', { class: 'text-sm text-neutral-400' }, statProps.label),
      h('div', {
        class: statProps.mono
          ? 'mt-1 font-mono text-base font-semibold text-neutral-100'
          : 'mt-1 text-base font-semibold text-neutral-100',
      }, statProps.value),
    ])
  },
})
