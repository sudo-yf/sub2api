  ...
  <BaseDialog :show="show" :title="plan ? t('payment.admin.editPlan') : t('payment.admin.createPlan')" width="wide" @close="emit('close')">
    <form id="plan-form" @submit.prevent="handleSavePlan" class="space-y-4">
      <div class="grid grid-cols-2 gap-4">
        <div>
          <label class="input-label">{{ t('payment.admin.planName') }} <span class="text-[#bf616a]/80">*</span></label>
          <input v-model="planForm.name" type="text" class="input" required />
        </div>
        <div>
          <label class="input-label">{{ t('payment.admin.group') }} <span class="text-[#bf616a]/80">*</span></label>
          <Select v-model="planForm.group_id" :options="groupOptions" :placeholder="t('payment.admin.selectGroup')" class="w-full">
            ...
            <template #option="{ option, selected }">
              ...
              <Icon v-if="selected" name="check" size="sm" class="text-neutral-500" :stroke-width="2" />
            </template>
          </Select>
        </div>
      </div>
      ...
      <!-- Group Info Preview -->
      <div v-if="selectedGroupInfo" class="rounded-lg border border-gray-200 bg-gray-50 p-3 dark:border-neutral-800/60 dark:bg-neutral-800">
        ...
        <div class="grid grid-cols-2 gap-2 text-xs">
          <div><span class="text-neutral-500">{{ t('payment.admin.dailyLimit') }}:</span> <span class="ml-1 font-medium text-neutral-600 dark:text-neutral-300">{{ selectedGroupInfo.daily_limit_usd != null ? '$' + selectedGroupInfo.daily_limit_usd : t('payment.admin.unlimited') }}</span></div>
          <div><span class="text-neutral-500">{{ t('payment.admin.weeklyLimit') }}:</span> <span class="ml-1 font-medium text-neutral-600 dark:text-neutral-300">{{ selectedGroupInfo.weekly_limit_usd != null ? '$' + selectedGroupInfo.weekly_limit_usd : t('payment.admin.unlimited') }}</span></div>
          <div><span class="text-neutral-500">{{ t('payment.admin.monthlyLimit') }}:</span> <span class="ml-1 font-medium text-neutral-600 dark:text-neutral-300">{{ selectedGroupInfo.monthly_limit_usd != null ? '$' + selectedGroupInfo.monthly_limit_usd : t('payment.admin.unlimited') }}</span></div>
        </div>
      </div>

      <div><label class="input-label">{{ t('payment.admin.planDescription') }} <span class="text-[#bf616a]/80">*</span></label><textarea v-model="planForm.description" rows="2" class="input" required></textarea></div>
      <div class="grid grid-cols-2 gap-4">
        <div><label class="input-label">{{ t('payment.admin.price') }} <span class="text-[#bf616a]/80">*</span></label><input v-model.number="planForm.price" type="number" step="0.01" min="0.01" class="input" required /></div>
        <div><label class="input-label">{{ t('payment.admin.originalPrice') }}</label><input v-model.number="planForm.original_price" type="number" step="0.01" min="0" class="input" /></div>
      </div>
      <div class="grid grid-cols-2 gap-4">
        <div><label class="input-label">{{ t('payment.admin.validityDays') }} <span class="text-[#bf616a]/80">*</span></label><input v-model.number="planForm.validity_days" type="number" min="1" class="input" required /></div>
        <div><label class="input-label">{{ t('payment.admin.validityUnit') }} <span class="text-[#bf616a]/80">*</span></label><Select v-model="planForm.validity_unit" :options="validityUnitOptions" /></div>
      </div>
      <div class="grid grid-cols-2 gap-4">
      ...
      <div>
        ...
        <p class="mt-1 text-xs text-neutral-500 dark:text-neutral-400">{{ t('payment.admin.featuresHint') }}</p>
      </div>
      <div class="flex items-center gap-3">
        <label class="text-sm text-neutral-600 dark:text-neutral-300">{{ t('payment.admin.forSale') }}</label>
        <button
          type="button"
          :class="[
            ...
            planForm.for_sale ? 'bg-neutral-600' : 'bg-gray-300 dark:bg-neutral-800'
          ]"
