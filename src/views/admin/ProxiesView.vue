<template>
  <AppLayout>
        ...
        >
          ...
          <template #cell-address="{ row }">
            <div class="flex items-center gap-1.5">
              ...
              <div class="relative">
                <button
                  type="button"
                  class="rounded p-0.5 text-neutral-500 hover:text-neutral-200 dark:hover:text-neutral-300"
                ...
                <div
                  ...
                  class="absolute left-0 top-full z-50 mt-1 w-auto min-w-[180px] rounded-lg border border-gray-200 bg-white py-1 shadow-lg dark:border-neutral-700/60 dark:bg-neutral-900"
                >
                  <button
                  ...
                  >
                    <span class="truncate font-mono text-neutral-300">{{ fmt.label }}</span>
                  </button>
                </div>
              </div>
            </div>
          ...
          <template #cell-auth="{ row }">
            <div v-if="row.username || row.password" class="flex items-center gap-1.5">
              <div class="flex flex-col text-xs">
                <span v-if="row.username" class="text-gray-700 dark:text-neutral-500">{{ row.username }}</span>
                <span v-if="row.password" class="font-mono text-neutral-400">
              ...
              <button
                v-if="row.password"
                type="button"
                class="ml-1 rounded p-0.5 text-neutral-500 hover:text-gray-600 dark:hover:text-neutral-400"
          ...
          <template #cell-location="{ row }">
            <div class="flex items-center gap-2">
              ...
              />
              <span v-if="formatLocation(row)" class="text-sm text-gray-700 dark:text-neutral-500">
                {{ formatLocation(row) }}
              </span>
              <span v-else class="text-sm text-neutral-500">-</span>
            </div>
          ...
          <template #cell-account_count="{ row, value }">
            <button
              v-if="(value || 0) > 0"
              type="button"
              class="inline-flex items-center rounded bg-gray-100 px-2 py-0.5 text-xs font-medium text-neutral-200 hover:bg-gray-200 dark:bg-neutral-800 dark:text-neutral-300 dark:hover:bg-neutral-700"
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-neutral-400 transition-colors hover:bg-[#a3be8c] hover:text-emerald-400/70 disabled:cursor-not-allowed disabled:opacity-50 dark:hover:bg-[#88c0d0]/15 dark:hover:text-emerald-400/80"
              >
                <svg
              ...
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-neutral-400 transition-colors hover:bg-[#81a1c1] hover:text-blue-400/70 disabled:cursor-not-allowed disabled:opacity-50 dark:hover:bg-[#81a1c1]/15 dark:hover:text-blue-400"
              >
                <svg
              ...
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-neutral-400 transition-colors hover:bg-gray-100 hover:text-neutral-200 dark:hover:bg-neutral-900 dark:hover:text-neutral-300"
              >
    ...
    >
      <!-- Tab Switch -->
      <div
        class="mb-6 flex items-center justify-between gap-3 border-b border-neutral-800/60"
      >
        <div class="flex min-w-0 shrink-0">
          <button
            ...
            :class="[
              ...
              createMode === 'standard'
                ? 'border-neutral-600/60 text-neutral-300 dark:text-neutral-400'
                : 'border-transparent text-neutral-400 hover:text-neutral-400 dark:hover:text-neutral-400'
            ]"
          >
      ...
      >
        ...
        <div>
          ...
          <div class="relative">
            ...
            <button
              type="button"
              class="absolute right-3 top-1/2 -translate-y-1/2 text-neutral-500 hover:text-gray-600 dark:hover:text-neutral-400"
      ...
      <div v-else class="space-y-5">
        ...
        <!-- Parse Result -->
        <div v-if="batchParseResult.total > 0" class="rounded-lg bg-gray-50 p-4 dark:bg-neutral-900">
            <div class="flex items-center gap-4 text-sm">
              <div class="flex items-center gap-1.5">
              <Icon name="checkCircle" size="sm" :stroke-width="2" class="text-neutral-500" />
            ...
            <div v-if="batchParseResult.invalid > 0" class="flex items-center gap-1.5">
              <Icon
                ...
                size="sm"
                :stroke-width="2"
                class="text-[#d08770]/80"
              />
              <span class="text-[#d08770]/80 dark:text-[#d08770]">
    ...
    >
      <div v-if="qualityReport" class="space-y-4">
        <div class="rounded-lg border border-gray-200 bg-gray-50 p-4 dark:border-neutral-800/60 dark:bg-neutral-900">
          <div class="flex items-center justify-between gap-4">
            <div>
              <div class="text-sm text-neutral-400">
                {{ qualityReportProxy?.name || '-' }}
              </div>
              <div class="mt-1 text-sm text-gray-700 dark:text-neutral-500">
        ...
        <div class="max-h-80 overflow-auto rounded-lg border border-neutral-800/60">
          <table class="min-w-full divide-y divide-neutral-800/60 text-sm">
            <thead class="bg-[#0d0d0d] text-xs uppercase text-neutral-400">
              <tr>
            ...
            <tbody class="divide-y divide-neutral-800/60 bg-[#0d0d0d]">
              <tr v-for="item in qualityReport.items" :key="item.target">
                <td class="px-3 py-2 text-neutral-100">{{ qualityTargetLabel(item.target) }}</td>
                <td class="px-3 py-2">
                  <span class="badge" :class="qualityStatusClass(item.status)">{{ qualityStatusLabel(item.status) }}</span>
                </td>
                <td class="px-3 py-2 text-neutral-300">{{ item.http_status ?? '-' }}</td>
                <td class="px-3 py-2 text-neutral-300">
                  {{ typeof item.latency_ms === 'number' ? `${item.latency_ms}ms` : '-' }}
                </td>
                <td class="px-3 py-2 text-neutral-300">
                  <span>{{ item.message || '-' }}</span>
                  <span v-if="item.cf_ray" class="ml-1 text-xs text-neutral-500">(cf-ray: {{ item.cf_ray }})</span>
                </td>
              </tr>
            </tbody>
          </table>
    ...
    >
      <div v-if="accountsLoading" class="flex items-center justify-center py-8 text-sm text-neutral-400">
        <Icon name="refresh" size="md" class="mr-2 animate-spin" />
        {{ t('common.loading') }}
      </div>
      <div v-else-if="proxyAccounts.length === 0" class="py-6 text-center text-sm text-neutral-400">
        ...
        <table class="min-w-full divide-y divide-neutral-800/60 text-sm">
          ...
          <tbody class="divide-y divide-neutral-800/60 bg-[#0d0d0d]">
            <tr v-for="account in proxyAccounts" :key="account.id">
              <td class="px-4 py-2 font-medium text-neutral-100">{{ account.name }}</td>
              <td class="px-4 py-2">
