  ...
  <AppLayout>
      ...
      <template #table>
        ...
        >
          ...
          <template #cell-group_count="{ row }">
            <span
              class="inline-flex items-center rounded bg-gray-100 px-2 py-0.5 text-xs font-medium text-neutral-300 dark:bg-neutral-800 dark:text-neutral-300"
            >
    ...
    >
      <div class="channel-dialog-body">
        <!-- Tab Bar -->
        <div class="flex items-center border-b border-neutral-800/60 flex-shrink-0 -mx-4 sm:-mx-6 px-4 sm:px-6 -mt-3 sm:-mt-4">
          <!-- Basic Settings Tab -->
          <button
            type="button"
        ...
        <form id="channel-form" @submit.prevent="handleSubmit" class="flex-1 overflow-y-auto pt-4">
          ...
          <div v-show="activeTab === 'basic'" class="space-y-5">
            <!-- Name -->
            <div>
              <label class="input-label">{{ t('admin.channels.form.name', 'Name') }} <span class="text-[#bf616a]/80">*</span></label>
              <input
                v-model="form.name"
                type="text"
                required
            ...
            <div>
              <label class="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  class="h-4 w-4 rounded border-gray-300 text-neutral-300 focus:ring-neutral-500"
                />
                <span class="input-label mb-0">{{ t('admin.channels.form.restrictModels', 'Restrict Models') }}</span>
              </label>
              <p class="mt-1 ml-6 text-xs text-neutral-400">
              ...
              <div class="flex flex-wrap gap-2">
                <label
                  ...
                  :class="activePlatforms.includes(p)
                    ? 'bg-neutral-900/40 border-neutral-600/60 dark:bg-neutral-800/30 dark:border-neutral-700/60'
                    : 'border-gray-200 hover:bg-gray-50 dark:border-neutral-800/60 dark:hover:bg-neutral-900'"
                >
                  <input
                    type="checkbox"
                    :checked="activePlatforms.includes(p)"
                    class="h-3.5 w-3.5 rounded border-gray-300 text-neutral-300 focus:ring-neutral-500"
          ...
          >
            ...
            <div>
              <label class="input-label text-xs">
                {{ t('admin.channels.form.groups', 'Associated Groups') }} <span class="text-[#bf616a]/80">*</span>
                <span v-if="section.group_ids.length > 0" class="ml-1 font-normal text-neutral-400">
                  ({{ t('admin.channels.form.selectedCount', { count: section.group_ids.length }, `已选 ${section.group_ids.length} 个`) }})
                </span>
              </label>
              <div class="max-h-40 overflow-auto rounded-lg border border-gray-200 bg-gray-50 p-2 dark:border-neutral-800/60 dark:bg-[#0d0d0d]">
                <div v-if="groupsLoading" class="py-2 text-center text-xs text-neutral-500">
                  {{ t('common.loading', 'Loading...') }}
                </div>
                <div v-else-if="getGroupsForPlatform(section.platform).length === 0" class="py-2 text-center text-xs text-neutral-500">
                ...
                <div v-else class="flex flex-wrap gap-1">
                  <label
                    ...
                    :key="group.id"
                    class="inline-flex cursor-pointer items-center gap-1.5 rounded-md border border-gray-200 px-2 py-1 text-xs transition-colors hover:bg-gray-50 dark:border-neutral-800/60 dark:hover:bg-neutral-900"
                    :class="[
                      section.group_ids.includes(group.id) ? 'bg-neutral-900/40 border-neutral-600/60 dark:bg-neutral-800/30 dark:border-neutral-700/60' : '',
                  ...
                  >
                    <input
                      ...
                      class="h-3 w-3 rounded border-gray-300 text-neutral-300 focus:ring-neutral-500"
                    ...
                    <span class="text-[10px] text-neutral-400">{{ group.account_count || 0 }}</span>
                    <span
            ...
            <div v-if="section.platform === 'anthropic' && webSearchGlobalEnabled" class="border-t border-gray-200 pt-3 dark:border-neutral-800/60">
              <div class="flex items-center justify-between">
                <div>
                  <label class="text-xs font-medium text-neutral-300">
                    {{ t('admin.channels.form.webSearchEmulation') }}
                  </label>
                  <p class="mt-0.5 text-[11px] text-[#bf616a]/80 dark:text-[#bf616a]">
            ...
            <div v-if="section.platform === 'openai'" class="border-t border-gray-200 pt-3 dark:border-neutral-800/60">
              <div class="flex items-center justify-between gap-4">
                <div>
                  <label class="text-xs font-medium text-neutral-300">
                    {{ t('admin.channels.form.codexImageGenerationBridge') }}
                  </label>
                  <p class="mt-0.5 text-[11px] text-[#d08770]/80 dark:text-[#d08770]">
            ...
            <div v-if="section.platform === 'anthropic'" class="border-t border-gray-200 pt-3 dark:border-neutral-800/60">
              <div class="flex items-center justify-between gap-4">
                <div>
                  <label class="text-xs font-medium text-neutral-300">
            ...
            <div>
              <div class="mb-1 flex items-center justify-between">
                ...
                <button type="button" @click="addMappingEntry(sIdx)" class="text-xs text-neutral-300 hover:text-neutral-200">
                  + {{ t('common.add', 'Add') }}
                </button>
              </div>
              <div
                v-if="Object.keys(section.model_mapping).length === 0"
                class="rounded border border-dashed border-gray-300 p-2 text-center text-xs text-neutral-400 dark:border-neutral-700/60"
              >
              ...
              <div v-else class="space-y-1">
                ...
                >
                  ...
                  <button
                    ...
                    class="rounded p-0.5 text-neutral-400 hover:text-red-400/80"
                  >
                    <Icon name="trash" size="sm" />
                  </button>
                </div>
              ...
              <div class="mb-1 flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                  <button
                    ...
                    class="text-xs text-neutral-500 hover:text-neutral-200 disabled:opacity-50"
                  >
                    {{ syncingPlatform === section.platform ? t('admin.channels.form.syncingModels') : t('admin.channels.form.syncLatestModels') }}
                  </button>
                  <button type="button" @click="addPricingEntry(sIdx)" class="text-xs text-neutral-300 hover:text-neutral-200">
                    + {{ t('common.add', 'Add') }}
                  </button>
                </div>
              </div>
            ...
            <div class="mt-4 border-t border-gray-200 pt-4 dark:border-neutral-800/60 space-y-3">
              <div class="flex items-center justify-between">
                <h4 class="text-sm font-medium text-neutral-300">
                  {{ t('admin.channels.form.accountStatsPricingRules') }}
                </h4>
                <button
                  type="button"
                  @click="addAccountStatsRule(sIdx)"
                  class="rounded-lg border border-neutral-600/60 px-3 py-1 text-xs font-medium text-neutral-300 hover:bg-neutral-900/40 dark:border-neutral-600 dark:text-neutral-400 dark:hover:bg-neutral-800/30"
                >
              ...
              <div
                ...
                :key="ruleIndex"
                class="space-y-3 rounded-lg border border-gray-200 p-4 dark:border-neutral-800/60"
              >
                <div class="flex items-center justify-between">
                  <input
                    v-model="rule.name"
                    :placeholder="t('admin.channels.form.ruleName')"
                    class="bg-transparent text-sm font-medium text-gray-700 placeholder-gray-400 outline-none dark:text-neutral-300"
                  />
                  <button type="button" @click="removeAccountStatsRule(sIdx, ruleIndex)" class="text-xs text-[#bf616a]/80 hover:text-red-400/80">
                    {{ t('common.delete') }}
                  </button>
                </div>

                <div>
                  <label class="text-xs text-neutral-400">{{ t('admin.channels.form.ruleGroups') }}</label>
                  <div class="mt-1 flex flex-wrap gap-1">
                    <label
                      ...
                      :class="rule.group_ids.includes(gid)
                        ? 'border-neutral-600/60 bg-neutral-900/40 dark:border-neutral-700/60 dark:bg-neutral-800/30'
                        : 'border-gray-200 hover:bg-gray-50 dark:border-neutral-800/60 dark:hover:bg-neutral-900'"
                    >
                      <input type="checkbox" :checked="rule.group_ids.includes(gid)" class="h-3 w-3 rounded border-gray-300 text-neutral-300 focus:ring-neutral-500" @change="rule.group_ids.includes(gid) ? rule.group_ids.splice(rule.group_ids.indexOf(gid), 1) : rule.group_ids.push(gid)" />
                      <span :class="['font-medium', platformTextClass(section.platform)]">{{ getGroupNameById(gid) }}</span>
                    </label>
                  </div>
                  <p v-if="section.group_ids.length === 0" class="mt-1 text-xs text-neutral-400">
                    {{ t('admin.channels.form.noGroupsInChannel') }}
                  </p>
                </div>

                <div>
                  <label class="text-xs text-neutral-400">{{ t('admin.channels.form.ruleAccounts') }}</label>
                  <!-- Selected account chips -->
                  <div class="mt-1 flex flex-wrap gap-1">
                    <span
                      v-for="accountId in rule.account_ids"
                      :key="accountId"
                      class="inline-flex items-center gap-1 rounded-md border border-neutral-600/60 bg-neutral-900/40 px-2 py-0.5 text-xs dark:border-neutral-700/60 dark:bg-neutral-800/30"
                    >
                      <span :class="['font-medium', platformTextClass(section.platform)]">{{ getRuleAccountLabel(accountId) }}</span>
                      <button type="button" @click="removeRuleAccount(rule, accountId)" class="text-neutral-400 hover:text-red-400/80">
                        <Icon name="x" size="xs" />
                      </button>
                    </span>
                  </div>
                  ...
                  <div class="relative mt-1 rule-account-search-container">
                    ...
                    >
                      ...
                      >
                        ...
                        <span class="ml-2 text-xs text-neutral-400">#{{ account.id }}</span>
                      </button>
                    </div>
                  </div>
                  <p class="mt-1 text-xs text-neutral-400">
                ...
                <div>
                  <div class="mb-1 flex items-center justify-between">
                    <label class="text-xs text-neutral-400">{{ t('admin.channels.form.ruleModelPricing') }}</label>
                    <button type="button" @click="addRulePricingEntry(sIdx, ruleIndex)" class="text-xs text-neutral-300 hover:text-neutral-200">
                      + {{ t('common.add') }}
                    </button>
                  </div>
                  <div v-if="rule.pricing.length === 0" class="rounded border border-dashed border-gray-300 p-2 text-center text-xs text-neutral-400 dark:border-neutral-700/60">
...
.channel-tab-active {
  @apply border-neutral-600 text-neutral-300 dark:border-neutral-600/50 dark:text-neutral-400;
}

.channel-tab-inactive {
  @apply border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 dark:text-neutral-400 dark:hover:text-gray-300;
}
</style>
