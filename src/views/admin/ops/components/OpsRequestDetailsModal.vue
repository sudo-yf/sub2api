...
const kindBadgeClass = (kind: string) => {
  if (kind === 'error') return 'bg-[#bf616a] text-white dark:bg-[#bf616a]/12 dark:text-[#bf616a]'
      ...
      <div class="flex h-full min-h-0 flex-col">
        ...
        <div v-if="loading" class="flex flex-1 items-center justify-center py-16">
          <div class="flex flex-col items-center gap-3">
            <svg class="h-8 w-8 animate-spin text-[#81a1c1]/80" fill="none" viewBox="0 0 24 24">
            ...
            <span class="text-sm font-medium text-neutral-400">{{ t('common.loading') }}</span>
          </div>
        </div>

        <!-- Table -->
        <div v-else class="flex min-h-0 flex-1 flex-col">
          <div v-if="items.length === 0" class="rounded-xl border border-dashed border-gray-200 p-10 text-center dark:border-neutral-800/60">
            <div class="text-sm font-medium text-neutral-300">{{ t('admin.ops.requestDetails.empty') }}</div>
            <div class="mt-1 text-xs text-neutral-500">{{ t('admin.ops.requestDetails.emptyHint') }}</div>
          </div>

          <div v-else class="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-neutral-800/60">
            <div class="min-h-0 flex-1 overflow-auto">
              ...
              <tbody class="divide-y divide-neutral-800/60 bg-[#111111]">
                <tr v-for="(row, idx) in items" :key="idx" class="hover:bg-[#161616]/50">
                  ...
                  <td class="whitespace-nowrap px-4 py-3 text-xs font-medium text-neutral-200">
                    {{ (row.platform || 'unknown').toUpperCase() }}
                  </td>
                  <td class="max-w-[240px] truncate px-4 py-3 text-xs text-neutral-300" :title="row.model || ''">
                    {{ row.model || '-' }}
                  </td>
                  <td class="whitespace-nowrap px-4 py-3 text-xs text-neutral-300">
                  ...
                  <td class="px-4 py-3">
                    <div v-if="row.request_id" class="flex items-center gap-2">
                      <span class="max-w-[220px] truncate font-mono text-[11px] text-neutral-200" :title="row.request_id">
                        {{ row.request_id }}
                      </span>
                      <button
                        class="rounded-md bg-gray-100 px-2 py-1 text-[10px] font-bold text-gray-600 hover:bg-neutral-800 dark:text-neutral-300 dark:hover:bg-dark-600"
                  ...
                  <td class="whitespace-nowrap px-4 py-3 text-right">
                    <button
                      ...
                      class="rounded-lg bg-[#bf616a]/12 px-3 py-1.5 text-xs font-bold text-white hover:bg-[#bf616a]/12 dark:bg-[#bf616a]/12 dark:text-[#bf616a] dark:hover:bg-[#bf616a]/15"
