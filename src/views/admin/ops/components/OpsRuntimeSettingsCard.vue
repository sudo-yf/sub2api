...
<template>
  <div class="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-gray-900/5 dark:bg-neutral-800 dark:ring-dark-700">
    <div class="mb-4 flex items-start justify-between gap-4">
      <div>
        <h3 class="text-sm font-bold text-neutral-100 dark:text-white">{{ t('admin.ops.runtime.title') }}</h3>
        <p class="mt-1 text-xs text-neutral-500 dark:text-neutral-400">{{ t('admin.ops.runtime.description') }}</p>
      </div>
      <button
        class="flex items-center gap-1.5 rounded-lg bg-gray-100 px-3 py-1.5 text-xs font-bold text-gray-700 transition-colors hover:bg-gray-200 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-neutral-900 dark:text-neutral-300 dark:hover:bg-dark-600"
        :disabled="loading"
        @click="loadSettings"
      >
    ...
    <div v-if="!alertSettings" class="text-sm text-neutral-500 dark:text-neutral-400">
      <span v-if="loading">{{ t('admin.ops.runtime.loading') }}</span>
      <span v-else>{{ t('admin.ops.runtime.noData') }}</span>
    </div>

    <div v-else class="space-y-6">
      <div class="rounded-2xl bg-gray-50 p-4 dark:bg-neutral-900/50">
        <div class="mb-3 flex items-center justify-between">
          <h4 class="text-sm font-semibold text-neutral-100 dark:text-white">{{ t('admin.ops.runtime.alertTitle') }}</h4>
        ...
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
          <div class="text-xs text-gray-600 dark:text-neutral-300">
            ...
            <span class="ml-1 font-medium text-neutral-100 dark:text-white">{{ alertSettings.evaluation_interval_seconds }}s</span>
          </div>
          <div
            v-if="alertSettings.silencing?.enabled && alertSettings.silencing.global_until_rfc3339"
            class="text-xs text-gray-600 dark:text-neutral-300 md:col-span-2"
          >
            {{ t('admin.ops.runtime.silencing.globalUntil') }}:
            <span class="ml-1 font-mono text-neutral-100 dark:text-white">{{ alertSettings.silencing.global_until_rfc3339 }}</span>
          </div>

          <details class="col-span-1 md:col-span-2">
            <summary class="cursor-pointer text-xs font-medium text-[#81a1c1]/80 hover:text-blue-400/80 dark:text-[#81a1c1]">
              {{ t('admin.ops.runtime.showAdvancedDeveloperSettings') }}
            </summary>
            <div class="mt-2 grid grid-cols-1 gap-3 rounded-lg bg-gray-100 p-3 dark:bg-neutral-800 md:grid-cols-2">
              <div class="text-xs text-neutral-500 dark:text-neutral-400">
                {{ t('admin.ops.runtime.lockEnabled') }}:
                <span class="ml-1 font-mono text-neutral-600 dark:text-neutral-300">{{ alertSettings.distributed_lock.enabled }}</span>
              </div>
              <div class="text-xs text-neutral-500 dark:text-neutral-400">
                {{ t('admin.ops.runtime.lockKey') }}:
                <span class="ml-1 font-mono text-neutral-600 dark:text-neutral-300">{{ alertSettings.distributed_lock.key }}</span>
              </div>
              <div class="text-xs text-neutral-500 dark:text-neutral-400">
                {{ t('admin.ops.runtime.lockTTLSeconds') }}:
                <span class="ml-1 font-mono text-neutral-600 dark:text-neutral-300">{{ alertSettings.distributed_lock.ttl_seconds }}s</span>
              </div>
            </div>
          </details>
        </div>
  ...
  <BaseDialog :show="showAlertEditor" :title="t('admin.ops.runtime.alertTitle')" width="extra-wide" @close="showAlertEditor = false">
    <div v-if="draftAlert" class="space-y-4">
      <div
        ...
        class="rounded-lg border border-[#d08770]/20 bg-[#ebcb8b] p-3 text-xs text-[#d08770] dark:border-[#d08770]/25 dark:bg-[#ebcb8b]/15 dark:text-[#ebcb8b]"
      >
      ...
      <div>
        <div class="mb-1 text-xs font-medium text-gray-600 dark:text-neutral-300">{{ t('admin.ops.runtime.evalIntervalSeconds') }}</div>
        <input
        ...
        />
        <p class="mt-1 text-xs text-neutral-500">{{ t('admin.ops.runtime.evalIntervalHint') }}</p>
      </div>

      <div class="rounded-2xl bg-gray-50 p-4 dark:bg-neutral-900/50">
        <div class="mb-2 text-sm font-semibold text-neutral-100 dark:text-white">{{ t('admin.ops.runtime.metricThresholds') }}</div>
        <p class="mb-4 text-xs text-neutral-500 dark:text-neutral-400">{{ t('admin.ops.runtime.metricThresholdsHint') }}</p>

        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <div class="mb-1 text-xs font-medium text-gray-600 dark:text-neutral-300">{{ t('admin.ops.runtime.slaMinPercent') }}</div>
            <input
              ...
              placeholder="99.5"
            />
            <p class="mt-1 text-xs text-neutral-500 dark:text-neutral-400">{{ t('admin.ops.runtime.slaMinPercentHint') }}</p>
          </div>



          <div>
            <div class="mb-1 text-xs font-medium text-gray-600 dark:text-neutral-300">{{ t('admin.ops.runtime.ttftP99MaxMs') }}</div>
            <input
              ...
              placeholder="500"
            />
            <p class="mt-1 text-xs text-neutral-500 dark:text-neutral-400">{{ t('admin.ops.runtime.ttftP99MaxMsHint') }}</p>
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-gray-600 dark:text-neutral-300">{{ t('admin.ops.runtime.requestErrorRateMaxPercent') }}</div>
            <input
              ...
              placeholder="5"
            />
            <p class="mt-1 text-xs text-neutral-500 dark:text-neutral-400">{{ t('admin.ops.runtime.requestErrorRateMaxPercentHint') }}</p>
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-gray-600 dark:text-neutral-300">{{ t('admin.ops.runtime.upstreamErrorRateMaxPercent') }}</div>
            <input
              ...
              placeholder="5"
            />
            <p class="mt-1 text-xs text-neutral-500 dark:text-neutral-400">{{ t('admin.ops.runtime.upstreamErrorRateMaxPercentHint') }}</p>
          </div>
        </div>
      </div>

      <div class="rounded-2xl bg-gray-50 p-4 dark:bg-neutral-900/50">
        <div class="mb-2 text-sm font-semibold text-neutral-100 dark:text-white">{{ t('admin.ops.runtime.silencing.title') }}</div>
        ...
        <div v-if="draftAlert.silencing.enabled" class="mt-4 space-y-4">
          <div>
            <div class="mb-1 text-xs font-medium text-gray-600 dark:text-neutral-300">{{ t('admin.ops.runtime.silencing.globalUntil') }}</div>
            <input
            ...
            />
            <p class="mt-1 text-xs text-neutral-500 dark:text-neutral-400">{{ t('admin.ops.runtime.silencing.untilHint') }}</p>
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-gray-600 dark:text-neutral-300">{{ t('admin.ops.runtime.silencing.reason') }}</div>
            <input
          ...
          <div class="rounded-xl border border-gray-200 bg-white p-4 dark:border-neutral-800/60 dark:bg-neutral-800">
            <div class="flex items-start justify-between gap-4">
              <div>
                <div class="text-xs font-bold text-neutral-100 dark:text-white">{{ t('admin.ops.runtime.silencing.entries.title') }}</div>
                <p class="text-[11px] text-neutral-500 dark:text-neutral-400">{{ t('admin.ops.runtime.silencing.entries.hint') }}</p>
              </div>
            ...
            <div v-if="!draftAlert.silencing.entries?.length" class="mt-3 rounded-lg bg-gray-50 p-3 text-xs text-neutral-500 dark:bg-[#0d0d0d] dark:text-neutral-400">
            ...
            <div v-else class="mt-4 space-y-4">
              <div
                ...
                :key="idx"
                class="rounded-lg border border-gray-200 bg-gray-50 p-4 dark:border-neutral-800/60 dark:bg-[#0d0d0d]"
              >
                <div class="mb-3 flex items-center justify-between">
                  <div class="text-xs font-bold text-neutral-100 dark:text-white">
                ...
                <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                  <div>
                    <div class="mb-1 text-xs font-medium text-gray-600 dark:text-neutral-300">{{ t('admin.ops.runtime.silencing.entries.ruleId') }}</div>
                    <input
                  ...
                  <div>
                    <div class="mb-1 text-xs font-medium text-gray-600 dark:text-neutral-300">{{ t('admin.ops.runtime.silencing.entries.severities') }}</div>
                    <input
                  ...
                  <div>
                    <div class="mb-1 text-xs font-medium text-gray-600 dark:text-neutral-300">{{ t('admin.ops.runtime.silencing.entries.until') }}</div>
                    <input
              ...
              placeholder="2026-01-05T00:00:00Z"
                  ...
                  <div>
                    <div class="mb-1 text-xs font-medium text-gray-600 dark:text-neutral-300">{{ t('admin.ops.runtime.silencing.entries.reason') }}</div>
                    <input
      ...
      <details class="rounded-lg border border-gray-200 bg-gray-50 p-3 dark:border-neutral-800/60 dark:bg-neutral-800">
        <summary class="cursor-pointer text-xs font-medium text-gray-600 dark:text-neutral-400">{{ t('admin.ops.runtime.advancedSettingsSummary') }}</summary>
        <div class="mt-3 grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div class="md:col-span-2">
            <div class="mb-1 text-xs font-medium text-neutral-500">{{ t('admin.ops.runtime.lockKey') }}</div>
            <input v-model="draftAlert.distributed_lock.key" type="text" class="input text-xs font-mono" />
            <p v-if="draftAlert.distributed_lock.enabled" class="mt-1 text-[11px] text-neutral-500 dark:text-neutral-400">
              {{ t('admin.ops.runtime.validation.lockKeyHint', { prefix: 'ops:' }) }}
            </p>
          </div>
          <div>
            <div class="mb-1 text-xs font-medium text-neutral-500">{{ t('admin.ops.runtime.lockTTLSeconds') }}</div>
