...
function severityBadgeClass(severity: string | undefined): string {
  ...
  if (s === 'p0' || s === 'critical') return 'bg-[#bf616a] text-white dark:bg-[#bf616a]/12 dark:text-[#bf616a]'
  if (s === 'p1' || s === 'warning') return 'bg-[#ebcb8b] text-neutral-900 dark:bg-[#ebcb8b]/15 dark:text-[#ebcb8b]'
  if (s === 'p2' || s === 'info') return 'bg-[#81a1c1] text-white dark:bg-[#81a1c1]/12 dark:text-[#81a1c1]'
  if (s === 'p3') return 'bg-gray-100 text-gray-700 dark:bg-neutral-900 dark:text-neutral-300'
  return 'bg-gray-100 text-gray-700 dark:bg-neutral-900 dark:text-neutral-300'
}

function statusBadgeClass(status: string | undefined): string {
  const s = String(status || '').trim().toLowerCase()
  if (s === 'firing') return 'bg-[#bf616a] text-white ring-red-600/20 dark:bg-[#bf616a]/12 dark:text-[#bf616a] dark:ring-red-500/30'
  if (s === 'resolved') return 'bg-[#a3be8c] text-white ring-green-600/20 dark:bg-[#a3be8c]/12 dark:text-[#a3be8c] dark:ring-green-500/30'
  ...
  return 'bg-gray-50 text-gray-700 ring-gray-600/20 dark:bg-neutral-900/30 dark:text-neutral-300 dark:ring-gray-500/30'
}
...
<template>
  <div class="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-gray-900/5 dark:bg-neutral-800 dark:ring-dark-700">
    <div class="mb-4 flex items-start justify-between gap-4">
      <div>
        <h3 class="text-sm font-bold text-neutral-100">{{ t('admin.ops.alertEvents.title') }}</h3>
        <p class="mt-1 text-xs text-neutral-400">{{ t('admin.ops.alertEvents.description') }}</p>
      </div>
    ...
    <div v-else-if="empty" class="rounded-xl border border-dashed border-gray-200 p-8 text-center text-sm text-neutral-500 dark:border-neutral-800/60 dark:text-neutral-400">
      {{ t('admin.ops.alertEvents.empty') }}
    </div>

    <div v-else class="overflow-hidden rounded-xl border border-neutral-800/60">
      <div class="max-h-[600px] overflow-y-auto" @scroll="onScroll">
        <table class="min-w-full divide-y divide-neutral-800/60">
          <thead class="sticky top-0 z-10 bg-gray-50 dark:bg-[#0d0d0d]">
            <tr>
              <th class="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-neutral-400">
              ...
              <th class="px-4 py-3 text-right text-[11px] font-bold uppercase tracking-wider text-neutral-400">
                {{ t('admin.ops.alertEvents.table.email') }}
              </th>
            </tr>
          </thead>
          <tbody class="divide-y divide-neutral-800/60 bg-[#111111]">
            <tr
              v-for="row in events"
              :key="row.id"
              class="cursor-pointer hover:bg-[#161616]/50"
              @click="openDetail(row)"
              :title="row.title || ''"
            >
              <td class="whitespace-nowrap px-4 py-3 text-xs text-neutral-300">
              ...
              <td class="min-w-[260px] px-4 py-3 text-xs text-neutral-200">
                <div class="font-semibold truncate max-w-[360px]">{{ row.title || '-' }}</div>
                <div v-if="row.description" class="mt-0.5 line-clamp-2 text-[11px] text-neutral-400">
                  {{ row.description }}
                </div>
              </td>
              <td class="whitespace-nowrap px-4 py-3 text-xs text-neutral-300">
              ...
              <td class="whitespace-nowrap px-4 py-3 text-[11px] text-neutral-400">
              ...
              <td class="whitespace-nowrap px-4 py-3 text-right text-xs">
                ...
                >
                  <Icon
                    ...
                    name="checkCircle"
                    size="sm"
                    class="text-[#a3be8c]/80 dark:text-[#a3be8c]"
                  />
                  <Icon
                    v-else
                    name="ban"
                  ...
                  />
                  <span class="text-[11px] font-bold text-neutral-300">
        ...
        <div v-if="loadingMore" class="flex items-center justify-center gap-2 py-3 text-xs text-neutral-400">
          <svg class="h-4 w-4 animate-spin" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
          </svg>
        ...
        <div v-else-if="!hasMore && events.length > 0" class="py-3 text-center text-xs text-neutral-500">
          -
        </div>
      </div>
    </div>
    ...
    <BaseDialog
      ...
      @close="closeDetail"
    >
      <div v-if="detailLoading" class="flex items-center justify-center py-10 text-sm text-neutral-400">
        {{ t('admin.ops.alertEvents.detail.loading') }}
      </div>

      <div v-else-if="!selected" class="py-10 text-center text-sm text-neutral-400">
        {{ t('admin.ops.alertEvents.detail.empty') }}
      </div>

      <div v-else class="space-y-5">
        <div class="rounded-xl bg-gray-50 p-4 dark:bg-[#0d0d0d]">
          <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <div v-if="selected.description" class="mt-1 whitespace-pre-wrap text-xs text-neutral-300">
                {{ selected.description }}
              </div>
            </div>

            <div class="flex flex-wrap gap-2">
              <div class="flex items-center gap-2 rounded-lg bg-white px-2 py-1 ring-1 ring-gray-200 dark:bg-neutral-800 dark:ring-dark-700">
                <span class="text-[11px] font-bold text-neutral-300">{{ t('admin.ops.alertEvents.detail.silence') }}</span>
                <Select
          ...
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div class="rounded-xl bg-gray-50 p-4 dark:bg-[#0d0d0d]">
              <div class="text-xs font-bold uppercase tracking-wider text-neutral-500">{{ t('admin.ops.alertEvents.detail.firedAt') }}</div>
              <div class="mt-1 text-sm font-medium text-neutral-100">{{ formatDateTime(selected.fired_at || selected.created_at) }}</div>
            </div>
            <div class="rounded-xl bg-gray-50 p-4 dark:bg-[#0d0d0d]">
              <div class="text-xs font-bold uppercase tracking-wider text-neutral-500">{{ t('admin.ops.alertEvents.detail.resolvedAt') }}</div>
              <div class="mt-1 text-sm font-medium text-neutral-100">{{ selected.resolved_at ? formatDateTime(selected.resolved_at) : '-' }}</div>
            </div>
            <div class="rounded-xl bg-gray-50 p-4 dark:bg-[#0d0d0d]">
              <div class="text-xs font-bold uppercase tracking-wider text-neutral-500">{{ t('admin.ops.alertEvents.detail.ruleId') }}</div>
              <div class="mt-1 flex flex-wrap items-center gap-2">
                <div class="font-mono text-sm font-bold text-neutral-100">#{{ selected.rule_id }}</div>
                <a
                  class="inline-flex items-center gap-1 rounded-md bg-white px-2 py-1 text-[11px] font-bold text-gray-700 ring-1 ring-gray-200 hover:bg-[#0d0d0d] dark:text-neutral-500 dark:ring-dark-700 dark:hover:bg-neutral-900"
            ...
            <div class="rounded-xl bg-gray-50 p-4 dark:bg-[#0d0d0d]">
              <div class="text-xs font-bold uppercase tracking-wider text-neutral-500">{{ t('admin.ops.alertEvents.detail.dimensions') }}</div>
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-4 dark:border-neutral-800/60 dark:bg-neutral-800">
          <div class="mb-3 flex flex-wrap items-center justify-between gap-3">
            <div>
              <div class="text-sm font-bold text-neutral-100">{{ t('admin.ops.alertEvents.detail.historyTitle') }}</div>
              <div class="mt-0.5 text-xs text-neutral-400">{{ t('admin.ops.alertEvents.detail.historyHint') }}</div>
            </div>
            <Select :model-value="historyRange" :options="historyRangeOptions" class="w-[140px]" @change="historyRange = String($event || '7d')" />
          </div>

          <div v-if="historyLoading" class="py-6 text-center text-xs text-neutral-400">
            {{ t('admin.ops.alertEvents.detail.historyLoading') }}
          </div>
          <div v-else-if="history.length === 0" class="py-6 text-center text-xs text-neutral-400">
            {{ t('admin.ops.alertEvents.detail.historyEmpty') }}
          </div>
          <div v-else class="overflow-hidden rounded-lg border border-neutral-800/60">
            <table class="min-w-full divide-y divide-neutral-800/60">
              <thead class="bg-gray-50 dark:bg-[#0d0d0d]">
                <tr>
                  <th class="px-3 py-2 text-left text-[11px] font-bold uppercase tracking-wider text-neutral-400">{{ t('admin.ops.alertEvents.table.time') }}</th>
                  <th class="px-3 py-2 text-left text-[11px] font-bold uppercase tracking-wider text-neutral-400">{{ t('admin.ops.alertEvents.table.status') }}</th>
                  <th class="px-3 py-2 text-left text-[11px] font-bold uppercase tracking-wider text-neutral-400">{{ t('admin.ops.alertEvents.table.metric') }}</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-neutral-800/60">
                <tr v-for="it in history" :key="it.id" class="hover:bg-[#161616]/50">
                  <td class="px-3 py-2 text-xs text-neutral-300">{{ formatDateTime(it.fired_at || it.created_at) }}</td>
