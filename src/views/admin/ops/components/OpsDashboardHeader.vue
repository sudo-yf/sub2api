...
function getThresholdColorClass(level: ThresholdLevel): string {
  switch (level) {
    case 'critical':
      return 'text-[#bf616a]/80 dark:text-[#bf616a]'
    case 'warning':
      return 'text-[#ebcb8b]/80 dark:text-[#ebcb8b]'
    default:
      return 'text-[#a3be8c]/80 dark:text-[#a3be8c]'
  }
}
...
const healthScoreColor = computed(() => {
  if (isSystemIdle.value) return '#9099a8'
  const score = healthScoreValue.value
  if (score == null) return '#9099a8'
  if (score >= 90) return '#a3be8c'
  if (score >= 60) return '#ebcb8b'
  return '#bf616a'
})

const healthScoreClass = computed(() => {
  if (isSystemIdle.value) return 'text-neutral-400'
  const score = healthScoreValue.value
  if (score == null) return 'text-neutral-400'
  if (score >= 90) return 'text-[#a3be8c]/80'
  if (score >= 60) return 'text-[#ebcb8b]/80'
  return 'text-[#bf616a]/80'
})
...
const cpuPercentClass = computed(() => {
  ...
  if (v == null) return 'text-neutral-100'
  if (v >= 95) return 'text-[#b48ead]/80 dark:text-[#b48ead]'
  if (v >= 80) return 'text-[#ebcb8b]/80 dark:text-[#ebcb8b]'
  return 'text-[#88c0d0]/80 dark:text-[#88c0d0]/80'
})
...
const memPercentClass = computed(() => {
  ...
  if (v == null) return 'text-neutral-100'
  if (v >= 95) return 'text-[#b48ead]/80 dark:text-[#b48ead]'
  if (v >= 85) return 'text-[#ebcb8b]/80 dark:text-[#ebcb8b]'
  return 'text-[#88c0d0]/80 dark:text-[#88c0d0]/80'
})
...
const dbMiddleClass = computed(() => {
  if (systemMetrics.value?.db_ok === false) return 'text-[#b48ead]/80 dark:text-[#b48ead]'
  if (dbUsagePercent.value != null) {
    if (dbUsagePercent.value >= 90) return 'text-[#b48ead]/80 dark:text-[#b48ead]'
    if (dbUsagePercent.value >= 70) return 'text-[#ebcb8b]/80 dark:text-[#ebcb8b]'
    return 'text-[#88c0d0]/80 dark:text-[#88c0d0]/80'
  }
  if (systemMetrics.value?.db_ok === true) return 'text-[#88c0d0]/80 dark:text-[#88c0d0]/80'
...
const redisMiddleClass = computed(() => {
  if (systemMetrics.value?.redis_ok === false) return 'text-[#b48ead]/80 dark:text-[#b48ead]'
  if (redisUsagePercent.value != null) {
    if (redisUsagePercent.value >= 90) return 'text-[#b48ead]/80 dark:text-[#b48ead]'
    if (redisUsagePercent.value >= 70) return 'text-[#ebcb8b]/80 dark:text-[#ebcb8b]'
    return 'text-[#88c0d0]/80 dark:text-[#88c0d0]/80'
  }
  if (systemMetrics.value?.redis_ok === true) return 'text-[#88c0d0]/80 dark:text-[#88c0d0]/80'
  return 'text-neutral-100'
})
...
const goroutineStatusClass = computed(() => {
  switch (goroutineStatus.value) {
    ...
    case 'warning':
      return 'text-[#ebcb8b]/80 dark:text-[#ebcb8b]'
    case 'critical':
      return 'text-[#b48ead]/80 dark:text-[#b48ead]'
    default:
      return 'text-neutral-100'
  }
})
...
<template>
  <div :class="['flex flex-col gap-4 rounded-3xl bg-white shadow-sm ring-1 ring-gray-900/5 dark:bg-neutral-800 dark:ring-dark-700', props.fullscreen ? 'p-8' : 'p-6']">
    <!-- Top Toolbar -->
    <div class="flex flex-wrap items-center justify-between gap-4 border-b border-gray-100 pb-4 dark:border-neutral-800/60">
      <div>
        <h1 class="flex items-center gap-2 text-xl font-black text-neutral-100">
          <svg class="h-6 w-6 text-[#81a1c1]/80" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path
        ...
        <div v-if="!props.fullscreen" class="mt-1 flex items-center gap-3 text-xs text-neutral-400">
          <span class="flex items-center gap-1.5" :title="props.loading ? t('admin.ops.loadingText') : t('admin.ops.ready')">
            <span class="relative flex h-2 w-2">
              <span class="relative inline-flex h-2 w-2 rounded-full" :class="props.loading ? 'bg-gray-400' : 'bg-[#a3be8c]'"></span>
            </span>
      ...
      <div class="flex flex-wrap items-center gap-3">
        <template v-if="!props.fullscreen">
          ...
          <div class="mx-1 hidden h-4 w-[1px] bg-neutral-800 sm:block"></div>
        ...
        <button
          ...
          type="button"
          class="flex h-8 w-8 items-center justify-center rounded-lg bg-gray-100 text-neutral-500 transition-colors hover:bg-neutral-800 dark:text-neutral-400 dark:hover:bg-dark-600"
          :disabled="loading"
        ...
        <div v-if="!props.fullscreen" class="mx-1 hidden h-4 w-[1px] bg-neutral-800 sm:block"></div>

        <!-- Alert Rules Button (hidden in fullscreen) -->
        <button
          v-if="!props.fullscreen"
          type="button"
          class="flex h-8 items-center gap-1.5 rounded-lg bg-[#81a1c1]/12 px-3 text-xs font-bold text-white transition-colors hover:bg-[#81a1c1]/15 dark:bg-[#81a1c1]/12 dark:text-[#81a1c1] dark:hover:bg-[#81a1c1]/15"
        ...
        <button
          v-if="!props.fullscreen"
          type="button"
          class="flex h-8 items-center gap-1.5 rounded-lg bg-gray-100 px-3 text-xs font-bold text-gray-700 transition-colors hover:bg-neutral-800 dark:text-neutral-300 dark:hover:bg-dark-600"
        ...
        <button
          v-if="!props.fullscreen"
          type="button"
          class="flex h-8 w-8 items-center justify-center rounded-lg bg-gray-100 text-gray-700 transition-colors hover:bg-neutral-800 dark:text-neutral-300 dark:hover:bg-dark-600"
    ...
    <div v-if="overview" class="grid grid-cols-1 gap-6 lg:grid-cols-12">
      ...
      <div :class="['rounded-2xl bg-gray-50 dark:bg-[#0d0d0d] lg:col-span-5', props.fullscreen ? 'p-6' : 'p-4']">
        <div class="grid h-full grid-cols-1 gap-6 md:grid-cols-[200px_1fr] md:items-center">
          <!-- 1) Health Score -->
          <div
            class="group relative flex cursor-pointer flex-col items-center justify-center rounded-xl py-2 transition-all hover:bg-white/60 dark:hover:bg-neutral-800/60 md:border-r md:border-gray-200 md:pr-6 dark:md:border-neutral-800/60"
          >
            ...
            >
              <div class="rounded-xl bg-white p-4 shadow-xl ring-1 ring-black/5 dark:bg-[#1e1e1e] dark:ring-white/10">
                <h4 class="mb-3 border-b border-gray-100 pb-2 text-sm font-bold text-gray-900 dark:border-gray-700 dark:text-white flex items-center gap-2">
                  <Icon name="brain" size="sm" class="text-[#81a1c1]/80" />
                ...
                <div class="space-y-3">
                  <div v-for="(item, idx) in diagnosisReport" :key="idx" class="flex gap-3">
                    <div class="mt-0.5 shrink-0">
                      <svg v-if="item.type === 'critical'" class="h-4 w-4 text-[#bf616a]/80" fill="currentColor" viewBox="0 0 20 20">
                        <path
                          fill-rule="evenodd"
                      ...
                      <svg v-else-if="item.type === 'warning'" class="h-4 w-4 text-[#ebcb8b]/80" fill="currentColor" viewBox="0 0 20 20">
                        <path
                          fill-rule="evenodd"
                      ...
                      <svg v-else class="h-4 w-4 text-[#81a1c1]/80" fill="currentColor" viewBox="0 0 20 20">
                        <path
                          fill-rule="evenodd"
                    ...
                    <div class="flex-1">
                      <div class="text-xs font-semibold text-neutral-100">{{ item.message }}</div>
                      <div class="mt-0.5 text-[11px] text-neutral-400">{{ item.impact }}</div>
                      <div v-if="item.action" class="mt-1 text-[11px] text-[#81a1c1]/80 dark:text-[#81a1c1] flex items-center gap-1">
                ...
                <div class="mt-3 border-t border-gray-100 pt-2 text-[10px] text-neutral-400 dark:border-gray-700">
            ...
            <div class="relative flex items-center justify-center">
              <svg :width="circleSize" :height="circleSize" class="-rotate-90 transform">
                <circle
                  ...
                  fill="transparent"
                  class="text-neutral-500 dark:text-dark-700"
                  stroke="currentColor"
                />
                <circle
                  :cx="circleSize / 2"
              ...
              <div class="absolute flex flex-col items-center">
                ...
                <span :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-bold uppercase tracking-wider text-neutral-400']">{{ t('admin.ops.health') }}</span>
              </div>
            </div>

            <div class="mt-4 text-center" v-if="!props.fullscreen">
              <div class="flex items-center justify-center gap-1 text-xs font-medium text-neutral-500">
          ...
          <div class="flex h-full flex-col justify-center py-2">
            <div class="mb-3 flex flex-wrap items-center justify-between gap-2">
              <div class="flex items-center gap-2">
                <div class="relative flex h-3 w-3 shrink-0">
                  <span class="absolute inline-flex h-full w-full animate-ping rounded-full bg-blue-800/50 opacity-75"></span>
                  <span class="relative inline-flex h-3 w-3 rounded-full bg-[#81a1c1]"></span>
                </div>
                <h3 class="text-xs font-bold uppercase tracking-wider text-neutral-400">{{ t('admin.ops.realtime.title') }}</h3>
              ...
              <div class="flex flex-wrap gap-1">
                <button
                  ...
                  :class="realtimeWindow === window
                    ...
                    : 'bg-gray-200 text-gray-600 hover:bg-gray-300 dark:bg-neutral-900 dark:text-neutral-400 dark:hover:bg-dark-600'"
            ...
            <div :class="props.fullscreen ? 'space-y-4' : 'space-y-3'">
              <!-- Row 1: Current -->
              <div>
                <div :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-bold uppercase text-neutral-400']">{{ t('admin.ops.current') }}</div>
                <div class="mt-1 flex flex-wrap items-baseline gap-x-4 gap-y-2">
                  <div class="flex items-baseline gap-1.5">
                    <span :class="[props.fullscreen ? 'text-4xl' : 'text-xl sm:text-2xl', 'font-black text-neutral-100']">{{ displayRealTimeQps.toFixed(1) }}</span>
                    <span :class="[props.fullscreen ? 'text-sm' : 'text-xs', 'font-bold text-neutral-500']">QPS</span>
                  </div>
                  <div class="flex items-baseline gap-1.5">
                    <span :class="[props.fullscreen ? 'text-4xl' : 'text-xl sm:text-2xl', 'font-black text-neutral-100']">{{ displayRealTimeTps.toFixed(1) }}</span>
                    <span :class="[props.fullscreen ? 'text-sm' : 'text-xs', 'font-bold text-neutral-500']">{{ t('admin.ops.tps') }}</span>
                  </div>
                </div>
              </div>
              ...
              <div class="grid grid-cols-2 gap-3">
                <!-- Peak -->
                <div>
                  <div :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-bold uppercase text-neutral-400']">{{ t('admin.ops.peak') }}</div>
                  <div :class="[props.fullscreen ? 'text-base' : 'text-sm', 'mt-1 space-y-0.5 font-medium text-neutral-400']">
                    <div class="flex items-baseline gap-1.5">
                      <span class="font-black text-neutral-100">{{ realtimeQpsPeakLabel }}</span>
                      <span class="text-xs">QPS</span>
                    </div>
                    <div class="flex items-baseline gap-1.5">
                      <span class="font-black text-neutral-100">{{ realtimeTpsPeakLabel }}</span>
                ...
                <!-- Average -->
                <div>
                  <div :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-bold uppercase text-neutral-400']">{{ t('admin.ops.average') }}</div>
                  <div :class="[props.fullscreen ? 'text-base' : 'text-sm', 'mt-1 space-y-0.5 font-medium text-neutral-400']">
                    <div class="flex items-baseline gap-1.5">
                      <span class="font-black text-neutral-100">{{ realtimeQpsAvgLabel }}</span>
                      <span class="text-xs">QPS</span>
                    </div>
                    <div class="flex items-baseline gap-1.5">
                      <span class="font-black text-neutral-100">{{ realtimeTpsAvgLabel }}</span>
                      <span class="text-xs">{{ t('admin.ops.tps') }}</span>
                    </div>
                  </div>
                </div>
      ...
      <div class="grid h-full grid-cols-1 content-center gap-4 sm:grid-cols-2 lg:col-span-7 lg:grid-cols-3">
        <!-- Card 1: Requests -->
        <div class="rounded-2xl bg-gray-50 p-4 dark:bg-[#0d0d0d]" style="order: 1;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-neutral-400">{{ t('admin.ops.requestsTitle') }}</span>
              <HelpTooltip v-if="!props.fullscreen" :content="t('admin.ops.tooltips.totalRequests')" />
            </div>
            <button
              v-if="!props.fullscreen"
              class="text-[10px] font-bold text-[#81a1c1]/80 hover:underline"
              type="button"
          ...
          <div class="mt-2 space-y-2 text-xs">
            <div class="flex justify-between">
              <span class="text-neutral-500">{{ t('admin.ops.requests') }}:</span>
              <span class="font-bold text-neutral-100">{{ totalRequestsLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-neutral-500">{{ t('admin.ops.tokens') }}:</span>
              <span class="font-bold text-neutral-100">{{ totalTokensLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-neutral-500">{{ t('admin.ops.avgQps') }}:</span>
              <span class="font-bold text-neutral-100">{{ qpsAvgLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-neutral-500">{{ t('admin.ops.avgTps') }}:</span>
              <span class="font-bold text-neutral-100">{{ tpsAvgLabel }}</span>
            </div>
          </div>
        </div>

        <!-- Card 2: SLA -->
        <div class="rounded-2xl bg-gray-50 p-4 dark:bg-[#0d0d0d]" style="order: 2;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
              <span class="text-[10px] font-bold uppercase text-neutral-400">{{ t('admin.ops.sla') }}</span>
              <HelpTooltip v-if="!props.fullscreen" :content="t('admin.ops.tooltips.sla')" />
              <span class="h-1.5 w-1.5 rounded-full" :class="getSLAThresholdLevel(slaPercent) === 'critical' ? 'bg-[#bf616a]' : getSLAThresholdLevel(slaPercent) === 'warning' ? 'bg-[#ebcb8b]' : 'bg-[#a3be8c]'"></span>
            </div>
            <button
              v-if="!props.fullscreen"
              class="text-[10px] font-bold text-[#81a1c1]/80 hover:underline"
          ...
          <div class="mt-3 h-2 w-full overflow-hidden rounded-full bg-neutral-800">
            <div class="h-full transition-all" :class="getSLAThresholdLevel(slaPercent) === 'critical' ? 'bg-[#bf616a]' : getSLAThresholdLevel(slaPercent) === 'warning' ? 'bg-[#ebcb8b]' : 'bg-[#a3be8c]'" :style="{ width: `${Math.max((slaPercent ?? 0) - 90, 0) * 10}%` }"></div>
          </div>
          <div class="mt-3 text-xs">
            <div class="flex justify-between">
              <span class="text-neutral-500">{{ t('admin.ops.exceptions') }}:</span>
              <span class="font-bold text-[#bf616a]/80 dark:text-[#bf616a]">{{ formatNumber((overview.request_count_sla ?? 0) - (overview.success_count ?? 0)) }}</span>
            </div>
          </div>
        </div>

        <!-- Card 4: Request Duration -->
        <div class="rounded-2xl bg-gray-50 p-4 dark:bg-[#0d0d0d]" style="order: 4;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-neutral-400">{{ t('admin.ops.latencyDuration') }}</span>
          ...
          <div class="mt-2 flex items-baseline gap-2">
            <div class="text-3xl font-black text-neutral-100">
              {{ durationP99Ms ?? '-' }}
            </div>
            <span class="text-xs font-bold text-neutral-400">ms (P99)</span>
          </div>
          <div class="mt-3 grid grid-cols-1 gap-x-3 gap-y-1 text-xs 2xl:grid-cols-2">
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-neutral-500">P95:</span>
              <span class="font-bold text-neutral-100">{{ durationP95Ms ?? '-' }}</span>
              <span class="text-neutral-400">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-neutral-500">P90:</span>
              <span class="font-bold text-neutral-100">{{ durationP90Ms ?? '-' }}</span>
              <span class="text-neutral-400">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-neutral-500">P50:</span>
              <span class="font-bold text-neutral-100">{{ durationP50Ms ?? '-' }}</span>
              <span class="text-neutral-400">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-neutral-500">Avg:</span>
              <span class="font-bold text-neutral-100">{{ durationAvgMs ?? '-' }}</span>
              <span class="text-neutral-400">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-neutral-500">Max:</span>
              <span class="font-bold text-neutral-100">{{ durationMaxMs ?? '-' }}</span>
              <span class="text-neutral-400">ms</span>
            </div>
          </div>
        </div>
        ...
        <!-- Card 5: TTFT -->
        <div class="rounded-2xl bg-gray-50 p-4 dark:bg-[#0d0d0d]" style="order: 5;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-neutral-400">TTFT</span>
        ...
        <div class="rounded-2xl bg-gray-50 p-4 dark:bg-[#0d0d0d]" style="order: 3;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-neutral-400">{{ t('admin.ops.requestErrors') }}</span>
              <HelpTooltip v-if="!props.fullscreen" :content="t('admin.ops.tooltips.errors')" />
            </div>
            <button v-if="!props.fullscreen" class="text-[10px] font-bold text-[#81a1c1]/80 hover:underline" type="button" @click="openErrorDetails('request')">
              {{ t('admin.ops.requestDetails.details') }}
            </button>
          </div>
          ...
          <div class="mt-3 space-y-1 text-xs">
            <div class="flex justify-between">
              <span class="text-neutral-500">{{ t('admin.ops.errorCount') }}:</span>
              <span class="font-bold text-neutral-100">{{ formatNumber(overview.error_count_sla ?? 0) }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-neutral-500">{{ t('admin.ops.businessLimited') }}:</span>
              <span class="font-bold text-neutral-100">{{ formatNumber(overview.business_limited_count ?? 0) }}</span>
            </div>
          </div>
        </div>

        <!-- Card 6: Upstream Errors -->
        <div class="rounded-2xl bg-gray-50 p-4 dark:bg-[#0d0d0d]" style="order: 6;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-neutral-400">{{ t('admin.ops.upstreamErrors') }}</span>
              <HelpTooltip v-if="!props.fullscreen" :content="t('admin.ops.tooltips.upstreamErrors')" />
            </div>
            <button v-if="!props.fullscreen" class="text-[10px] font-bold text-[#81a1c1]/80 hover:underline" type="button" @click="openErrorDetails('upstream')">
              {{ t('admin.ops.requestDetails.details') }}
            </button>
          </div>
          ...
          <div class="mt-3 space-y-1 text-xs">
            <div class="flex justify-between">
              <span class="text-neutral-500">{{ t('admin.ops.errorCountExcl429529') }}:</span>
              <span class="font-bold text-neutral-100">{{ formatNumber(overview.upstream_error_count_excl_429_529 ?? 0) }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-neutral-500">429/529:</span>
              <span class="font-bold text-neutral-100">{{ formatNumber((overview.upstream_429_count ?? 0) + (overview.upstream_529_count ?? 0)) }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-if="overview" class="mt-2 border-t border-gray-100 pt-4 dark:border-neutral-800/60">
      <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        <!-- CPU -->
        <div class="rounded-xl bg-gray-50 p-3 dark:bg-[#0d0d0d]">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-neutral-400">CPU</div>
          ...
          <div v-if="!props.fullscreen" class="mt-1 text-[10px] text-neutral-400">
        ...
        <div class="rounded-xl bg-gray-50 p-3 dark:bg-[#0d0d0d]">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-neutral-400">{{ t('admin.ops.memory') }}</div>
        ...
        <div class="rounded-xl bg-gray-50 p-3 dark:bg-[#0d0d0d]">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-neutral-400">{{ t('admin.ops.db') }}</div>
        ...
        <div class="rounded-xl bg-gray-50 p-3 dark:bg-[#0d0d0d]">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-neutral-400">Redis</div>
        ...
        <div class="rounded-xl bg-gray-50 p-3 dark:bg-[#0d0d0d]">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-neutral-400">{{ t('admin.ops.goroutines') }}</div>
        ...
        <div class="rounded-xl bg-gray-50 p-3 dark:bg-[#0d0d0d]">
          <div class="flex items-center justify-between gap-2">
            <div class="flex items-center gap-1">
              <div class="text-[10px] font-bold uppercase tracking-wider text-neutral-400">{{ t('admin.ops.jobs') }}</div>
              <HelpTooltip v-if="!props.fullscreen" :content="t('admin.ops.tooltips.jobs')" />
            </div>
            <button v-if="!props.fullscreen" class="text-[10px] font-bold text-[#81a1c1]/80 hover:underline" type="button" @click="openJobsDetails">
              {{ t('admin.ops.requestDetails.details') }}
            </button>
          </div>
    ...
    <BaseDialog :show="showJobsDetails" :title="t('admin.ops.jobs')" width="wide" @close="showJobsDetails = false">
      <div v-if="!jobHeartbeats.length" class="text-sm text-neutral-400">
        {{ t('admin.ops.noData') }}
      </div>
      <div v-else class="space-y-3">
        <div
          ...
          :key="hb.job_name"
          class="rounded-xl border border-gray-100 bg-white p-4 dark:border-neutral-800/60 dark:bg-[#0d0d0d]"
        >
          <div class="flex items-center justify-between gap-3">
            <div class="truncate text-sm font-semibold text-neutral-100">{{ hb.job_name }}</div>
          ...
          <div class="mt-2 grid grid-cols-1 gap-2 text-xs text-neutral-300 sm:grid-cols-2">
            <div>
          ...
          <div
            v-if="hb.last_error"
            class="mt-3 rounded-lg bg-[#bf616a]/12 p-2 text-xs text-[#b48ead]/80 dark:bg-[#b48ead]/12 dark:text-[#b48ead]"
          >
            {{ hb.last_error }}
          </div>
        </div>
      ...
      <div class="space-y-4">
        <div>
          ...
          <input
            ...
            type="datetime-local"
            class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:border-[#81a1c1]/20 focus:outline-none focus:ring-1 focus:ring-neutral-600 dark:border-neutral-800/60 dark:bg-neutral-800 dark:text-white"
          />
        </div>
        <div>
          <label class="block text-sm font-medium text-neutral-300 mb-1">
        ...
        <div class="flex justify-end gap-3 pt-2">
          <button
            type="button"
            class="rounded-lg bg-gray-100 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-neutral-800 dark:text-neutral-300 dark:hover:bg-dark-600"
          ...
          <button
            type="button"
            class="rounded-lg bg-[#81a1c1] px-4 py-2 text-sm font-medium text-white hover:bg-[#81a1c1]/15"
