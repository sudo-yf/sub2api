<template>
  <div class="flex h-full min-h-0 flex-col bg-[#0d0d0d]">
    ...
    <div v-else class="flex min-h-0 flex-1 flex-col">
      <div class="min-h-0 flex-1 overflow-auto border-b border-neutral-800/60">
        <table class="w-full border-separate border-spacing-0">
          <thead class="sticky top-0 z-10 bg-[#0d0d0d]">
            <tr>
              <th class="border-b border-gray-200 px-4 py-2.5 text-left text-[11px] font-bold uppercase tracking-wider text-neutral-500 dark:border-neutral-800/60 dark:text-neutral-400">
              ...
              <th class="border-b border-gray-200 px-4 py-2.5 text-right text-[11px] font-bold uppercase tracking-wider text-neutral-500 dark:border-neutral-800/60 dark:text-neutral-400">
          ...
          <tbody class="divide-y divide-neutral-800/60">
            <tr v-if="rows.length === 0">
              <td colspan="10" class="py-12 text-center text-sm text-neutral-500">
            ...
            <tr
              v-for="log in rows"
              :key="log.id"
              class="group cursor-pointer transition-colors hover:bg-[#161616]"
            ...
            >
              ...
              <td class="whitespace-nowrap px-4 py-2">
                <el-tooltip :content="log.request_id || log.client_request_id" placement="top" :show-after="500">
                  <span class="font-mono text-xs font-medium text-neutral-200">
              ...
              <td class="px-4 py-2">
                <div class="max-w-[160px]">
                  <el-tooltip v-if="log.inbound_endpoint" :content="formatEndpointTooltip(log)" placement="top" :show-after="500">
                    <span class="truncate font-mono text-[11px] text-neutral-300">
              ...
              <!-- Platform -->
              <td class="whitespace-nowrap px-4 py-2">
                <span class="inline-flex items-center rounded bg-neutral-900 px-1.5 py-0.5 text-[10px] font-bold uppercase text-neutral-300">
                  {{ log.platform || '-' }}
                </span>
              </td>
                ...
                <div class="max-w-[160px]">
                  <template v-if="hasModelMapping(log)">
                    <el-tooltip :content="modelMappingTooltip(log)" placement="top" :show-after="500">
                      <span class="flex items-center gap-1 truncate font-mono text-[11px] text-neutral-300">
                        <span class="truncate">{{ log.requested_model }}</span>
                        <span class="flex-shrink-0 text-neutral-500">→</span>
                        <span class="truncate text-neutral-300 dark:text-neutral-400">{{ log.upstream_model }}</span>
                      </span>
                    </el-tooltip>
                  </template>
                  <template v-else>
                    <span v-if="displayModel(log)" class="truncate font-mono text-[11px] text-neutral-300" :title="displayModel(log)">
                      {{ displayModel(log) }}
                    </span>
                    <span v-else class="text-xs text-neutral-500">-</span>
                  </template>
              ...
              <td class="px-4 py-2">
                 <el-tooltip v-if="log.group_id" :content="t('admin.ops.errorLog.id') + ' ' + log.group_id" placement="top" :show-after="500">
                  <span class="max-w-[100px] truncate text-xs font-medium text-neutral-200">
                    {{ log.group_name || '-' }}
                  </span>
                </el-tooltip>
                <span v-else class="text-xs text-neutral-500">-</span>
              ...
              <td class="whitespace-nowrap px-4 py-2">
                <div class="flex items-center gap-1.5">
                  ...
                  <span
                    ...
                    class="rounded bg-neutral-900 px-1.5 py-0.5 text-[10px] font-bold text-neutral-300"
                  >
              ...
              <td class="px-4 py-2">
                <div class="max-w-[200px]">
                  <p class="truncate text-[11px] font-medium text-neutral-400" :title="log.message">
              ...
              <td class="whitespace-nowrap px-4 py-2 text-right" @click.stop>
                <div class="flex items-center justify-end gap-3">
                  <button type="button" class="text-neutral-300 hover:text-neutral-200 dark:text-neutral-400 text-xs font-bold" @click="emit('openErrorDetail', log.id)">
...
function getTypeBadge(log: OpsErrorLog): { label: string; className: string } {
  ...
  if (isUpstreamRow(log)) {
    return { label: t('admin.ops.errorLog.typeUpstream'), className: 'bg-[#bf616a] text-white ring-red-600/20 dark:bg-[#bf616a]/12 dark:text-[#bf616a] dark:ring-red-500/30' }
  }
  if (phase === 'request' && owner === 'client') {
    return { label: t('admin.ops.errorLog.typeRequest'), className: 'bg-[#ebcb8b] text-neutral-800 ring-amber-600/20 dark:bg-[#ebcb8b]/15 dark:text-[#ebcb8b] dark:ring-amber-500/30' }
  }
  if (phase === 'auth' && owner === 'client') {
    return { label: t('admin.ops.errorLog.typeAuth'), className: 'bg-[#81a1c1] text-white ring-blue-600/20 dark:bg-[#81a1c1]/12 dark:text-[#81a1c1] dark:ring-neutral-600/30' }
  }
  if (phase === 'routing' && owner === 'platform') {
    return { label: t('admin.ops.errorLog.typeRouting'), className: 'bg-[#b48ead] text-white ring-purple-600/20 dark:bg-[#b48ead]/12 dark:text-[#b48ead] dark:ring-purple-500/30' }
  }
  if (phase === 'internal' && owner === 'platform') {
    return { label: t('admin.ops.errorLog.typeInternal'), className: 'bg-neutral-900 text-neutral-200 ring-neutral-700/40' }
  }

    const fallback = phase || owner || t('common.unknown')
    return { label: fallback, className: 'bg-[#0d0d0d] text-neutral-300 ring-neutral-800/60' }
}
...
function getStatusClass(code: number): string {
  ...
  return 'bg-neutral-900/30 text-neutral-400 ring-neutral-700/30'
}
