...
const colors = computed(() => ({
  ...
  green: '#a3be8c',
  greenAlpha: '#a3be8c20',
  grid: isDarkMode.value ? '#3b4252' : '#e5e9f0',
  text: isDarkMode.value ? '#9099a8' : '#6b7280'
}))
...
const options = computed(() => {
  ...
  return {
    ...
    plugins: {
      ...
      tooltip: {
        backgroundColor: isDarkMode.value ? '#2e3440' : '#ffffff',
        titleColor: isDarkMode.value ? '#e5e9f0' : '#2e3440',
        bodyColor: isDarkMode.value ? '#d8dee9' : '#4b5563',
        borderColor: c.grid,
        borderWidth: 1,
        padding: 10,
        displayColors: true,
  ...
  <div class="flex h-full flex-col rounded-3xl bg-white p-6 shadow-sm ring-1 ring-gray-900/5 dark:bg-neutral-800 dark:ring-dark-700">
    <div class="mb-4 flex shrink-0 items-center justify-between">
      ...
      <div class="flex items-center gap-2 text-xs text-neutral-500 dark:text-neutral-400">
        <span class="flex items-center gap-1"><span class="h-2 w-2 rounded-full bg-[#81a1c1]"></span>QPS</span>
        <span class="flex items-center gap-1"><span class="h-2 w-2 rounded-full bg-[#a3be8c]"></span>{{ t('admin.ops.tpsK') }}</span>
        <template v-if="!props.fullscreen">
          <button
            type="button"
            class="ml-2 inline-flex items-center rounded-lg border border-gray-200 bg-white px-2 py-1 text-[11px] font-semibold text-gray-600 hover:bg-gray-50 disabled:opacity-50 dark:border-neutral-800/60 dark:bg-[#0d0d0d] dark:text-neutral-300 dark:hover:bg-neutral-800"
          ...
          <button
            type="button"
            class="inline-flex items-center rounded-lg border border-gray-200 bg-white px-2 py-1 text-[11px] font-semibold text-gray-600 hover:bg-gray-50 disabled:opacity-50 dark:border-neutral-800/60 dark:bg-[#0d0d0d] dark:text-neutral-300 dark:hover:bg-neutral-800"
            :disabled="state !== 'ready'"
    ...
    <div v-if="(props.topGroups?.length ?? 0) > 0" class="mb-3 flex flex-wrap gap-2">
      <button
        ...
        :key="g.group_id"
        type="button"
        class="inline-flex items-center gap-2 rounded-full border border-gray-200 bg-white px-3 py-1 text-[11px] font-semibold text-gray-700 hover:bg-gray-50 dark:border-neutral-800/60 dark:bg-[#0d0d0d] dark:text-neutral-500 dark:hover:bg-neutral-800"
        @click="emit('selectGroup', g.group_id)"
      >
        <span class="max-w-[180px] truncate">{{ g.group_name || `#${g.group_id}` }}</span>
        <span class="text-neutral-400 dark:text-neutral-500">{{ formatNumber(g.request_count) }}</span>
      </button>
    </div>
    ...
    <div v-else-if="(props.byPlatform?.length ?? 0) > 0" class="mb-3 flex flex-wrap gap-2">
      ...
      >
        ...
        <span class="text-neutral-400 dark:text-neutral-500">{{ formatNumber(p.request_count) }}</span>
      </button>
    </div>
    ...
    <div class="min-h-0 flex-1">
      ...
      <div v-else class="flex h-full items-center justify-center">
        <div v-if="state === 'loading'" class="animate-pulse text-sm text-neutral-400">{{ t('common.loading') }}</div>
