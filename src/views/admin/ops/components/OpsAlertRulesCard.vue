...
<template>
  <div class="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-gray-900/5 dark:bg-neutral-800 dark:ring-dark-700">
    <div class="mb-4 flex items-start justify-between gap-4">
      <div>
        <h3 class="text-sm font-bold text-neutral-100">{{ t('admin.ops.alertRules.title') }}</h3>
        <p class="mt-1 text-xs text-neutral-400">{{ t('admin.ops.alertRules.description') }}</p>
      </div>
    ...
    <div v-else-if="sortedRules.length === 0" class="rounded-xl border border-dashed border-gray-200 p-8 text-center text-sm text-neutral-500 dark:border-neutral-800/60 dark:text-neutral-400">
      {{ t('admin.ops.alertRules.empty') }}
    </div>

    <div v-else class="max-h-[520px] overflow-hidden rounded-xl border border-neutral-800/60">
      <div class="max-h-[520px] overflow-y-auto">
        <table class="min-w-full divide-y divide-neutral-800/60">
          ...
          <tbody class="divide-y divide-neutral-800/60 bg-[#111111]">
            <tr v-for="row in sortedRules" :key="row.id" class="hover:bg-[#161616]/50">
              <td class="px-4 py-3">
                <div class="text-xs font-bold text-neutral-100">{{ row.name }}</div>
                <div v-if="row.description" class="mt-0.5 line-clamp-2 text-[11px] text-neutral-400">
                  {{ row.description }}
                </div>
                <div v-if="row.updated_at" class="mt-1 text-[10px] text-neutral-500">
                  {{ formatDateTime(row.updated_at) }}
                </div>
              </td>
              <td class="whitespace-nowrap px-4 py-3 text-xs text-neutral-200">
                <span class="font-mono">{{ row.metric_type }}</span>
                <span class="mx-1 text-neutral-500">{{ row.operator }}</span>
                <span class="font-mono">{{ row.threshold }}</span>
              </td>
              <td class="whitespace-nowrap px-4 py-3 text-xs font-bold text-neutral-200">
                {{ row.severity }}
              </td>
              <td class="whitespace-nowrap px-4 py-3 text-xs text-neutral-200">
    ...
    >
      <div class="space-y-4">
        <div v-if="!editorValidation.valid" class="rounded-xl bg-[#bf616a]/12 p-4 text-xs text-white dark:bg-[#bf616a]/12 dark:text-[#bf616a]">
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div>
            ...
            <div v-if="selectedMetricDefinition" class="mt-1 space-y-0.5 text-xs text-neutral-400">
          ...
          <div class="md:col-span-2">
            <label class="input-label">
              ...
              <span v-if="isGroupMetricSelected" class="ml-1 text-[#bf616a]/80">*</span>
            </label>
            <Select
          ...
          <div class="flex items-center justify-between rounded-xl bg-gray-50 px-4 py-3 dark:bg-neutral-800/50 md:col-span-2">
            <span class="text-xs font-bold text-neutral-200">{{ t('admin.ops.alertRules.form.enabled') }}</span>
            <input v-model="draft!.enabled" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-neutral-300 focus:ring-neutral-500" />
          </div>

          <div class="flex items-center justify-between rounded-xl bg-gray-50 px-4 py-3 dark:bg-neutral-800/50 md:col-span-2">
            <span class="text-xs font-bold text-neutral-200">{{ t('admin.ops.alertRules.form.notifyEmail') }}</span>
            <input v-model="draft!.notify_email" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-neutral-300 focus:ring-neutral-500" />
          </div>
        </div>
      </div>
