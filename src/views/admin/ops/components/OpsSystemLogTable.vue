...
const levelBadgeClass = (level: string) => {
  ...
  if (v === 'error' || v === 'fatal') return 'bg-[#bf616a] text-white dark:bg-[#bf616a]/12 dark:text-[#bf616a]'
  if (v === 'warn' || v === 'warning') return 'bg-[#ebcb8b] text-neutral-800 dark:bg-[#ebcb8b]/15 dark:text-[#ebcb8b]'
...
<template>
  <section class="rounded-2xl border border-neutral-800/60 bg-[#111111] p-4">
    <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
      <div>
        <h3 class="text-sm font-bold text-neutral-100">系统日志</h3>
        <p class="mt-1 text-xs text-neutral-400">默认按最新时间倒序，支持筛选搜索与按条件清理。</p>
      </div>
      <div class="flex flex-wrap items-center gap-2 text-xs">
        <span class="rounded-md bg-neutral-900 px-2 py-1 text-neutral-200">队列 {{ health.queue_depth }}/{{ health.queue_capacity }}</span>
        <span class="rounded-md bg-neutral-900 px-2 py-1 text-neutral-200">写入 {{ health.written_count }}</span>
        <span class="rounded-md bg-[#ebcb8b] px-2 py-1 text-[#d08770] dark:bg-[#ebcb8b]/15 dark:text-[#ebcb8b]">丢弃 {{ health.dropped_count }}</span>
        <span class="rounded-md bg-[#bf616a]/12 px-2 py-1 text-white dark:bg-[#bf616a]/12 dark:text-[#bf616a]">失败 {{ health.write_failed_count }}</span>
      </div>
    </div>

    <div class="mb-4 rounded-xl border border-neutral-800/60 bg-[#141414] p-3">
      <div class="mb-2 flex items-center justify-between">
        <div class="text-xs font-semibold text-neutral-200">运行时日志配置（实时生效）</div>
        <span v-if="runtimeLoading" class="text-xs text-neutral-400">加载中...</span>
      </div>
      ...
      <p v-if="health.last_error" class="mt-2 text-xs text-[#bf616a]/80 dark:text-[#bf616a]">最近写入错误：{{ health.last_error }}</p>
    </div>
    ...
    <div class="overflow-hidden rounded-xl border border-neutral-800/60">
      <div v-if="loading" class="px-4 py-8 text-center text-sm text-neutral-500">加载中...</div>
      <div v-else-if="!hasData" class="px-4 py-8 text-center text-sm text-neutral-500">暂无系统日志</div>
      <div v-else class="overflow-auto">
        <table class="min-w-full table-fixed divide-y divide-neutral-800/60">
          <thead class="bg-gray-50 dark:bg-[#0d0d0d]">
            <tr>
              <th class="w-[170px] px-3 py-2 text-left text-[11px] font-semibold text-neutral-500">时间</th>
              <th class="w-[80px] px-3 py-2 text-left text-[11px] font-semibold text-neutral-500">级别</th>
              <th class="px-3 py-2 text-left text-[11px] font-semibold text-neutral-500">日志详细信息</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-100 dark:divide-dark-800">
            <tr v-for="row in logs" :key="row.id" class="align-top">
              <td class="px-3 py-2 text-xs text-neutral-300">{{ formatTime(row.created_at) }}</td>
              <td class="px-3 py-2 text-xs">
                <span class="inline-flex rounded-full px-2 py-0.5 font-semibold" :class="levelBadgeClass(row.level)">
                  {{ row.level }}
                </span>
              </td>
              <td class="px-3 py-2 text-xs text-neutral-300 whitespace-normal break-all">
