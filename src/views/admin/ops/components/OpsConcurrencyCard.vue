...
function getLoadBarClass(loadPct: number): string {
  if (loadPct >= 90) return 'bg-[#bf616a] dark:bg-[#bf616a]/12'
  if (loadPct >= 70) return 'bg-[#ebcb8b] dark:bg-[#ebcb8b]/15'
  if (loadPct >= 50) return 'bg-[#ebcb8b] dark:bg-[#ebcb8b]/12'
  return 'bg-[#a3be8c] dark:bg-[#a3be8c]/12'
}
...
function getLoadTextClass(loadPct: number): string {
  if (loadPct >= 90) return 'text-[#bf616a]/80 dark:text-[#bf616a]'
  if (loadPct >= 70) return 'text-[#d08770]/80 dark:text-[#d08770]'
  if (loadPct >= 50) return 'text-[#ebcb8b]/80 dark:text-[#ebcb8b]'
...
<template>
  <div class="flex h-full flex-col rounded-3xl bg-white p-6 shadow-sm ring-1 ring-gray-900/5 dark:bg-neutral-800 dark:ring-dark-700">
    <!-- 头部 -->
    <div class="mb-4 flex shrink-0 items-center justify-between gap-3">
      <h3 class="flex items-center gap-2 text-sm font-bold text-neutral-100 dark:text-white">
        <svg class="h-4 w-4 text-[#81a1c1]/80" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div class="flex items-center gap-2">
        ...
        <button
          ...
          :class="showByUser
            ...
            : 'bg-gray-100 text-neutral-500 hover:bg-gray-200 hover:text-neutral-400 dark:bg-neutral-900 dark:text-neutral-400 dark:hover:bg-dark-600 dark:hover:text-neutral-400'"
        ...
        <!-- 刷新按钮 -->
        <button
          class="flex items-center gap-1 rounded-lg bg-gray-100 px-2 py-1 text-[11px] font-semibold text-gray-700 transition-colors hover:bg-gray-200 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-neutral-900 dark:text-neutral-300 dark:hover:bg-dark-600"
          :disabled="loading"
    ...
    <!-- 错误提示 -->
    <div v-if="errorMessage" class="mb-3 shrink-0 rounded-xl bg-[#bf616a]/12 p-2.5 text-xs text-white dark:bg-[#bf616a]/12 dark:text-[#bf616a]">
      {{ errorMessage }}
    </div>
    ...
    <div
      ...
      class="flex flex-1 items-center justify-center rounded-xl border border-dashed border-gray-200 text-sm text-neutral-500 dark:border-neutral-800/60 dark:text-neutral-400"
    >
      {{ t('admin.ops.concurrency.disabledHint') }}
    </div>

    <!-- 数据展示区域 -->
    <div v-else class="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-gray-200 dark:border-neutral-800/60">
      <!-- 维度标题栏 -->
      <div class="flex shrink-0 items-center justify-between border-b border-gray-200 bg-gray-50 px-3 py-2 dark:border-neutral-800/60 dark:bg-[#0d0d0d]">
      ...
      <!-- 空状态 -->
      <div v-if="displayRows.length === 0" class="flex flex-1 items-center justify-center text-sm text-neutral-500 dark:text-neutral-400">
      ...
      <div v-else-if="displayDimension === 'user'" class="custom-scrollbar max-h-[360px] flex-1 space-y-2 overflow-y-auto p-3">
        <div v-for="row in (displayRows as UserRow[])" :key="row.key" class="rounded-lg bg-gray-50 p-2.5 dark:bg-[#0d0d0d]">
          <!-- 用户信息和并发 -->
          <div class="mb-1.5 flex items-center justify-between gap-2">
            <div class="flex min-w-0 flex-1 items-center gap-1.5">
              <span class="truncate text-[11px] font-bold text-neutral-100 dark:text-white" :title="row.username || row.user_email">
                {{ row.username || row.user_email }}
              </span>
              <span v-if="row.username" class="shrink-0 truncate text-[10px] text-neutral-400 dark:text-neutral-500" :title="row.user_email">
                {{ row.user_email }}
              </span>
            </div>
            <div class="flex shrink-0 items-center gap-2 text-[10px]">
              <span class="font-mono font-bold text-neutral-100 dark:text-white"> {{ row.current_in_use }}/{{ row.max_capacity }} </span>
              <span :class="['font-bold', getLoadTextClass(row.load_percentage)]"> {{ Math.round(row.load_percentage) }}% </span>
            </div>
          </div>

          <!-- 进度条 -->
          <div class="h-1.5 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-neutral-900">
          ...
          <div v-if="row.waiting_in_queue > 0" class="mt-1.5 flex justify-end">
            <span class="rounded-full bg-[#b48ead]/12 px-1.5 py-0.5 text-[10px] font-semibold text-white dark:bg-[#b48ead]/12 dark:text-[#b48ead]">
      ...
      <div v-else-if="displayDimension === 'platform' || displayDimension === 'group'" class="custom-scrollbar max-h-[360px] flex-1 space-y-2 overflow-y-auto p-3">
        <div v-for="row in (displayRows as SummaryRow[])" :key="row.key" class="rounded-lg bg-gray-50 p-3 dark:bg-[#0d0d0d]">
          <!-- 标题行 -->
          <div class="mb-2 flex items-center justify-between gap-2">
            <div class="flex items-center gap-2">
              <div class="truncate text-[11px] font-bold text-neutral-100 dark:text-white" :title="row.name">
                {{ row.name }}
              </div>
              <span v-if="displayDimension === 'group' && row.platform" class="text-[10px] text-neutral-400 dark:text-neutral-500">
                {{ row.platform.toUpperCase() }}
              </span>
            </div>
            <div class="flex shrink-0 items-center gap-2 text-[10px]">
              <span class="font-mono font-bold text-neutral-100 dark:text-white"> {{ row.used_concurrency }}/{{ row.total_concurrency }} </span>
              <span :class="['font-bold', getLoadTextClass(row.concurrency_percentage)]"> {{ row.concurrency_percentage }}% </span>
            </div>
          </div>

          <!-- 进度条 -->
          <div class="mb-2 h-1.5 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-neutral-900">
            <div
          ...
          <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px]">
            ...
            <div class="flex items-center gap-1">
              <svg class="h-3 w-3 text-neutral-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path
              ...
              <span class="text-gray-600 dark:text-neutral-300">
                <span class="font-bold text-[#a3be8c]/80 dark:text-[#a3be8c]">{{ row.available_accounts }}</span
                >/{{ row.total_accounts }}
              </span>
              <span class="text-neutral-400 dark:text-neutral-500">{{ row.availability_percentage }}%</span>
            </div>

            <!-- 限流账号 -->
            <span
              v-if="row.rate_limited_accounts > 0"
              class="rounded-full bg-[#ebcb8b] px-1.5 py-0.5 font-semibold text-[#d08770] dark:bg-[#ebcb8b]/15 dark:text-[#ebcb8b]"
            >
            ...
            <span
              ...
              class="rounded-full bg-[#bf616a]/12 px-1.5 py-0.5 font-semibold text-white dark:bg-[#bf616a]/12 dark:text-[#bf616a]"
            >
            ...
            <span
              ...
              class="rounded-full bg-[#b48ead]/12 px-1.5 py-0.5 font-semibold text-white dark:bg-[#b48ead]/12 dark:text-[#b48ead]"
            >
              {{ t('admin.ops.concurrency.queued', { count: row.waiting_in_queue }) }}
            </span>
          </div>
      ...
      <div v-else class="custom-scrollbar max-h-[360px] flex-1 space-y-2 overflow-y-auto p-3">
        <div v-for="row in (displayRows as AccountRow[])" :key="row.key" class="rounded-lg bg-gray-50 p-2.5 dark:bg-[#0d0d0d]">
          <!-- 账号名称和并发 -->
          <div class="mb-1.5 flex items-center justify-between gap-2">
            <div class="min-w-0 flex-1">
              ...
              <div class="mt-0.5 text-[9px] text-neutral-400 dark:text-neutral-500">
                {{ row.group_name }}
              </div>
            </div>
            <div class="flex shrink-0 items-center gap-2">
              <!-- 并发使用 -->
              <span class="font-mono text-[11px] font-bold text-neutral-100 dark:text-white"> {{ row.current_in_use }}/{{ row.max_capacity }} </span>
              <!-- 状态徽章 -->
              <span
                v-if="row.is_available"
                class="inline-flex items-center gap-1 rounded bg-[#a3be8c]/12 px-1.5 py-0.5 text-[10px] font-medium text-white dark:bg-[#a3be8c]/12 dark:text-[#a3be8c]"
              >
              ...
              <span
                ...
                class="inline-flex items-center gap-1 rounded bg-[#ebcb8b] px-1.5 py-0.5 text-[10px] font-medium text-[#d08770] dark:bg-[#ebcb8b]/15 dark:text-[#ebcb8b]"
              >
                <svg class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <span
                ...
                class="inline-flex items-center gap-1 rounded bg-[#bf616a]/12 px-1.5 py-0.5 text-[10px] font-medium text-white dark:bg-[#bf616a]/12 dark:text-[#bf616a]"
              >
                <svg class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path
                    stroke-linecap="round"
              ...
              <span
                v-else
                class="inline-flex items-center gap-1 rounded bg-gray-100 px-1.5 py-0.5 text-[10px] font-medium text-gray-700 dark:bg-[#1e1e1e] dark:text-neutral-400"
              >
