...
import {
  getPreviewDashboardModels,
  getPreviewDashboardStats,
  getPreviewDashboardTrend,
  getPreviewKeyUsageMap,
  getPreviewUsageLogs,
  getPreviewUsageStats,
  isDevWuiPreviewEnabled
...
export async function query(
  ...
  if (isDevWuiPreviewEnabled()) {
    return Promise.resolve(getPreviewUsageLogs())
  }
  const { data } = await apiClient.get<PaginatedResponse<UsageLog>>('/usage', {
    ...config,
    params
...
export async function getStatsByDateRange(
  ...
  if (isDevWuiPreviewEnabled()) {
    return Promise.resolve(getPreviewUsageStats())
  }
...
export async function getDashboardStats(): Promise<UserDashboardStats> {
  if (isDevWuiPreviewEnabled()) {
    return Promise.resolve(getPreviewDashboardStats())
  }
...
export async function getDashboardTrend(params?: TrendParams): Promise<TrendResponse> {
  if (isDevWuiPreviewEnabled()) {
    return Promise.resolve(getPreviewDashboardTrend() as TrendResponse)
  }
...
export async function getDashboardModels(params?: {
  ...
  if (isDevWuiPreviewEnabled()) {
    return Promise.resolve(getPreviewDashboardModels() as ModelStatsResponse)
  }
  ...
  if (isDevWuiPreviewEnabled()) {
    return Promise.resolve({
      stats: Object.fromEntries(
        Object.entries(getPreviewKeyUsageMap()).map(([key, value]) => [
          key,
          { api_key_id: Number(key), ...value }
        ])
      )
    })
  }
