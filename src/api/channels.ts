...
import { getPreviewChannels, isDevWuiPreviewEnabled } from '@/utils/devPreview'
...
export interface UserAvailableGroup {
  ...
  /** true = 当前用户具备绑定该分组的权限；false = 仅展示价格/状态，不代表可用。 */
  can_bind?: boolean
}
...
export async function getAvailable(options?: { signal?: AbortSignal }): Promise<UserAvailableChannel[]> {
  if (isDevWuiPreviewEnabled()) {
    return Promise.resolve(getPreviewChannels())
  }
